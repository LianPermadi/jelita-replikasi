...
<form action="<?php echo site_url('approve_nonesign/cari_data_no_pendaftaran'); ?>" method="post">
      <div class="row">
        <div class="input-field col s12">
...
<input type="submit" name="" value="KIRIM NOTIFIKASI" class="btn red">
	...
	<!--	<select name="action">
		...
		<?php
		echo '<h5><center>APPROVE PERMOHONAN NON ESIGN</center></h5>';
...
if($h==4 || $h==3 || $h==2){
?>
		<br><input type="submit" name="submit"  value="Approve Non Esign"  class="btn"></center>
		<?php
	}else{ 
					...
					foreach ($hasilakdp_cetak as $data):
						...
						<?php echo $i++;?></label>
							...
							<td rowspan="3" style="border-bottom: 1px solid silver;border-collapse: none;"><?php //echo $data->n_perizinan; ?>
								<form action="<?php echo site_url('approve_nonesign/detail_permohonan').'/'.$data->id; ?>" method="post">
		<input type="submit" name="submit"  value="Detail"  class="btn">
<!--<a href="<?php echo site_url('../../../kaltara/backoffice/assets/download').'/sk_'.$data->pendaftaran_id.'.docx'; ?>" class="btn">preview sk</a>-->
		</br></br><a href="<?php echo site_url('approve_nonesign').'/preview/'.$data->pendaftaran_id.'/'.$data->id; ?>" class="btn">preview sk</a>
	</form>	
