...
class Approve_permohonan extends CI_Controller {

	public function index()
	{
		
        $userid =  $this->session->userdata('id');
		$this->load->model('m_approve_permohonan');
		$data['hasilakdp_cetak'] = $this->m_approve_permohonan->ambildata($userid);
		$now = date("Y-m-d");
		...
		$data['no_pendaftaran'] = '';
		$this->load->view('v_approve_permohonan', $data);
	}
	...
	public function cari_data()
	{
		...
		$data['tgl2'] = $tg2;
		$this->load->model('m_approve_permohonan');
		$data['hasilakdp_cetak'] = $this->m_approve_permohonan->caridata($tg1,$tg2,$userid);
	...
	function update_multiple_edit($iduser) {
		...
		$this->load->model('m_approve_permohonan');
		$this->m_approve_permohonan->update_permohonan($iduser);
		$username = '';
	...
	function update_multiple($iduser) {
		...
		redirect('approve_permohonan/pesan_approve/gagal');
		}else{
		$this->load->model('m_approve_permohonan');
		$this->m_approve_permohonan->update_permohonan($iduser);
		redirect('approve_permohonan/pesan_esign/berhasil');
	}
	}
	...
	{
		$this->load->model('m_approve_permohonan');
		$data['detail_cetak'] = $this->m_approve_permohonan->detail_data($id);
		$this->load->view('v_detail_akdp', $data);
	}
	public function detail_permohonan($id)
	{
		$this->load->model('m_approve_permohonan');
		$data['detail_cetak'] = $this->m_approve_permohonan->detail_datapermohonan($id);
		$this->load->view('v_detail_permohonan', $data);
	}
	...
	public function cari_data_no_pendaftaran()
	{
		...
		$this->load->model('m_approve_permohonan');
		$data['hasilakdp_cetak'] = $this->m_approve_permohonan->caridata_no_pendaftaran($no_pendaftaran,$userid);
		$this->load->view('v_approve_permohonan', $data);
	}
	...
	function notifikasi(){
		...
		$host             =	"smtp.gmail.com";
			...
			if (!$mailer->send()) {
                ...
                redirect('approve_permohonan/index');
            } else {
	...
	function revisi_approve($id_permohonan){	
		...
		$this->load->model('m_approve_permohonan');
		$data['kode_tmsk']= $this->m_approve_permohonan->ambil_tmsk($id_permohonan);
	...
	function update_revisi(){
		...
		$this->load->model('m_approve_permohonan');
		$this->m_approve_permohonan->update_revisi($id,$msg_revisi,$kode);
	...
	public function list_approve()
	{
        ...
        $userid =  $this->session->userdata('id');
		$this->load->model('m_approve_permohonan');
		$data['hasilakdp_cetak'] = $this->m_approve_permohonan->list_approve($userid);
		$data['tg1'] = date("Y-m-d");;
		$data['tg2'] = date("Y-m-d");
	...
	public function cari_list_approve()
	{
		...
		$this->load->model('m_approve_permohonan');
		$data['hasilakdp_cetak'] = $this->m_approve_permohonan->cari_list_approve($userid,$tg1,$tg2);
		$data['tg1'] = $tg1;
		$data['tg2'] = $tg2;
		$this->load->view('v_listapprove_permohonan', $data);
	}

	function pesan_esign($pesan){
		// Pastikan pesan diteruskan ke view
		$data['pesan'] = $pesan;
		$this->load->view('v_pesan_esign', $data);
	}
