...
class M_approve_permohonan extends CI_Model {
...
//------------------------- ttd digital
        ...
        if ($pos !== false) {
            ...
            $local = $url;
				...
				$path_jar = $local.'/android/assets/esign/signer/JSignPdf.jar';
				$path_pdf = $local.'/backoffice/assets/skpdf/SK_'.$nodaftar[$i].'.pdf'; 
				...
				$path_p12 = $local.'/android/assets/esign/signer/kepala.p12';  
				...
				// 		}else{
				// 			redirect('approve_permohonan/pesan_esign');
				// 		}
        ...
        if($json == NULL && ($httpcode == 200 || $httpcode == 201)) { //berhasil tandatangan 
          $output_filename = $output_path.'SK_'.$nodaftar[$i].'.pdf';
...
$otherdb = $this->load->database('otherdb',TRUE);
  ...
  $otherdb->select('eselon');
		...
		$otherdb->select('tmpermohonan.pendaftaran_id,tmpermohonan.bidang,tmperusahaan.n_perusahaan,tmpermohonan.pendaftaran_id,tmpermohonan.id,trperizinan.e_sertifikat,
