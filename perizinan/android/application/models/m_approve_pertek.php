  ...
  function update_permohonan($iduser) {
    ...
    for($i=0; $i < count($update) ; $i++) {
      ...
      if($h	== 4){
        ...
        $command = 'java -jar "'.$path_jar.'" "'.$path_pdf.'" -kst PKCS12 -ksf "'.$path_p12.'" -ksp "'.$passphrase.'" -l "Dinas PMPTSP Jawa Barat" -r "Pengesahan Naskah Perizinan" -c "kontak yang bisa dihubungi" -tsh SHA256 -ha SHA256 -d "'.$output_path.'" -os "" -ts '.$tsa_url.' -ta PASSWORD -tsu "coba" -tsp "1234"';
        exec($command, $val, $er);
        if($er == 0 || $er == 3){
        }else{
