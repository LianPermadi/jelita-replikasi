...
function ambildata($userid) {
  ...
  $otherdb->select('tmpermohonan.bidang,tmperusahaan.n_perusahaan,tmpermohonan.pendaftaran_id,tmpermohonan.id,trperizinan.n_perizinan,tmpermohonan.d_terima_berkas');
  ...
  date_default_timezone_set("Asia/Bangkok");
    ...
    for($i=0; $i < count($update) ; $i++) {
      if($h == 4){
        ...
        $pdf = new FPDI();
        $filepdf = $_SERVER['DOCUMENT_ROOT'].'/spekta/backoffice/assets/skpdf/SK_'.$nodaftar[$i].'.pdf';
        try{
        ...
        $path_pdf     = $_SERVER['DOCUMENT_ROOT'].'/spekta/backoffice/assets/skpdf/SK_'.$nodaftar.'.pdf'; 
        ...
        if ($nik != '') {
          $command = "java -jar ".$path_jar_new." -m sign -f ".$path_pdf." -d ".$output_path." -p '".$passphrase."' -nik ".$nik." -t invisible -l Tasikmalaya -r Dokumen_di-TTE";

          exec($command, $val, $er);

          if($er == 0 || $er == 3){ //berhasil
            $hasilesign = str_replace('"', "", $val[0]);
            $cop = copy($hasilesign, $output_path."SK_".$nodaftar.".pdf");
            if (!$cop) {
          ...
          $command = 'java -jar "'.$path_jar.'" "'.$path_pdf.'" -kst PKCS12 -ksf "'.$path_p12.'" -ksp "'.$passphrase.'" -l "Dinas PMPTSP Kab.Tasikmalaya" -r "Pengesahan Naskah Perizinan" -c "kontak yang bisa dihubungi" -tsh SHA256 -ha SHA256 -d "'.$output_path.'" -os "" -ts '.$tsa_url.' -ta PASSWORD -tsu "coba" -tsp "1234" --ocsp --ocsp-server-url "'.$ocsp.'"';
