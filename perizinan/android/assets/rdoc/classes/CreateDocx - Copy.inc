...
class CreateDocx extends CreateDocument
{
    ...
    public function __construct($baseTemplatePath = PHPDOCX_BASE_TEMPLATE, $docxTemplatePath = '')
    {
        ...
        if (file_exists(dirname(__FILE__) . '/DOCXStructureTemplate.inc')) {
            if (PHPDOCX_BASE_TEMPLATE == PHPDOCX_BASE_FOLDER . 'phpdocxBaseTemplate.docx' && !isset($docxTemplatePath)) {
        ...
        if (isset($this->_phpdocxconfig['settings']['stream']) && 
            ...
            file_exists(dirname(__FILE__) . '/ZipStream.inc')) {
     ...
     *  'scalingMax' (float) scaling max X axis value for bar and line charts
     *  'scalingMin' (float) scaling min X axis value for bar and line charts
     *  'tickLblPos' tick label position for bar charts
     *  'majorUnit' (float) for bar and line charts
     *  'minorUnit' (float) for bar and line charts
     */
    public function addChart($options = array())
    {
        ...
        try {
            if (isset($options['data']) && isset($options['type'])) {
                ...
                $this->_zipDocx->addFile('word/embeddings/datos' . self::$intIdWord . '.xlsx', $this->_tempFileXLSX[self::$intIdWord]);
    ...
    public function addEndnote($options = array())
    {
        ...
        if ($options['textEndnote'] instanceof WordFragment) {
            ...
            $endnoteBase .= '<w:t xml:space="preserve">' . $options['textEndnote'] . '</w:t></w:r></w:p></w:endnote>';
        }
        if (!is_array($options['textDocument'])) {
            $options['textDocument'] = array('text' => $options['textDocument']);
        }
    ...
    public function addFootnote($options = array())
    {
        ...
        if ($options['textFootnote'] instanceof WordFragment) {
            ...
            $footnoteBase .= '<w:t xml:space="preserve">' . $options['textFootnote'] . '</w:t></w:r></w:p></w:footnote>';
        }
        if (!is_array($options['textDocument'])) {
            $options['textDocument'] = array('text' => $options['textDocument']);
        }
                    ...
                    if ($attrImage['mime'] == 'image/jpg' ||
                        ...
                        if ((string) $image != '') {
                            ...
                            if ($data['target'] == 'defaultHeader' ||
                                ...
                                if (isset($data['hyperlink'])) {
                                    CreateDocx::$_relsNotesLink[$data['target']][] = array('rId' => 'rId' . $data['rIdHyperlink'], 'Target' => $data['hyperlink'], 'url' => $data['hyperlink'], 'TargetMode' => 'External');
                                }
                            } else {
                                $this->generateRELATIONSHIP(
    ...
    public function addMathEquation($equation, $type)
    {
        if ($type == 'docx') {
            ...
            if ($class == 'WordFragment') {
                $this->wordML .= '<m:oMathPara>' . (string) $math . '</m:oMathPara>';
            } else {
                $this->_wordDocumentC .= '<' . CreateDocx::NAMESPACEWORD . ':p>' .
     ...
     * @param array $options Style options to apply to numerical and alphabetical numbering
     */
     ...
     * Keys and values:
     *     'type' (string) can be * (all, default value), break, chart, endnote (content reference, the whole paragraph is removed), footnote (content reference, the whole paragraph is removed), image, paragraph (also for bookmarks, links and lists), section, shape, table
     *     'contains' (string) for paragraph (text, bookmark, link, list), shape
     *     'ocurrence' (int)
     *     'attributes' (array)
    ...
    {
        if (!file_exists(dirname(__FILE__) . '/DOCXPath.inc')) {
        ...
        $referenceWordContentXML = $domDocument->saveXML($contentNodesReferencedWordContent->item(0));

        if ($contentNodesReferencedWordContent <= 0) {
            PhpdocxLogger::logger('The reference node could not be found.', 'info');

            return;
        }

        // get the refereceNodeTo
        ...
        if ($contentNodesReferencedToWordContent->length > 0 || $forceAppend) {
            if ($contentNodesReferencedToWordContent->length <= 0 && $forceAppend) {
                ...
                $referencedWordContentToQuery = $this->getWordContentQuery(array('type' => '*', 'ocurrence' => -1));
                $contentNodesReferencedToWordContent = $domXpath->query($referencedWordContentToQuery);
            }
            ...
            if ($location == 'before') {
                $contentNodesReferencedToWordContent->item(0)->parentNode->insertBefore($cursor, $contentNodesReferencedToWordContent->item(0));
            } else {
                $contentNodesReferencedToWordContent->item(0)->parentNode->insertBefore($cursor, $contentNodesReferencedToWordContent->item(0)->nextSibling);
            }
    ...
    public function createDocx($fileName = 'document')
    {
        ...
        $this->_zipDocx->saveDocx($fileName);
     ...
     * Keys and values:
     *     'type' (string) can be * (all, default value), bookmark, break, chart, endnote (content reference), footnote (content reference), image, paragraph (also for links and lists), section, shape, table
     *     'contains' (string) for bookmark, paragraph (text, link, list), shape
     *     'ocurrence' (int)
     *     'attributes' (array)
     *     'parent' (string) w:body (default), '/' (any parent) or any other specific parent (/w:tbl/, /w:tc/, /w:r/...)
     *     'customQuery' (string) if set overwrites all previous references. It must be a valid XPath query
    ...
    public function insertWordFragment($wordFragment, $referenceNode = array(), $location = 'after', $forceAppend = false)
    {
        ...
        if (isset($referenceNode['customQuery']) && !empty($referenceNode['customQuery'])) {
            ...
            $query = DOCXPath::xpathContentQuery($type, $referenceNode, array('location' => $location));
        }
        ...
        if ($contentNodes->length > 0) {
            if ($location == 'before' || $location == 'inlineBefore' || $location == 'inlineAfter') {
                ...
                if (!($referenceNode instanceof DOMNode) || $referenceNode->nodeName == 'w:sectPr') {
                    $referenceNode = $contentNodes->item(0)->parentNode;
    ...
    public function moveWordContent($referenceNodeFrom, $referenceNodeTo, $location = 'after', $forceAppend = false)
    {
        ...
        if ($contentNodesReferencedToWordContent->length > 0 || $forceAppend) {
            ...
            $contentNodesReferencedWordContent->item(0)->parentNode->removeChild($contentNodesReferencedWordContent->item(0));
    ...
    public function removeWordContent($referenceNode)
    {
        ...
        if ($contentNodes->length > 0) {
            // remove referenceNodeFrom
            $contentNodes->item(0)->parentNode->removeChild($contentNodes->item(0));
     ...
     *      · 'odfconverter' : true (default) or false. Use odf-converter. Only for OpenOffice
     ...
     * @access private
     */
    private function addImageCaption($isWordFragment, $data){
        ...
        if ($isWordFragment) {
            $this->wordML .= (string) $caption;
        } else {
            $this->_wordDocumentC .= (string) $caption;
        }
    }
     ...
     */
    private function appendWordFragment($wordFragment)
    {
        if ($wordFragment instanceof WordFragment) {
    ...
    /**
     * Return a Word content based on the reference
     *
     * @access private
     * @param array $referenceNode
     * Keys and values:
     ...
     * @return DOM or null
     */
    private function getWordContentQuery($referenceNode)
    {
        if (!file_exists(dirname(__FILE__) . '/DOCXPath.inc')) {
            PhpdocxLogger::logger('This method is not available for your license.', 'fatal');
        }
     ...
     * @return void
     */
    private function insertContentToDocument($wordFragment, $domDocument, $source, $refNode, $inlineLocation = false)
    {
        $cursor = $domDocument->createElement('cursor', 'WordFragment');
        if ($inlineLocation == 'inlineAfter') {
            $refNode->parentNode->appendChild($cursor);
     ...
     * @return void
     */
    private function removeContentInDocument($domDocument, $refNode)
    {
