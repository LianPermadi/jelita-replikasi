...
class DocxUtilities
{
     ...
     * @access private
     */
     private $_sectionProperties = array('w:footnotePr',
                                              'w:endnotePr',
                                              'w:type',
                                              'w:pgSz',
                                              'w:pgMar',
   ...
   /**
     ...
     * @param array $options, 
     * Values:
     * 'enforceSectionPageBreak' (boolean) if true enforces section page breaks even if the section type is continuous
    ...
    /**
     * Modifies de general settings of an existing Word document
     *
     * @access public
     * @example ../examples/easy/modifyWordSettings.php
    ...
    { 
        ...
        foreach($settingParameters as $tag => $value){
            ...
            $settingIndex = array_search('w:' . $tag, CreateDocx::$settings);           
            ...
            if($selectedElements->length == 0){
                ...
                foreach($childNodes as $node){
                    $name = $node->nodeName;
                    $index = array_search($node->nodeName, CreateDocx::$settings);
     ...
     *
     * @access public
     * @example ../examples/easy/setlineNumbering.php
     * @param string $document path to the document
     * @param string $finalDocument path to the resulting document
     * @param array $options 
    ...
    {    
        ...
        if($lnNumNodes->length > 0){
            foreach($lnNumNodes as $lnNumNodeBase){
                ...
                $propIndex = array_search('w:lnNumType', CreateDocx::$sectionProperties);           
                $childNodes = $lnNumNodeBase->childNodes;
                $index = false;
                foreach($childNodes as $node){
                    $name = $node->nodeName;
                    $index = array_search($node->nodeName, CreateDocx::$sectionProperties);
                    if($index > $propIndex){
     ...
     * @access public
     * @example ../examples/intermediate/ParseCheckboxes.php
                ...
                if(isset($checkboxes[$counter])){
                   if($node->parentNode->nodeName != 'w:sdtPr'){
                    $node->setAttribute('w:val', $checkboxes[$counter]); 
                     ...
                     $nodeVals = $node->getElementsByTagNameNS('http://schemas.microsoft.com/office/word/2010/wordml',
                     'checked'); 
                     ...
                     $txt = $sdt->getElementsByTagNameNS('http://schemas.openxmlformats.org/wordprocessingml/2006/main',
                        't');
...
/**
     ...
     *
     * @access public
     * @example ../examples/easy/RemoveLineNumbering.php
     * @param string $document path to the document
     * @param string $finalDocument path to the resulting document
     */
        ...
        private function mergeDocuments($firstDocx, $secondDocx, $options)
        {
             ...
             if(isset($options['numbering']) && $options['numbering'] == 'restart'){
                 ...
                 if($numberingNodes->length > 0){
                    ...
                    //insert the node
                    $tagIndex = array_search('w:pgNumType', $this->_sectionProperties);           
                    $childNodes = $secondDocx['sectionProperties'][1]->documentElement->childNodes;
                    $index = false;
                    foreach($childNodes as $node){
                        $name = $node->nodeName;
                        $index = array_search($node->nodeName, $this->_sectionProperties);
                        if($index > $tagIndex){
             ...
             if($options['enforceSectionPageBreak']){
                ...
                if($typeNodes->length > 0){
                    ...
                    //insert the node
                    $tagIndex = array_search('w:type', $this->_sectionProperties);           
                    $childNodes = $secondDocx['sectionProperties'][1]->documentElement->childNodes;
                    $index = false;
                    foreach($childNodes as $node){
                        $name = $node->nodeName;
