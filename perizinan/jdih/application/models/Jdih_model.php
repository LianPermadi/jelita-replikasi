...
left join tmpegawai B on replace(B.nip,' ', '') = replace(A.id_pegawai,' ', '') ")->result();*/
    return $this->db->query(" SELECT * from jdih ")->result();
}
    public function get_kategori2(){
                return $this->db->query(" SELECT *
        FROM jdih_kategori")->result();
    }
    
    public function getfilterkategori($id){
        return $this->db->query(" SELECT * from jdih where kategori = '$id' ")->result();
    }
    ...
    public function get_kategori($id) {
     ...
     public function getdetail_jdih($id){
        return $this->db->query(" SELECT *
        FROM jdih where id = '$id' ")->result();
