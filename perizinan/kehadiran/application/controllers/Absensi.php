...
class Absensi extends CI_Controller { 
  ...
  public function save(){
    ...
    if($responseKeys["success"]) { //Jika captcha berhasil
      ...
      if ($simpan != 0) {
        if (!empty($signed)) {
          $routees_portal = str_replace('index.php', '', $_SERVER['SCRIPT_NAME']); 
          $folderPath = $_SERVER['DOCUMENT_ROOT'].$routees_portal.'uploads/';
          ...
          if (file_put_contents($file, $image_base64)) {
            ...
            // ❌ Gagal simpan file tanda tangan, hapus data dari DB
            $this->db->where('id', $simpan)->delete('absensi_mpp');

            $this->session->set_flashdata('error', "Terjadi kesalahan saat menyimpan tanda tangan. Data telah dibatalkan.");
            redirect('/absensi/info/'.$kegiatan);
          }
        } else {
          // Tidak ada tanda tangan, tetap dianggap berhasil
          $this->session->set_flashdata('success', "Data Berhasil Tersimpan, Terima Kasih.");
          redirect('/absensi/info/'.$kegiatan);
        }
      } else {
