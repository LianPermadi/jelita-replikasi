 ...
 *
 * @author PBS
 * Created : 4 Maret 2012Fredirec
 */
class Upload_arsip extends WRC_AdminCont {
    var $obj;
    ...
    public function edit($edt = NULL, $id_daftar = NULL, $asal_menu = NULL) {
        ...
        $this->load->vars($data);
		if($edt == 'E')
            $this->session_info['page_name'] = "Dokumen Arsip";
		else
    ...
    public function save() {
        ...
        redirect('upload_arsip/arsip/list_index');
//        $this->uploadfile();
    }
...
//    function showform($data=array()){
	function showform($hit = Null){
		...
		$data["judulapp"]=$jdl;
		$data["scriptaksi"]="upload_arsip/uploadfile";
		$data["aksi"]="Upload";
		$data["error"]=(isset($data["error"]))?$data["error"]:"";
		$viewfile="v_cupload_form";
		$this->load->view($viewfile,$data);
    ...
    function deleteform($hit = Null){
        ...
        while ($a < 50) {
           ...
           $permohonan->save();
           redirect('upload_arsip/edit'.'/E/'.$id_daftar.'/0/');   // E:edit; L:List, 0:Menu Asal 
	}
...
//        $config['allowed_types'] = 'csv|txt|gif|jpg|png|pdf|doc|docx|xls|xlsx|ppt|pptx|ini';
    ...
    function upload_syarat(){
      ...
      $dir = 'assets/upload/syarat/'.$id_perusahaan.' '.$nama_perusahaan;
      $nopendaftaran = $this->input->post('nopendaftaran');
        ...
        $namafolder=$dir.'/'; //tempat menyimpan file
    for ($n=0; $n < count($no) ; $n++) {
    //for($i=0; $i<count($_FILES['nama_file']); $i++) 
   // {
        if (!empty($_FILES["nama_file"]["tmp_name"][$n]))
        {

            $r1 = str_replace('/',' ',$syarat[$n]);
            $r2 = str_replace(':',' ',$r1);
            $r3 = str_replace('*',' ',$r2);
            $r4 = str_replace('?',' ',$r3);
            $r5 = str_replace('"',' ',$r4);
            ...
            $namafile = str_replace('"\"',' ',$r9);
             ...
             //   $gambar = $namafolder . basename('tes'.$_FILES['nama_file']['name'][$i]);
   $gambar = $namafolder . basename($nopendaftaran.' '.substr($namafile,0,150).'.'. strtolower(end(explode('.', $_FILES['nama_file']['name'][$n]))));

                
                if (move_uploaded_file($_FILES['nama_file']['tmp_name'][$n], $gambar)) {
                     
                            $this->db->select('*');
                            $this->db->from('t_upload_arsip');
                            $this->db->where('id_pendaftaran',$nopendaftaran);
                            $this->db->where('nama_file',basename($nopendaftaran.' '.substr($namafile,0,150).'.'. strtolower(end(explode('.', $_FILES['nama_file']['name'][$n])))));
                            $ambildata = $this->db->get();
                                ...
                                if ($ambildata->num_rows() > 0) {
                                    $data=array(
                                        ...
                                        'tg_upload'=>date("Y-m-d H:i:s")
                                    );  

                                    $this->db->where('id_pendaftaran',$nopendaftaran);
                                    $this->db->where('nama_file',basename($nopendaftaran.' '.substr($namafile,0,150).'.'. strtolower(end(explode('.', $_FILES['nama_file']['name'][$n])))));
                                    $this->db->update('t_upload_arsip',$data);
                                    
        }else{
                    $data=array(
                        'id_pendaftaran'=>$nopendaftaran,
                        'nama_file'=>basename($nopendaftaran.' '.substr($namafile,0,150).'.'. strtolower(end(explode('.', $_FILES['nama_file']['name'][$n])))),
                        'keterangan'=>'syarat',
                        'user'=>$this->session->userdata('username'),
                        'tg_upload'=>date("Y-m-d H:i:s")
                    );  
$result = $this->db->insert('t_upload_arsip',$data);
   ...
   redirect('upload_arsip/edit'.'/E/'.$id_daftar.'/0/');
//redirect('arsip');
    }
     ...
     $dir = 'assets/upload/syarat/'.$id_perusahaan.' '.$nama_perusahaan;
      ...
      $jenis_izin = $this->input->post('jenis_izin');
        ...
        if (!empty($_FILES["nama_file"]["tmp_name"]))
        {
            ...
            $r7 = str_replace('>',' ',$r6);
            $r8 = str_replace('|',' ',$r7);
            $namafile = str_replace('"\"',' ',$r8);
           

             //   $gambar = $namafolder . basename('tes'.$_FILES['nama_file']['name'][$i]);
   $file = $namafolder . basename($nopendaftaran.' '.$namafile.'.'. strtolower(end(explode('.', $_FILES['nama_file']['name']))));
                ...
                if (move_uploaded_file($_FILES['nama_file']['tmp_name'], $file)) {
                            ...
                            $this->db->from('t_upload_arsip');
                            $this->db->where('id_pendaftaran',$nopendaftaran);
                            $this->db->where('nama_file',basename($nopendaftaran.' '.$namafile.'.'. strtolower(end(explode('.', $_FILES['nama_file']['name'])))));
                            $ambildata = $this->db->get();
                      ...
                      $data=array(
                        'id_pendaftaran'=>$nopendaftaran,
                        'nama_file'=>basename($nopendaftaran.' '.$namafile.'.'. strtolower(end(explode('.', $_FILES['nama_file']['name'])))),
                        'keterangan'=>'Izin',
                        'user'=>$this->session->userdata('username'),
                        'tg_upload'=>date("Y-m-d H:i:s")
                    );  
...
<?php
     ...
     function rekapupload(){
       ...
       $query= "select * from t_upload_arsip where tg_upload >= '$tgla' and tg_upload <='$tglb 24:00:00'";
       $jum_upload = $this->db->query($query)->num_rows();
       // $jum_arsip = $this->db->query($query." AND A.desc_arsip != ' '")->num_rows();
       $data['list'] = $query;
       $data['jum_upload'] = $jum_upload;
        //$data['jum_arsip'] = $jum_arsip;
        ...
        $js = "
                ...
                ";
        $this->template->set_metadata_javascript($js);        
        $this->session_info['page_name'] = "Rekap Jumlah Upload Arsip";
        $this->template->build('rekap_upload', $this->session_info);
     }

      public function report_arsip() {  // Masuk Pertama Kali saat klok menu Pengarsipan
        $file_upload = $this->file_upload;
        $query = $this->view_query();
        $mark = $this->input->post('mark');
        $list_sektor = $this->input->post('list_sektor');
       ...
       // //if ($lokasi_user === 'DPMPTSP Prov. tasikmalaya' || $lokasi_user === 'BPMPT Prov. tasikmalaya' || $lokasi_user === 'BPPT Prov. tasikmalaya')
        ...
        $js = "
                ...
                ";
        $this->template->set_metadata_javascript($js);        
        $this->session_info['page_name'] = "Report Pengarsipan Berkas Izin";
        $this->template->build('reportarsip_list', $this->session_info);
    }

    public function report_timteknis($edt = NULL, $id_daftar = NULL, $asal_menu = NULL) {
        $username = new user();
        $username->where('username', $this->session->userdata('username'))->get();
        $tgla = $username->gvar1;
        $tglb = $username->gvar2;
        ...
        if($edt == 'E')
            ...
            $this->session_info['page_name'] = "Informasi Data Detail";
        $this->template->build('report_timteknis', $this->session_info);
    }
