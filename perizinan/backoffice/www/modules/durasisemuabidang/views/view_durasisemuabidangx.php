...
inner join tmsk i on h.tmsk_id = i.id
                                ...
                                and a.d_terima_berkas between '$tgla' and '$tglb'";
									...
									$qx = "select  a.id, a.c_izin_selesai, e.n_perizinan,e.v_hari,
										
				   i.tgl_surat,i.tgl_surat_edit,a.d_terima_berkas from tmpermohonan a
                                inner join tmpermohonan_trperizinan b on a.id = b.tmpermohonan_id
                                inner join tmbap_tmpermohonan c on a.id = c.tmpermohonan_id
                                inner join tmbap d on d.id = c.tmbap_id
				inner join trperizinan e on e.id = b.trperizinan_id
...
where d.c_penetapan = 1 and a.trsektor_id='$idsektor' and a.d_terima_berkas between '$tgla' and '$tglb'";
                    ...
                    while($rowx = mysql_fetch_assoc($exex)){
					if($rowx >0){
					$tg1=$rowx['tgl_surat'];
					$tg3=$rowx['d_terima_berkas'];
					$tg2=$rowx['tgl_surat_edit'];
					//echo $tg1;
					}
					//$holidayperizin=0;
					$qholiday="select date holi from tmholiday where date between '$tg1' and '$tg2'";
$exeholiday = mysql_query($qholiday);
				...
				$holidayperizin = $rowholiday['holi'];
					...
					//echo $cc;
										...
										$q3 = "select  a.id, a.c_izin_selesai, e.n_perizinan,e.v_hari,
										AVG(DATEDIFF(i.tgl_surat,a.d_terima_berkas)) rata,
				   sum((DATEDIFF(i.tgl_surat,a.d_terima_berkas))<= e.v_hari) tepat from tmpermohonan a
                                inner join tmpermohonan_trperizinan b on a.id = b.tmpermohonan_id
                                inner join tmbap_tmpermohonan c on a.id = c.tmpermohonan_id
                                inner join tmbap d on d.id = c.tmbap_id
				inner join trperizinan e on e.id = b.trperizinan_id
