...
<body>
    ...
    <div class="post">
		...
		<div align="center"><h2><b><?php echo 'Daftar Jumlah Izin Selesai '; echo $a;?></h2></b></div>
		<?php
		}
		?>
        </div>
 ...
 <table  width="100%">
        <tr align="center">
          ...
          $img_cetak = array(
                           ...
                           'onclick' => 'parent.location=\''. site_url('durasisemuabidang/lwperbidangselesai').'/'.$bidangizin .'/'.$tgla.'/'.$tglb.'\''
                        );
           ...
           $img_cetak = array(
                            ...
                            'onclick' => 'parent.location=\''. site_url('durasisemuabidang/leperbidangselesai').'/'.$bidangizin .'/'.$tgla.'/'.$tglb.'\''
                        );
...
<!---------------------------------------------------------------------------------------------------.-->
			   ...
			   //$bidangizin=$this->input->post('id');
                   $bidangizin=$sektor_id;
                    ...
                    $query_data = "
					select  a.pendaftaran_id no,e.n_perizinan jenis,a.d_terima_berkas terima,a.d_selesai_proses selesai,i.tgl_surat,i.tgl_surat_edit,
					e.v_hari durasi,a.a_izin,a.keterangan,g.n_pemohon nama from tmpermohonan a
...
where d.c_penetapan = 1 and a.trsektor_id='$bidangizin' and a.d_terima_berkas between '$tgla' and '$tglb'" ;
                    ...
                    while ($data = mysql_fetch_assoc(@$results)){
					 ...
					 if($tg2 == '0000-00-00'){
					  ...
					  $qholiday="select count(date) holi from tmholiday where date between '$tg1' and '$tg2'";
$exeholiday = mysql_query($qholiday);
				...
				$holidayperizin = $rowholiday['holi'];
					 ...
					 //echo $realisaitarget;
					  ?>
					   <tr>
                        ...
                        <td align="center"><?php  echo $tglselesaiizin;//echo $data['selesai']; ?></td>
					...
					/*
					   $query_data2 = "
					select  a.pendaftaran_id no,e.n_perizinan jenis,a.d_terima_berkas terima,a.d_selesai_proses selesai,i.tgl_surat,i.tgl_surat_edit,
					e.v_hari durasi,a.a_izin,a.keterangan,
				   DATEDIFF(tgl_surat,a.d_terima_berkas) x,g.n_pemohon nama from tmpermohonan a
                                inner join tmpermohonan_trperizinan b on a.id = b.tmpermohonan_id
                                inner join tmbap_tmpermohonan c on a.id = c.tmpermohonan_id
                                inner join tmbap d on d.id = c.tmbap_id
				inner join trperizinan e on e.id = b.trperizinan_id
