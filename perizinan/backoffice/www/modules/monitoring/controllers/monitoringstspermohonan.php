 ...
 *
 */
class Monitoringstspermohonan extends WRC_AdminCont {
    ...
    public function __construct() {
        ...
        $this->monitoringstspermohonan = NULL;

        foreach ($list_auths as $list_auth) {
            if($list_auth->id_role === '20') {     // khusus Mencetak old 2
                $enabled = TRUE;
                $this->monitoringstspermohonan = new user_auth();
            }
        }
    ...
    public function index() {
        ...
        $this->session_info['page_name'] = "Monitoring Per Status Permohonan";
        $this->template->build('liststspermohonan', $this->session_info);
    }
    ...
    public function filterdata() {
        ...
        $this->load->vars($data);
        $this->session_info['page_name'] = "Monitoring Per Status Permohonan : " . $this->input->post('jenis_izin');
        $this->template->build('liststspermohonan', $this->session_info);
    }

    public function cetak_stspermohonan() {
      ...
      //  $list =  $this->perizinan->get();
        //$this->stspermohonan->where('id',$this->input->post('statusmohon'))->get();//
        ...
        $odf->setVars('tanggal', $this->lib_date->mysql_to_human(date('YYYY/mm/dd')));
         $odf->setVars('title','Monitoring Per Status Permohonan');

        $i = 1;


                        $relasi = new tmpermohonan_trstspermohonan();
                        $list_relasi = $relasi->where('trstspermohonan_id',$this->input->post('statusmohon'))->get();
                        if($relasi->trstspermohonan_id){
    ...
    public function selector($id_stspermohonan = NULL) {
        $iDisplayStart = NULL;
        $iSortCol_0 = NULL;
        $sEcho = 88;
        $iFilteredTotal = NULL;
        ...
        $this->stspermohonan->where('id', $id_stspermohonan);
        $this->stspermohonan->get($iDisplayStart, $iSortCol_0);

        $iTotal = $this->stspermohonan->tmpermohonan->count();
        $lists = $this->stspermohonan->tmpermohonan->get();
