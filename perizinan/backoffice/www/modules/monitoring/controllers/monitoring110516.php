	...
	public function perketegori_tertentu() {
        ...
        switch ($list_state) {
            case 1:    // untuk Nomor Pendaftaran
			    If(!$kt_cari)
			        $where = "WHERE A.pendaftaran_id LIKE '%xxx%'";
				else
				    $where = "WHERE A.pendaftaran_id LIKE '%".$kt_cari."%'";
                break;
            case 2:    // untuk Nama Pemohon
        ...
        if($this->All) {
		    $query = "SELECT distinct A.id, A.pendaftaran_id, A.id_lama, A.d_terima_berkas, A.a_izin, A.kd_status, A.d_selesai_proses,  A.c_izin_selesai,
                 A.d_perubahan, A.d_perpanjangan, A.d_daftarulang, B.trperizinan_id, A.keterangan, A.status_berkas, A.desc_arsip, A.nama_file,
