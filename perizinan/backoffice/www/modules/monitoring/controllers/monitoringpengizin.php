 ...
 *
 */
class Monitoringpengizin extends WRC_AdminCont {
    ...
    public function __construct() {
        ...
        foreach ($list_auths as $list_auth) {
            if($list_auth->id_role === '20' or $list_auth->id_role === '8') {   // khusus Mencetak 20 old 2
                $enabled = TRUE;
    ...
    public function filterdata() {
	 ...
	 $idizin = $this->input->post('ambilizin');
...
//        $odf->setImage('header', 'assets/css/' . $app_folder . '/images/dinas_1.jpg', '17.5', '3.5');
 ...
 $list2 = $this->perizinan->where('id',$this->input->post('ambilizin'))->get();


        $odf->setVars('tanggal', $this->lib_date->mysql_to_human(date('Y/m/d')));
        $odf->setVars('title','Monitoring Per Perizinan');
        $odf->setVars('jenis',$list2->n_perizinan);
        $odf->setVars('periode_awal',$this->lib_date->mysql_to_human($this->input->post('first_date')));
        $odf->setVars('periode_akhir',$this->lib_date->mysql_to_human($this->input->post('second_date')));
