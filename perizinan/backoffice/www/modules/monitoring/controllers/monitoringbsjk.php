...
class Monitoringbsjk extends WRC_AdminCont {
    ...
    public function index() {
        $monkelurahan = new trkelurahan();
        $data['listkelurahan'] = $monkelurahan->get();
        $data['listpemohon'] = $this->pemohon->limit(0)->get();
        $data['listpermohonan'] = $this->permohonan->limit(0)->get();
        $data['liststspermohonan'] = $this->stspermohonan->limit(0)->get();
        ...
        $this->template->build('listbsjk', $this->session_info);
    }
 ...
 public function filterdata() {
        ...
        $today = date('Y-m-d');
        $bsjk = $this->input->post('subeka');
        $data['listpemohon'] = $this->pemohon->get();
        $data['listpermohonan'] = $this->permohonan->get();
        $data['bsjk'] = $this->input->post('subeka');
        // Test One
        if($bsjk === '1'){
        $data['listpermohonan'] = $this->permohonan->where('c_izin_selesai', 1)->get();
        }
        elseif($bsjk === '0'){
        $data['listpermohonan'] = $this->permohonan->where('c_izin_selesai', 0)->get();
        }
        else{
        $data['listpermohonan'] = $this->permohonan->where("d_berlaku_izin < $today")->get();
        }
        ...
        $this->template->set_metadata_javascript($js );
        $this->session_info['page_name'] = "Monitoring Per Perizinan Belum/Sudah jadi dan Kadaluarsa";
        $this->template->build('view_bsjk', $this->session_info);
	}

     public function selector($id_bsjk = NULL) {
        $iDisplayStart = NULL;
        $iSortCol_0 = NULL;
        $sEcho = 88;
        $iFilteredTotal = NULL;
        ...
        $this->permohonan->where('c_izin_selesai', $id_bsjk);
        $this->permohonan->get($iDisplayStart, $iSortCol_0);

        $iTotal = $this->tmpermohonan->count();
        $lists = $this->tmpermohonan->get();
	...
	echo $sOutput;
     ...
     public function cetak_kadaluarsa($list_state = NULL){
	...
	$tglb = $this->input->post('second_date');
	 ...
	 $this->settings = new settings();
        ...
        // Test One
        if($list_state === '1'){
        $listpermohonan = $this->permohonan->where_related("trstspermohonan",'id <> 1')->where('c_izin_selesai', 1)->where("d_terima_berkas >= '$tgla' AND d_terima_berkas <= '$tglb'")->get();
        }
        elseif($list_state === '0'){
        $listpermohonan = $this->permohonan->where_related("trstspermohonan",'id <> 1')->where('c_izin_selesai', 0)->where("d_terima_berkas >= '$tgla' AND d_terima_berkas <= '$tglb'")->get();
        }
        else{
        $listpermohonan = $this->permohonan->where_related("trstspermohonan",'id <> 1')->where("d_berlaku_izin < $today")->get();
        }
        ...
        $nama_surat = "cetak_monitoring_bsjk_generic";
...
//        $odf->setImage('header', 'assets/css/' . $app_folder . '/images/dinas_1.jpg', '17.5', '3.5');
        ...
        $odf->setVars('title','Monitor Per Perizinan Belum/Sudah jadi dan Kadaluarsa');
        $odf->setVars('status',$list);
	...
	$sql = "select a.pendaftaran_id, b.n_pemohon, c.n_perizinan , c.id, a.d_terima_berkas, d.n_sts_permohonan, b.a_pemohon, l.n_kelurahan
	from tmpermohonan as a 
	inner join tmpemohon_tmpermohonan as  x on a.id = x.tmpermohonan_id
	inner join tmpemohon as b on b.id = x.tmpemohon_id 
	inner join tmpermohonan_trperizinan as y ON  a.id =  y.tmpermohonan_id
	inner join trperizinan as c on c.id = y.trperizinan_id
	...
	left join tmpemohon_trkelurahan as k on k.tmpemohon_id = a.id
	left join trkelurahan as l on k.trkelurahan_id = l.id
	where a.d_entry between  '".$tgla."' and '".$tglb."'
	and c.id = ".$id."";  
