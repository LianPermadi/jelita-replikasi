...
<div id="content">
    <div class="post">
        ...
        <fieldset id="half">
            ...
            $opsi_stat['0'] = "-------- Seluruhnya --------";
            if($list_status) {
                foreach ($list_status as $row) { 
					$stat = $row->n_sts_permohonan;
					if (substr($stat, 0, 1) != "-" && $stat != "Izin Disetujui" && $stat != "Izin Ditolak" && $stat != "Izin Dicabut") {
                        $opsi_stat[$row->id] = $stat;     
					}
                }
            }		
		   	    ...
		   	    <tr>
				    ...
				    <td><?php
                        if ($mark=="tanda") {
                            ...
                            echo form_dropdown('list_status', $opsi_stat,'0','class = "input-select-wrc" id="selecStatus"');
                        }
                        ?>
				    </td>
			    </tr>
				<tr>
                    <td> <?php echo form_label('Asal Permohonan', 'label_permohonan'); echo form_hidden('mark1', 'tanda1'); ?> </td>
                    <td> <?php
                        if ($mark1 == "tanda1") {
