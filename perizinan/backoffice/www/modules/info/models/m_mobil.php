...
class M_mobil extends Model
{
    ...
    public function get_peminjaman($tgla = NULL, $tglb = NULL)
    {
        // if($All){
            
        // var_dump($tgla, $tglb);die();
        // }
    ...
    public function get_data_peminjaman_mobil($id)
    {
        $sql = "SELECT * FROM peminjaman_mobil WHERE peminjam LIKE $id ORDER BY status DESC";
        $result = $this->db->query($sql)->result();
        return $result;
    }
...
public function get_tim_pengampu($id){
    ...
    public function edit_data($nama_mobil, $plat, $tahun, $date, $kategori, $status, $kondisi, $jenis, $kapasitas, $id, $nama_file, $on_off, $tim, $ketegori_jenis, $harga, $jumlah){
        $data = array(
            'nama_mobil' => $nama_mobil,
            'kategori' => $ketegori_jenis,
            'plat' => $plat,
