...
class InfoPerizinan extends WRC_AdminCont {
	...
	function cetak_syarat($no_izin = NULL, $var = NULL) {
		...
		foreach ($data as $baris) {
           ...
           //     $pdf->Cell($header[$i]['length'], 5, $cell, 0, '0', 'L', $fill);
           //     $i++;
           // }
           // $fill = !$fill;
            $pdf->Ln(1);
	...
	function cetak_syarat_OLD($no_izin = NULL) {
		$data_izin = $this->perizinan->get_by_id($no_izin);
		$nama_jenisperizinan = $data_izin->n_perizinan;
        //$list = $this->perizinan->where('id', $no_izin)->get();
        ...
        $pdf->Ln(13); $pdf->Cell(0,0.5,'DAFTAR PERSYARATAN',0,1,'C');
		$pdf->Ln(5); $pdf->Cell(0,0.5,$nama_jenisperizinan,0,1,'C');

        //perintah untuk query dari database

        $query = "select username,realname,oriname,lokasi from user order by username";
...
//		$pdf->Ln(5); $pdf->Cell(0,0.5,$r,0,1,'C');
