<script>
    function validasi() {
        var survey = document.forms[0].survey.value;
        var survey_h = document.forms[0].survey_h.value;
        var berkas = document.getElementById('berkas').value;
        var survey2 = document.getElementById('survey').value;
	    var survey_h2 = document.getElementById('survey_h').value;
        var petugas = document.forms[0].petugas.value;
        var pemeriksa = document.forms[0].listizin.value;
        if (survey=='0000-00-00') {
            document.getElementById('erorSurvey').innerHTML = 'Field ini wajib diisi';
            document.getElementById('erorSurvey').style.visibility = "visible"; 
            document.getElementById('erorSurvey').style.color = "#FF2F2F"; 
            return false;
        } else {
	    	if(survey2<=berkas && survey2!=='0000-00-00') {
                document.getElementById('erorSurvey').innerHTML = ' * Tanggal peninjauan tidak boleh lebih kecil dari tgl berkas';
                document.getElementById('erorSurvey').style.visibility = "visible"; 
                document.getElementById('erorSurvey').style.color = "#FF2F2F"; 
                return false;
            } else {
                if(survey_h2<=servey && survey_h2!=='0000-00-00') {
                    document.getElementById('erorSurvey_h').innerHTML = ' * Tanggal tidak boleh lebih kecil';
                    document.getElementById('erorSurvey_h').style.visibility = "visible"; 
                    document.getElementById('erorSurvey_h').style.color = "#FF2F2F"; 
                    return false;
                } else {
        			if (petugas=="") {
                        document.getElementById('erorPetugas').innerHTML = 'Field ini wajib diisi';
    	                document.getElementById('erorPetugas').style.visibility = "visible"; 
                        document.getElementById('erorPetugas').style.color = "#FF2F2F"; 
                        return false;
                    } else {
		        		if (pemeriksa=="") {
                            document.getElementById('erorPeriksa').innerHTML = 'Field ini wajib diisi';
                            document.getElementById('erorPeriksa').style.visibility = "visible"; 
                            document.getElementById('erorPeriksa').style.color = "#FF2F2F"; 
                            return false;
                        }
					}
				}
            }
        }
    }
</script>

<div id="content">
    <div class="post">
        <div class="title">
            <h2><?php echo $page_name; ?></h2>
        </div>
        <div class="entry">
            <?php
            $attr = array('name' => 'form', 'id' => 'form','onsubmit' => 'return validasi()'); 
            echo form_open('survey/' . $save_method,$attr);
            echo form_hidden('id_permohonan', $id);
            echo form_hidden('id_survey', $id_survey);
			echo form_hidden('visitasi', $visitasi);
			echo form_hidden('no_sp_awal',$no_sp_awal); 
            echo form_hidden('no_sp_akhir',$no_sp_akhir); 
			echo form_hidden('in_awal', '1');
            ?>
            <label class="label-wrc">ID Pendaftaran</label>
            <?php
            echo $pendaftaran_id;
            ?>
			<br style="clear: both;" />
            <label class="label-wrc">Nama</label>
            <?php
            echo $nama_pendaftar;
            ?>
			<br style="clear: both;" />
            <label class="label-wrc">Nama Perizinan</label>
            <?php
            echo $nama_perizinan;
            ?>
            <br style="clear: both;" />
            <label class="label-wrc">Tanggal Terima Berkas</label>
            <?php
            echo $this->lib_date->mysql_to_human($berkas);
            ?>
            <br style="clear: both;" />
			<label class="label-wrc"></label>

            <?php
            if($visitasi==0){  // untuk peninjauan lapangan
                $tinjauanOK = array(
                    'name' => 'button',
                    'content' => 'Ditinjau',
                    'disabled' => true,
                    'onclick' => 'parent.location=\''. site_url('survey') . '\''
                );
                echo form_button($tinjauanOK);
		        echo "  ";
                $tinjauanNO = array(
                    'name' => 'button',
                    'content' => 'Tidak Ditinjau',
                    'onclick' => 'parent.location=\''. site_url('survey/edit/'.$id.'/'.$method.'/1/1') . '\''
                );
                echo form_button($tinjauanNO);
            }else{             // untuk tidak perlu peninjauan lapangan
                $tinjauanOK = array(
                    'name' => 'button',
                    'content' => 'Ditinjau',
                    'onclick' => 'parent.location=\''. site_url('survey/edit/'.$id.'/'.$method.'/0/1') . '\''
                );
                echo form_button($tinjauanOK);
                echo "  ";
                $tinjauanNO = array(
                    'name' => 'button',
                    'disabled' => true,
                    'content' => 'Tidak Ditinjau',
                    'onclick' => 'parent.location=\''. site_url('survey') . '\''
                );
                echo form_button($tinjauanNO);
            }
            ?>
        </div>
        <?php
        if($visitasi==0){
        ?>
            <!-- tabel untuk melakukan tinjauan -->
            <div class="entry">
                <fieldset id="half">
                    <legend> Inisialisasi Peninjauan Lapangan </legend>
                    <table>  
                        <tr>
                            <td><label class="label-wrc">Tanggal Peninjauan</label> </td>
                            <td>
                                <?php
					            // Tanggal Awal Survay
                                $survey_input = array('name' => 'survey',
                                    'value' => $survey,
                                    'readOnly'=>TRUE,
                                    'class' => 'input-wrc required',
                                    'id' => 'survey',
                                    'class' => 'survey'
                                );
                                echo form_input($survey_input);
                                ?>
                                <span id="erorSurvey" style=" clear: both; visibility: hidden;"></span>
                        
                                <?php
                                echo "s/d ";
                                // Tanggal Akhir Survay
                                $survey_hingga = array('name' => 'survey_h',
                                    'value' => $survey_h,
   	                                'readOnly'=>TRUE,
                                    'class' => 'input-wrc required',
                                    'id ' => 'survey_h',
                                    'class' => 'survey'
                                );
                                echo form_input($survey_hingga);
                                ?>
                                <span id="erorSurvey_h" style=" clear: both; visibility: hidden;"></span>
                            </td>
                        </tr>
                        
						<tr>
                            <td><label class="label-wrc">No Surat</label></td>
                            <td>
                                <?php
                                //$no_surat_input = array('name' => 'no_surat', 'value' => $no_surat, 'class' => 'input-wrc', 'readOnly'=>TRUE);
						        //if($save_method === "update") {
                                //    $no_surat_input = array('name' => 'no_surat', 'value' => $no_surat, 'class' => 'input-wrc');
                                //    echo form_input($no_surat_input);
								//} else {
                                $no_surat_input = array(
                                    'name' => 'no_surat',
                                    'value' => $no_surat,
                                    'class' => 'input-wrc'
                                );
                                //$no_surat_input = $no_surat;
								echo $no_sp_awal.' / ';
                                echo form_input($no_surat_input);
								echo ' / '.$no_sp_akhir;
								//}
                                ?>
                            </td>
                        </tr>
                        <br style="clear: both;" />

                        <tr>
                            <td><label class="label-wrc">Penandatangan Surat</label></td>
                            <td>
                                <select name="petugas" id="petugas" class="input-wrc required" style="width:100%">
                                    <?php
                                    $selected = NULL;
                                    foreach ($petugas as $petugas_data) {
                                        if ($petugas_data->id === $petugas_id) {
                                            $selected = ' selected="selected" ';
                                        } else {
                                            $selected = NULL;
                                        }
                                        echo "<option value=\"" . $petugas_data->id . "\"" . $selected . ">" .
                                             $petugas_data->n_pegawai . " | " .
											 $petugas_data->n_jabatan . " | " .
                                             $petugas_data->nip . " | " .
                                             $petugas_data->pangkat_gol . "</option>\n" ;
                                        }
                                    ?>
                                </select>
                                <span id="erorPetugas" style=" clear: both; visibility: hidden;"></span>
                            </td>
                        </tr>
                        <br style="clear: both;" />
                        
						<tr>
                            <td valign='top'><label class="label-wrc">Tim Peninjauan Lapangan</label></td>
                            <td>        
                                <select id="listizin1" name="listizin1[]" multiple="multiple"  style="width:100%">
                                    <?php
							        $pilih = FALSE;
                                    foreach ($list as $data) {
                                        foreach ($idp as $data_p) {
											$selected= 'testing';
                                            if ($data_p->id == $data->id) {
                                                $selected= 'selected'; break;
                                            }
                                        }
                                        if ($selected=="selected") {
                                            $pilih = TRUE;
                                            $data->trunitkerja->get();
                                            echo "<option value='".$data->id."'" . $selected . " >".$data->n_pegawai." | ".
                                                  $data->nip." | ".$data->pangkat_gol." | ".$data->n_jabatan." </option>";
                                        }   
                                    }
									if(!$pilih){
                                        echo "<option>".'Belum Ada TIM Pemerikas Peninjauan Lapangan '." </option>";
                                    }
                                    ?>
                                </select>
                                <span id="erorPeriksa" style=" clear: both; visibility: hidden;"></span>
                            </td>
                        </tr>
                        <tr>
                            <td><label class="label-wrc"></label></td>
                            <td>        
                                <select id="listizin" name="listizin[]" multiple="multiple" >
                                    <?php
                                    if($save_method === "update") {
                                        foreach ($list as $data) {
                                            foreach ($idp as $data_p) {
                                                $selected= 'testing';
                                                if ($data_p->id == $data->id) {
                                                    $selected= 'selected'; break;
                                                }
                                            }
                                            if ($selected=="selected") {
                                                $data->trunitkerja->get();
                                                echo "<option style='width:100%' value='".$data->id."'" . $selected . ">".$data->n_pegawai." | ".
                                                     $data->nip." | ".$data->pangkat_gol." | ".$data->n_jabatan." </option>";
                                            } else {
                                                $data->trunitkerja->get();
                                                echo "<option style='width:100%' value='".$data->id."'>".$data->n_pegawai." | ".
													 $data->nip." | ".$data->pangkat_gol." | ".$data->n_jabatan. "</option>";
                                            }   
                                        }
                                    } else {
                                        foreach ($list as $data) {
                                            echo "<option style='width:100%' value='".$data->id."'>".$data->n_pegawai." | ".$data->nip." |".
												 $data->pangkat_gol." | ".$data->n_jabatan." </option>";
                                        }
                                    }
                                    ?>
                                </select>
                                <span id="erorPeriksa" style=" clear: both; visibility: hidden;"></span>
                            </td>
                        </tr>
                    </table>
                </fieldset>
            </div>
            <!-- EOF() tabel untuk melakukan tinjauan -->
            <?php
        } else {
            ?>
            <!-- tabel untuk melakukan visitasi atau tidak perlu peninjauan -->
            <div class="entry">
                <fieldset class="entry">
                    <legend> Inisialisasi Tidak Ditinjau </legend>
                    <table>  
                        <tr>
						    <b><?php echo 'Permohonan ini tidak perlu dilakukan peninjauan lapangan dengan alasan' ?></b>
                        </tr>
                        <tr>
                            <?php
   // 					    if($lht == '0') {
                            $ctt_alasan_input = array(
                                'name' => 'ctt_alasan',
                                'value' => $ctt_alasan,
                                'style' => 'width:100%',
                                'class' => 'input-area-wrc required'
                            );
                            echo form_textarea($ctt_alasan_input);
	//			        } else {
	//						echo $ctt_alasan;
    //                    }
                        ?>
                        </tr>
                    </table>
                </fieldset>
            </div>
            <!-- EOF() tabel untuk melakukan visitasi -->
            <?php
        }
            ?>
        <br style="clear: both;" />
        <label>&nbsp;</label>
        <div class="spacer"></div>
        <?php
        $add_role = array(
            'name' => 'submit',
            'class' => 'submit-wrc',
            'content' => 'Simpan',
            'type' => 'submit',
            'value' => 'Simpan'
        );
        echo form_submit($add_role);
        echo "<span></span>";
        $cancel_role = array(
             'name' => 'button',
             'class' => 'button-wrc',
             'content' => 'Batal',
             'onclick' => 'parent.location=\''. site_url('survey') . '\''
        );
        echo form_button($cancel_role);
        echo form_close();
        ?>
    </div>
    <br style="clear: both;" />
</div>