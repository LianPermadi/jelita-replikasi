...
class Autocomplete extends WRC_AdminCont
{
	...
	/*public function search()
	{
		...
		// cari di database
			$dbmysql2 = $this->load->database('db_bb',TRUE);
...
$data = $dbmysql2->from('bb_traak')->like('NAMA_TRAYE',$keyword)->get();	
	...
	*/
	...
	{
		// tangkap variabel keyword dari URL
		$keyword = str_replace('_', '.', $this->uri->segment(3));
	...
	public function search_terminal()
	{
		// tangkap variabel keyword dari URL
		$keyword = str_replace('_', '.', $this->uri->segment(3));
		$dbmysql2 = $this->load->database('db_bb',TRUE);
	/*	$dbmysql2->select('*');
		...
		$data = $dbmysql2->from('bb_kota')->like('NAMA_KOTA',$keyword)->get();
	...
	public function search_pengusaha()
	{
		// tangkap variabel keyword dari URL
		$keyword = str_replace('_', '.', $this->uri->segment(3));
	...
	public function search_tgsk()
	{
		// tangkap variabel keyword dari URL
		$isi = '';
		$keyword = str_replace('_', '.', $this->uri->segment(3));
		if($this->uri->total_segments() > 3) {
			for ($i=3; $i < $this->uri->total_segments()+1; $i++) { 
				$isi[] = str_replace("/", "", str_replace('_', '.', $this->uri->segment($i)));
			}
		}
		
		$NO_SK = ($isi ? implode("/", $isi) : $keyword);

		// cari di database
		//$data = $this->db->from('bb_pau')->like('NO_IP',$keyword)->get();	
		$dbmysql2 = $this->load->database('db_bb',TRUE);
		$dbmysql2->select('NO_SK, TG_SK, BERLAKU');
		$dbmysql2->from('bb_pit');
		$dbmysql2->like('NO_SK', $NO_SK);
		$dbmysql2->order_by('NO_SK');
