...
class Permintaanbarang extends WRC_AdminCont
{
    ...
    public function simpan()
    {
        ...
        move_uploaded_file($_FILES['foto1']['tmp_name'], '/var/www/html/jelita/backoffice/www/modules/permintaanbarang/views/barang/' . date('YmdGis') . $_FILES['foto1']['name']);
    ...
    public function save()
    {
        ...
        $oldfoto = $this->input->post('oldfoto');
            ...
            $path = '/var/www/html/jelita/backoffice/www/modules/permintaanbarang/views/barang/'.$old;
            chmod($path, 0777);
            unlink($path);
            $upload = move_uploaded_file($_FILES['foto1']['tmp_name'], '/var/www/html/jelita/backoffice/www/modules/permintaanbarang/views/barang/' . date('YmdGis') . $_FILES['foto1']['name']);
...
public function update_data_test($tahun)
{
    ...
    public function accept_barang(){ // checkout permohonan barang
      ...
      if($null == '0'){
        ...
        $this->lib_date->postWaSms('08121314834', $n_pesan, 'Setting');
        //if($this->All){var_dump($n_pesan); die();}
        // EOF() Kirim SMS
        $this->session->set_flashdata('sukses', "Berhasil di ajukan");
