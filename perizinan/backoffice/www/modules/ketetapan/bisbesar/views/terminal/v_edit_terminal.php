...
<?php foreach($bb_terminal as $u){ ?>

<form method = "POST" action = "<?php echo base_url();?>bisbesar/c_terminal/update_aksi" enctype="multipart/form-data">
             <input type="hidden" name ="TERMINAL_I" class="form-control" value=<?php echo $u->TERMINAL_I ?> required>

            <br style="clear: both" />
            <label>TERMINAL / LINTASAN</label>
            <input type="text" name ="NAMA_TERMI" class="form-control" value="<?php echo $u->NAMA_TERMI ?>" required>

            <br style="clear: both" />
            <label>KAPASITAS</label>
            <input type="number" name ="KAPASITAS" class="form-control" min ="0" value = "<?php echo $u->KAPASITAS ?>" required>
           
          
            <br style="clear: both" />
            <label>TYPE</label>
            <select name ="JENIS" class="form-control select" required>
            <option selected value = "<?php echo $u->JENIS ?>"><?php echo $u->JENIS ?></option>
