 ...
 *
 */
class back_lap extends WRC_AdminCont {
...
//        $data = $this->_funcwilayah();
        ...
        $data['listpermohonan'] = $this->permohonan->where('c_izin_selesai',1)->limit(0)->get();
        $this->load->vars($data);

        $this->session_info['page_name'] = "Rekapitulasi Berkas Kembali";
        $this->template->build('list_back_lap', $this->session_info);
    }
     ...
     public function filterData() {
        ...
        $this->session_info['page_name'] = "Rekapitulasi Berkas Kembali";
        $this->template->build('view_back_lap', $this->session_info);
    }
...
//        $p_kabupaten = $permohonan->tmpemohon->trkelurahan->trkecamatan->trkabupaten->get();
        ...
        $nama_surat = "cetak_berkas_kembali";
...
//            $alamat =  $permohonan->tmpemohon->a_pemohon;
        ...
        $odf->setVars('title','Rekapitulasi Berkas Kembali Periode '.$this->lib_date->mysql_to_human($tgla).' s/d '.$this->lib_date->mysql_to_human($tglb));
        ...
        else
        {
            ...
            $listeArticles3 = array(
                    array(	'property' =>  $i,
                            'content1' =>  $data['pendaftaran_id'],
                            'content2' =>  $this->lib_date->mysql_to_human($data['d_survey']),
                            'content3' =>  $data['n_pemohon']." ".$data['a_pemohon'],
                            'content4' =>  $n_perusahaan." ".$data['a_izin'],
                            'content5' =>  $data['c_pesan'],
                            'content6' =>  $this->lib_date->mysql_to_human($data['d_ambil_izin']),
                            'content7' =>  $data['n_perizinan'],
                    ),
            ...
            foreach($listeArticles3 AS $element) {
                    ...
                    $article3->texteArticle7($element['content7']);
                    $article3->merge();
            }
