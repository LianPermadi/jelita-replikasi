      ...
      <table cellpadding="0" cellspacing="0" border="0" class="display" id="pendataan">
        ...
        <tbody>
          ...
          foreach ($list_pengembangan as $row) {
            if($row->status == '2'){ 
            $iduser      = $this->session->userdata('id_auth');
            $userid = $this->m_pengembangan->get_user_id($iduser);
                if($userid == $row->nama_pengaju || $this->All == TRUE){
              ?>
            <tr>
              <td><?php echo $i; ?></td>
              ...
              <td><?php 
                if ($row->status == '0') {
                  ...
                  echo '<span style="color:yellow">In Proses</span>';
                }elseif($row->status == '2'){
            ...
            <?php
            // var_dump($userid);die();
            if($userid == $row->nama_pengaju){
            ?>
                <a href="pengembangan/approve_text/<?php echo $row->id; ?>" onclick="return confirm('Apa Anda Yakin Hapus permintaan dari <?php echo $row->nama_pengaju; ?>?')" title="Fixing Aplikasi">
                    <!-- <button class="button-wrc">Fixing</button> -->
                    <img src="https://cdn-icons-png.flaticon.com/512/6672/6672028.png" alt="" width="20px">
                </a>
                <a href="pengembangan/done_fix/<?php echo $row->id; ?>">
                <!-- <button class="button-wrc">Selesai</button> -->
                    <img src="https://cdn.icon-icons.com/icons2/3284/PNG/512/complete_done_checkmark_completed_icon_208202.png" alt="" width="20px" title="Fixing Aplikasi" title="Aplikasi Bisa di gunakan">
                </a>
            <?php
            }elseif($this->All){
            ?>
