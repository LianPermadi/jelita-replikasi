...
class Datatables extends WRC_AdminCont {
    ...
    public function getDataTables() {
        $obj = new tmpemohon();
        $obj->start_cache();
        $columns = array('no_referensi', 'source', 'n_pemohon', 'a_pemohon');
    ...
    private function test($obj) {
        ...
        foreach ($obj as $list) {
            ...
            $action .= anchor(site_url('pemohon/edit') . '/' . $list->id, img($img_edit)) . "&nbsp;";
            $relasi = new tmpemohon_tmpermohonan();
            $relasi->where('tmpemohon_id', $list->id)->get();

            if (!$relasi->id) {
                $action .= anchor(site_url('pemohon/delete') . '/' . $list->id, img($img_delete)) . "&nbsp;";
            }
