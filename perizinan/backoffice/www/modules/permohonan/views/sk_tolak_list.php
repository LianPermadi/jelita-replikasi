            ...
            <table cellpadding="0" cellspacing="0" border="0" class="display" id="sk">
                ...
                <tbody>
                    ...
                    while ($rows = mysql_fetch_assoc(@$results)){
                        $showed = FALSE;
                        $c_cetak = NULL;
                        //$query_data = "SELECT b.c_skrd, b.status_bap, b.bap_id, b.c_pesan, d.no_surat, d.c_cetak
                        ...
                        //               AND c.tmsk_id = d.id";
						$query_data = "SELECT d.no_surat,b.status_bap, d.c_cetak
                                       FROM tmbap_tmpermohonan a, tmbap b, tmpermohonan_tmsk c, tmsk d
                                       WHERE a.tmpermohonan_id = '".$rows['id']."'
                                       AND a.tmbap_id = b.id
                                       AND c.tmpermohonan_id = a.tmpermohonan_id
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    if($rows['idjenis'] == '1') 
										...
										if($rows['idjenis'] == '2') 
										    $tgl_permohonan = $rows['d_perubahan'];
                                        else 
											if($rows['idjenis'] == '3') 
											    $tgl_permohonan = $rows['d_perpanjangan'];
                                            else 
												if($rows['idjenis'] == '4') 
												    $tgl_permohonan = $rows['d_daftarulang'];
                                    if($tgl_permohonan){
                                ...
                                <td>
                                    <?php
                                    $sz = $this->db->query("select c_cetak from tmpermohonan where id = $rows[id]")->row_array();
                                    if($sz['c_cetak'] != 0){
                                        echo "<b>Dicetak ".$sz['c_cetak']." kali</b>";
                                    }else{
                                        echo "Belum di-cetak";
                                    }
                                    ?>
                                ...
                                <td>
                                    <center>
                                        ...
                                        echo anchor(site_url('permohonan/skditolak/cetak_penolakan') .'/'. $rows['id'], img($img_cetak))."&nbsp;";
                                        ?>
                                    </center>
                                </td>
                            </tr>
