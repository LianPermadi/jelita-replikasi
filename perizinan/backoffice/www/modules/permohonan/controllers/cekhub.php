...
class cekhub extends MY_Controller {
  ...
  function download_sk($nodaftar = NULL) {  // Download file ber ttE BSrE
    ...
    if(file_exists($lok_esign.$file.'.pdf')){        // cek PDF yang sudah SE
	    ...
	    if(file_exists($non_esign.$file.'.pdf')){        // cek PDF yang belum SE
	      $data = file_get_contents($non_esign.$file.'.pdf');
	      force_download($file.'.pdf', $data);
	    }
	  }
  }
  ...
  function download_kp($nodaftar = NULL) {  // Download file ber ttE BSrE
    ...
    $lok_esign = 'backoffice/assets/esignfile/';
    $non_esign = 'backoffice/assets/skpdf/';
    if(file_exists($lok_esign.$file.'.pdf')){        // cek PDF yang sudah sudah SE
	    $data = file_get_contents($lok_esign.$file.'.pdf');
	    force_download($file.'.pdf', $data);
	  }else{
	    if(file_exists($non_esign.$file.'.pdf')){        // cek PDF yang sudah belum SE
	      $data = file_get_contents($non_esign.$file.'.pdf');
	      force_download($file.'.pdf', $data);
	    }
	  }
