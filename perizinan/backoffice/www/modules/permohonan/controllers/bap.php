 ...
 * @author Eva, Dichi Al Faridi, PBS
 */
class Bap extends WRC_AdminCont {
  ...
  public function __construct() {
    ...
    $this->pegawai = $this->username->tmpegawai->get();
    $this->eselon = $this->pegawai->eselon;
  ...
  public function index() {  //  Untuk Penjadualan Tinjauan Lapangan pertama
    ...
    $this->session_info['page_name'] = "Data Pembuatan Rekomendasi Teknis";
  ...
  //
  //      $this->template->set_metadata_javascript($js);
  //      $this->session_info['page_name'] = "Data Pembuatan Pertimbangan Teknis";
  //      $this->template->build('bap_list', $this->session_info);
  //  }

  public function viewBAP($id=NULL, $idizin=NULL) {  
    ...
    if($bap->c_pesan == 'Dengan Tinjauan Lapangan'){
      $c_pesan = $bap->c_pesan.', Permohonan dinyatakan : ';
    }else{
      $c_pesan = $bap->c_pesan;
    }  
    ...
    $this->load->vars($data);
    $this->session_info['page_name'] = "Rekomendasi Teknis";
    ...
    public function viewBAP2($id=NULL, $idizin=NULL) {
        ...
        $data['n_manual'] = $this->getTinjauan($permohonan->id, '45');
        $data['m_hitung'] = $permohonan->$perizinan->$retribusi->m_perhitungan;
		$data['m_denda'] = $permohonan->$perizinan->$retribusi->v_denda;
		$data['m_ret_auto'] = $permohonan->$perizinan->$retribusi->v_retribusi; 
        $data['list'] = $permohonan->$perizinan->trproperty->order_by('c_parent_order asc, c_order asc')->get();
        $data['list_daftar'] = $permohonan->tmproperty_jenisperizinan->get();
    ...
    public function save() {
        ...
        if ($bap->id) {
            ...
            $bap->nilai_retribusi = $this->input->post('nilai_ret');
            $bap->nilai_bap_awal = $this->input->post('nilai_ret');
			...
			$this->bapp->tgl_bap = $this->lib_date->get_date_now();
        ...
        $status_data = "3";   //Entri data [Lihat Tabel trstspermohonan()]     -> kominfo old 3 
        $status_rekom = "4";  //Rekomendasi [Lihat Tabel trstspermohonan()]    -> kominfo old 5
        $status_skr = "4";    //BAP Lapangan [Lihat Tabel trstspermohonan()]   -> kominfo old 6 
        $id_status = "8";     //Penetapan Izin [Lihat Tabel trstspermohonan()] -> kominfo old 7
...
//            $tracking_izin->pendaftaran_id = $permohonan->pendaftaran_id;
		...
		if($permohonan->kd_status < 5){ // ubah kd_status menjadi 5 (BAP sudah di entry) untuk proses selanjutnya (Penetapan SK)
		    $permohonan->kd_status = 5;
            $permohonan->approve = 7;   // transisi 'non approve PD' jika BA di simpan beri nilai 6 utk kd kunci ESL2 PD 
            $permohonan->save();
        }
		...
		if(!$tracking_izin2->pendaftaran_id){
	    	...
	    	$tracking_izin2->his_ubah = $his_ubah.'BAP^'.$r_name.'^'.$this->lib_date->get_date_now().';';
            $tracking_izin2->save();
        }
        
		// Untuk Pesan Tempat Penyusunan / Pencetakan Naskah Perizinan
		$tracking_izin2 = new tmtrackingperizinan();
        $tracking_izin2->where('pendaftaran_id', $no_pendaftaran)
		...
		if(!$tracking_izin2->pendaftaran_id){
            ...
            $tracking_izin2->d_entry = $this->lib_date->get_date_now();
            $sts_izin2 = new trstspermohonan();
    	    $sts_izin2->get_by_id('8'); //[Lihat Tabel trstspermohonan()]   => edit PBS
            $sts_izin2->save($permohonan);
            $tracking_izin2->save($permohonan);
            $tracking_izin2->save($sts_izin2);
		}
...
//        $odf->setVars('title', strtoupper('Berita Acara Pemeriksaan'));
		...
		if($tracking_izin2->pendaftaran_id){
	        ...
	        $tracking_izin2->his_cetak = $his_cetak.'Cetak BAP Peninjauan^'.$r_name.'^'.$this->lib_date->get_date_now().';';
            $tracking_izin2->save();
        }
...
// ................................ Isi ...........................
                    if ($list_daftar->id) {
                        foreach ($list_daftar as $data_daftar) {
                            ...
                            if ($entry_property->tmproperty_jenisperizinan_id) {
                                ...
                                $property->child->isi($pil->kategori . " " . $data_koefisient . " " . $izin_property->satuan);
                            }
                        }
                    }
        ...
        foreach ($listform as $data) {
            ...
            if ($entry_daftar->v_tinjauan) {
                if ($entry_daftar->v_tinjauan == " "){
					...
					if (strval($entry_daftar->v_tinjauan) === "0"){
                        $hasil = $data_koefisien2->kategori;
			        }else{
            ...
            $listeArticles3 = array(array('property3' => $prop,'content33' => $hasil,),);
    ...
    public function cetakBAP2_archive($id=NULL, $idjenis=NULL) {
        ...
        $listeArticles = array(array('property' => 'No BAP','content' => $bap->bap_id,),
                               array('property' => 'Nomor pendaftaran','content' => $permohonan->pendaftaran_id,),
    ...
    $judul = "Upload Pertek (.pdf)";
    $data["judulapp"]=$judul;
    $data["id"]=$id; // Nomor Induk Pegawai
    $viewfile="v_pertek_form";
    $this->load->view($viewfile,$data);
  }

  function uploadpertek(){   // Upload File TTD
    $id = $this->input->post('id');  // Nomor pendaftaran
    $permohonan = $this->sk->get_by_id($id);
    $resi = $permohonan->pendaftaran_id;

    $file = $_FILES["fileToUpload"]["name"];
    $file_name = basename($_FILES["fileToUpload"]["name"]);
    $ext = pathinfo($file, PATHINFO_EXTENSION);
    
    $target_dir = "assets/pertek/";
    ...
    if(isset($_POST["submit"])) {
      ...
      $fileBaru = $target_dir.'PT_'.$resi.'.'.$ext;
      ...
      if($upload) {
        rename($target_file, $fileBaru);
        $this->session->set_flashdata('sukses', "Berhasil Upload Pertek.");
        redirect('permohonan/bap');
      }else{
        $this->session->set_flashdata('gagal', var_dump($upload));
        redirect('permohonan/bap');
      }

    } else {
      $this->session->set_flashdata('gagal', "Gagal Upload Pertek 2.");
      redirect('permohonan/bap');
    }
  }

  public function downloadpertek($pendaftaran_id){
    $this->load->helper('download');
    $id = $pendaftaran_id;

    $data = file_get_contents('assets/pertek/PT_'.$id.'.pdf');
    force_download('PT_'.$id.'.pdf', $data);
  }

    public function main_sql($u_ser,$tgla,$tglb,$lokasi_user,$no_daftar=NULL) {
        ...
        if($this->All){
            $query = "SELECT A.id, A.pendaftaran_id, A.c_tinjauan, A.d_terima_berkas, A.d_survey, A.kd_status, A.kd_gerai, A.d_selesai_proses, A.a_izin,
                      A.d_perubahan, A.d_perpanjangan, A.d_daftarulang, A.status_berkas, A.approve, A.no_per_pertek,
                      C.id idizin, C.n_perizinan, C.template_gub, E.n_pemohon,
                      G.id idjenis, G.n_permohonan, K.n_sts_permohonan, C.pt_ttd, C.pt_sertifikat
                      FROM tmpermohonan as A
                      INNER JOIN tmpermohonan_trperizinan as B ON B.tmpermohonan_id = A.id
                      INNER JOIN trperizinan as C ON B.trperizinan_id = C.id
                      INNER JOIN tmpemohon_tmpermohonan as D ON D.tmpermohonan_id = A.id
    		...
    		//if ($lokasi_user === 'DPMPTSP Prov. tasikmalaya' || $lokasi_user === 'BPMPT Prov. tasikmalaya' || $lokasi_user === 'BPPT Prov. tasikmalaya' || $lokasi_user === 'OPD Teknis') {
                $query = "SELECT A.id, A.pendaftaran_id, A.c_tinjauan, A.d_terima_berkas, A.d_survey, A.kd_status, A.kd_gerai, A.d_selesai_proses, A.a_izin,
                          A.d_perubahan, A.d_perpanjangan, A.d_daftarulang, A.status_berkas, A.approve, A.no_per_pertek,
                          C.id idizin, C.n_perizinan, C.template_gub, E.n_pemohon,
                          G.id idjenis, G.n_permohonan, L.n_sts_permohonan, C.pt_ttd, C.pt_sertifikat
                          FROM tmpermohonan as A
                          INNER JOIN tmpermohonan_trperizinan as B ON B.tmpermohonan_id = A.id
                          INNER JOIN trperizinan as C ON B.trperizinan_id = C.id
                          INNER JOIN tmpemohon_tmpermohonan as D ON D.tmpermohonan_id = A.id
