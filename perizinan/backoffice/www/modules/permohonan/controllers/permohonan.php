...
class Permohonan extends MY_Controller {
  ...
  function step2($id) {
    ...
    $query	= $otherdb->query("select * from trperizinan where c_online = '0' and via = '0' and id <> '237' and id in(select trperizinan_id from trperizinan_trsektor where trsektor_id='".$id."') order by n_perizinan")->result();
  ...
  function step3($id,$uuid=null) {
    ...
    $integrasi = $otherdb->query($sql)->first_row();
    ...
    $datintegrasi = "";
    if ($integrasi->integrasi == 1) {
      $val = "1";
      $sqlintegrasi = "select * from integrasi where app=? AND (id_tmpemohon_portal is null AND id_tmpermohonan is null)";
      $datintegrasi = $otherdb->query($sqlintegrasi, array($val))->result();
    }

    //Provinsi, Kabupaten, Kecamatan, Kelurahan
    $provinsi   = $otherdb->query('select * from trpropinsi order by n_propinsi')->result();
    $kabupaten  = $otherdb->query("select * from pabar_backoffice.trkabupaten where kd_prov='34' order by n_kabupaten")->result();
    
    $data['title']        = "Permohonan Perizinan";
    $data['load']         = "permohonan/step3";
    $data['property']     = $query;
    $data['datintegrasi'] = $datintegrasi;
    $data['id']           = $id;
    $data['id_sektor']    = $sektor->trsektor_id;
    $data['sektor']       = $sektor2->n_sektor;
    $data['integrasi']    = $integrasi->integrasi;
    $data['jml']          = $properti;
    $data['judul']        = $judul;
    $data['uuid']         = $uuid;
    $data['provinsi']     = $provinsi;
    $data['kabupaten']    = $kabupaten;
    if($integrasi->integrasi != 0) {
      $this->load->view('template_user_step3',$data);
    } else {
      $this->load->view('template_user',$data);
    }
  ...
  function step4($id,$jml_property,$uuid,$valintegrasi=NULL) {
    $username = $this->session->userdata("username");
    $session  = $this->session->userdata("userlogin");
    $otherdb  = $this->load->database('otherdb', TRUE); // the TRUE paramater tells CI that you'd like to return the database object.
    ...
    $sql   = "select trsyarat_perizinan.* from trsyarat_perizinan 
              ...
              where trperizinan_id='".$id."' and status='1') ORDER BY (trsyarat_perizinan.urutan * -1) DESC, trsyarat_perizinan.status ASC, urutan = 0, urutan";
    $query = $otherdb->query($sql)->result();
    ...
    $data['title']        = "Permohonan Perizinan Step 4";
  ...
  function savestep3($cid){  // proses simpan step 1, 2 da 3
    ...
    if (!empty($judul) && $judul->indeks == "AKDP") {
        if (!empty($_POST['nokend']) && !empty($_POST['nouji']) && !empty($_POST['lintasan'])) {
          $vnokend    = htmlspecialchars($_POST['nokend'],ENT_QUOTES);
          $vnouji     = htmlspecialchars($_POST['nouji'],ENT_QUOTES);
          $vlintasan  = htmlspecialchars($_POST['lintasan'],ENT_QUOTES);

          $vnouji = str_replace(" ", "", $vnouji);

          if ($vnouji == "-") {
            $vnouji = str_replace("-", "000", $vnouji);
          }

          $lokasi_izin = trim(htmlspecialchars($_POST['var_teknis'.$vnokend],ENT_QUOTES))." / ".htmlspecialchars($_POST['var_teknis'.$vnouji],ENT_QUOTES)." / ".htmlspecialchars($_POST['var_teknis'.$vlintasan],ENT_QUOTES);
        } else {
          $lokasi_izin  = str_replace(',', '.', htmlspecialchars($_POST['lokasi_izin'],ENT_QUOTES));
        }
    } else {
    ...
    $longitude    = (!empty($_POST['maps_longitude']) ? htmlspecialchars($_POST['maps_longitude'],ENT_QUOTES) : "");
    $latitude     = (!empty($_POST['maps_latitude']) ? htmlspecialchars($_POST['maps_latitude'],ENT_QUOTES) : "");
    


    $nib          = htmlspecialchars($_POST['nib'],ENT_QUOTES);
    if (isset($_POST['noss'])) {
      $noss         = htmlspecialchars($_POST['noss'],ENT_QUOTES);
    } else {
      $noss       = "";
    }
    ...
    //cek integrasi
    $integrasi         = htmlspecialchars($_POST['integrasi'],ENT_QUOTES);
    $valintegrasi      = 0;

    if ($integrasi == 1) {
      $valintegrasi = htmlspecialchars($_POST['iddkp'],ENT_QUOTES);
    }
    //end cek
    ...
    $data = array('id_pemohon'      => $user->id,
                  ...
                  'nib'             => $nib,
                  'no_ss'           => !empty($noss) ? $noss : "",
                  'lon'             => $longitude,
                  'lat'             => $latitude,
    	            'status'	        => 0,        // status pemohon
                  'trkelurahan_id'  => $kelurahan,
                 );
    $update = FALSE;
    if($uuid != ''){
    ...
    redirect('main/permohonan/step4/'.$cid.'/'.$jml_property.'/'.$uuid.'/'.$valintegrasi, 'refresh');
  }
  ...
  function addpermohonanbaru(){  // proses simpan data permohonan simpan ke Backoffice
    ...
    if (!empty($user)) {
	    $entry_daftar = $this->db->order_by("id","DESC");
	    $entry_daftar = $this->db->get_where("tmpermohonan_portal",array("id_pemohon"=>$user->id))->first_row();

	    if (date('Y-m-d', strtotime($entry_daftar->d_entry)) != date('Y-m-d')) {
	      redirect('main/permohonan/step1', 'refresh');
	      die;
	    }
	    ...
	    $nib          = $eksis->nib;
      $noss         = $eksis->no_ss;
	    $lon          = (!empty($eksis->lon) ? $eksis->lon : "");
	    $lat          = (!empty($eksis->lat) ? $eksis->lat : "");
	    $lokasi_izin  = $eksis->lokasi_izin;
	    $kelurahan    = $eksis->trkelurahan_id;

	    $valintegrasi = htmlspecialchars($_POST['integrasi'],ENT_QUOTES);
	    ...
	    if ($valintegrasi != 0) {
	      ...
	      $datainp = array('id_tmpemohon_portal' => $insert_id,);
	      $otherdb->where('id', $valintegrasi);
	      $otherdb->update('integrasi', $datainp);
	    }
	    ...
	    foreach($query as $syarat){
	      ...
	      if(!empty($_FILES["file_".$syarat->id]["name"])){
	        ...
	        //if(strtolower(pathinfo($_FILES["file_".$syarat->id]["name"], PATHINFO_EXTENSION)) != "pdf" || strtolower(pathinfo($_FILES["file_".$syarat->id]["name"], PATHINFO_EXTENSION)) != "jpg" || strtolower(pathinfo($_FILES["file_".$syarat->id]["name"], PATHINFO_EXTENSION)) != "jpeg"){
	          //if(end($temp)!="pdf"){  
	    ...
	    foreach($query as $syarat){
	      ...
	      if (strtolower(pathinfo($_FILES["file_".$syarat->id]["name"], PATHINFO_EXTENSION)) == "jpg") {
	        $namafile = $syarat->id.".jpg";
	      } elseif (strtolower(pathinfo($_FILES["file_".$syarat->id]["name"], PATHINFO_EXTENSION)) == "jpeg") {
	        $namafile = $syarat->id.".jpeg";
	      } elseif (strtolower(pathinfo($_FILES["file_".$syarat->id]["name"], PATHINFO_EXTENSION)) == "rar") {
          $namafile = $syarat->id.".rar";
        } else {
	        $namafile = $syarat->id.".pdf";
	      }
      ...
      if (isset($_FILES["file_foto"]) && !empty($_FILES["file_foto"]["name"])) {
      $foto = $_FILES["file_foto"]["name"];
      $ext = "jpg";
      $lokasi = $_FILES['file_foto']['tmp_name'];

      $file = "foto_".$eksis->id.".".$ext;
      $dir2 = $path."/";
      move_uploaded_file($lokasi,$dir2.$file);
      // var_dump($lokasi, $dir2.$file);die();
    }
    
    	// Kirim Notifikasi ke Penerima otoritas 
	    $bidang = $sektor;
	    if ($bidang == "ENERGI DAN SUMBER DAYA MINERAL") {
	      $trkel = $otherdb->query("select kd_kab from trkelurahan where id = ".$kelurahan)->row_array();
	      $kd_kab = $trkel['kd_kab'];
	      ...
	      $host             = "smtp.gmail.com";
	      $emailpengirim    = "dpmptspjabar@gmail.com"; //sebelumnya dpmptsp-online@jabarprov.go.id
	      ...
	      foreach($querysms as $kirimsms){
	        ...
	        $val_kab = Array();
	        $tmpegawai = $otherdb->query("select A.val_kab from tmpegawai A 
	                                      INNER JOIN tmpegawai_user B ON A.id = B.tmpegawai_id 
	                                      where B.user_id = ". $kirimsms->id)->row_array();
	        if (!empty($tmpegawai['val_kab'])) {
	          $val_kab = explode("^", $tmpegawai['val_kab']);
	        }
	          ...
	          if (in_array($kd_kab, $val_kab)) {
	            if($cekid != ''){
	              ...
	              if($email != ''){
	                ...
	                $isi .= "<h2>DPMPTSP OnLine</h2><hr>";
	                $isi .= "<p>Terdapat Permohonan Izin Bidang: ".$bidang. "<br> Izin : ".$nizin."</p>";
	                $isi .= "<p>Nomor Referensi : ".$nomor."<br> Atas Nama : ".$pemohon."<br> Tanggal Daftar : ".$tgl_masuk."</p>";
	            ...
	            if($telp_pemohon != ''){
	              ...
	              $message  = urlencode("DPMPTSP OnLine NoRef.".$nomor." Tgl:".$tgl_masuk." a/n:".$pemohon." Izin:".$nizin);
    ...
    //End Kirim Notifikasi ke Penerima otoritas

    //}
    $this->session->set_flashdata('success', 'Berhasil Mengajukan Permohonan');
  ...
  public function editpermohonanbaru_back(){
    var_dump('test');die();
  }
  public function editpermohonanbaru(){
    ...
    $sql  =	"select trsyarat_perizinan.* from trsyarat_perizinan 
              where trsyarat_perizinan.id IN(select trsyarat_perizinan_id from trperizinan_trsyarat_perizinan 
              where trperizinan_id=? and status='1') ORDER BY (trsyarat_perizinan.urutan * -1) DESC, trsyarat_perizinan.status ASC, urutan = 0, urutan";
             
    $query = $otherdb->query($sql, $id_perizinan)->result();
    $ada_upload	= 0;
    foreach($query as $syarat){
      if(!empty($_FILES["file_".$syarat->id]["name"])){
        $ada_upload=1;
        $temp	= explode(".",$_FILES["file_".$syarat->id]["name"]); // get file ekstensi
        //if(end($temp)!="pdf"){
        $semogasaja = strtolower(end($temp));
        if($semogasaja == "rar" || $semogasaja == "pdf"){
        }else{
          $this->session->set_flashdata('error', 'Terjadi Kesalahan, mohon hanya mengunggah file pdf saja');
        ...
        if($this->db->update('tmpermohonan_trsyarat_perizinan', $data)){
        	if (file_exists($path.$syarat->id.".pdf")) {
        		unlink($path.$syarat->id.".pdf");
            $pdfformat = '.pdf';
        	}
          if (file_exists($path.$syarat->id.".rar")) {
            unlink($path.$syarat->id.".rar");
            $pdfformat = '.rar';
          }
          $pdfformat = '.'.strtolower(end($temp));
          $file = $syarat->id.$pdfformat;
          $lokasi=$_FILES['file_'.$syarat->id]['tmp_name'];
    ...
    if($ada_upload==1){
      ...
      $revisi = array('revisi' => "1");
      $otherdb->where('id_permohonan_portal', $permohonan->id);
      $otherdb->update('tmpemohon_portal', $revisi);
    }
  ...
  function cetak_resi($id) {
    ...
    $trperizinan_trsektor = $otherdb->get_where("trperizinan_trsektor",array("trperizinan_id"=>$permohonan_izin->trperizinan_id))->first_row();
    $sektor = $trperizinan_trsektor->trsektor_id;
    //Cetak ke PDF
    ...
    $link = 'https://dpmptsp.jabarprov.go.id/jelita/main/ceksts/index/';
    $codeContents = 'ID.'.$no_resi;
    ...
    $pdf->Ln(2); $pdf->SetX(50); $pdf->Cell(0,0.5,'LOKASI',0,1,'L');
                 $pdf->SetX(85); $pdf->Cell(0,-0.5,':',0,1,'L');
                 $pdf->SetXY(87, $pdf->GetY()-1.5); $pdf->MultiCell(0,4,htmlspecialchars_decode($permohonan->a_izin),0,'L');
    $pdf->Ln(2); $pdf->SetX(50); $pdf->Cell(0,0.5,'TANGGAL DAFTAR',0,1,'L');
                 $pdf->SetX(85); $pdf->Cell(0,-0.5,':',0,1,'L');
                 $pdf->SetX(87); $pdf->Cell(0,0,date('d-m-Y', strtotime($permohonan->d_terima_berkas)),0,1,'L');
    ...
    $pdf->SetXY(53,95); $pdf->MultiCell(0,2.5,'Status permohonan atau Informasi dapat dicek di Website https://dpmptsp.jabarprov.go.id / 022 2112 5000 (Gratis) atau Scan QRCode untuk link webcek.',0,'L');
  ...
  public function cetak_sk($id=NULL, $link=NULL) {
    $permohonan_portal = new tmpermohonan_portal();
    $permohonan_portal->where('id', $id)->get();
    $otherdb  = $this->load->database('otherdb', TRUE);
    $no_resi = $permohonan_portal->no_permohonan;
    ...
    $lok_nonsign = 'backoffice/assets/skpdf/';
  ...
  public function cetak_tolak($id) {
  ...
  function detail($uuid=NULL, $his=NULL) {
    ...
    if($permohonan->editable=="1" && count($belakang)>0){
      ...
      $sql = "select trsyarat_perizinan.* from trsyarat_perizinan 
              where trsyarat_perizinan.id IN(select trsyarat_perizinan_id from trperizinan_trsyarat_perizinan 
              where trperizinan_id=? and status='1') ORDER BY (trsyarat_perizinan.urutan * -1) DESC, trsyarat_perizinan.status ASC, urutan = 0, urutan";
      $query 	= $otherdb->query($sql, $belakang->izin)->result();
      ...
      $data['obj']        = $permohonan->lokasi_izin;
    ...
    $idbelakang = '';
    if (!empty($belakang->izin)) {
      $idbelakang = $belakang->izin;
    }
    ...
    $sql = "select trsyarat_perizinan.* from trsyarat_perizinan 
              where trsyarat_perizinan.id IN(select trsyarat_perizinan_id from trperizinan_trsyarat_perizinan 
              where trperizinan_id=? and status='1') ORDER BY (trsyarat_perizinan.urutan * -1) DESC, trsyarat_perizinan.status ASC, urutan = 0, urutan";
    $query = $otherdb->query($sql, $idbelakang)->result();
    ...
    if(!empty($permohonan->no_permohonan)){
      ...
      if(count($databackoffice)==1){
        ...
        $stat = $otherdb->get_where("trstspermohonan",array("id"=>$databackoffice->kd_status))->first_row();
        
        if(count($stat)!=0){
          $status = $stat->n_sts_permohonan_dashboard;
        }
        if ($databackoffice->status_berkas == "Izin Ditolak") {
          $status = "DITOLAK";
        }elseif($databackoffice->status_berkas == "Izin Disetujui") {
          $status = "DISETUJUI";
        }	
      }else{
        $status = "Ditolak";
  ...
  function detailpenolakan($uuid=NULL, $his=NULL) {
    $uuid     =	htmlspecialchars($uuid,ENT_QUOTES);
    $session  = $this->session->userdata("userlogin");
    $username = $this->session->userdata("username");
    $otherdb  = $this->load->database('otherdb', TRUE); // the TRUE paramater tells CI that you'd like to return the database object.
    ...
    $tmpermohonan = $otherdb->get_where("tmpermohonan", array("pendaftaran_id"=>$permohonan->no_permohonan))->first_row();

    $qalasan = array();
    if (!empty($tmpermohonan->id)) {
    	$alasan = "select * from alasan_penolakan where id_permohonan='".$tmpermohonan->id."'";
    	$qalasan = $otherdb->query($alasan)->result();
    }
    
    $data['property']  	 = $qry;
    $data['title']       = "Detail Penolakan";
    $data['load']        = "permohonan/detail_penolakan";
    $data['nama']        = $nama->n_perizinan;
    $data['username']    = $username;
    $data['pemohon']     = $user->namaPerusahaan;
    $data['permohonan']  = $permohonan;
  ...
  public function setlat() {
    ...
    $otherdb->select('*');
    $otherdb->from('survei_perijinan_portal');
    $lat = $otherdb->get();

    foreach ($lat->result() as $kel) {
          if (!empty($kel->lon) && !empty($kel->lat)) {
              $revisi = array('lon' => $kel->lon, 'lat' => $kel->lat);
              $otherdb->where('id', $kel->portal_id);
              if ($otherdb->update('tmpemohon_portal', $revisi)) {
                echo "OK <br>";
              } else {
                echo "Not OK <br>";
              }
