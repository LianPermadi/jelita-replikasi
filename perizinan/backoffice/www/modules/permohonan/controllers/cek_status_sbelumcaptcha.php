...
class Cek_status extends MY_Controller {
  ...
  function get2() {
    ...
    if (strpos($id, $cari) !== false) {
      echo "Sedang dalam pengembangan. <br> Nomor tersebut belum terkoneksi dengan aplikasi pusat. <br> <a href='https://sicantikui.layanan.go.id/#/login'>Info Lebih Lanjut</a>";die;
      redirect("https://sicantikui.layanan.go.id/#/login");
    }
    ...
    $urls = 'https://www.google.com/recaptcha/api/siteverify?secret=' . urlencode($secretKey) .  '&response=' . urlencode($gcaptcha);
    ...
    if($news_items == NULL) {
      ...
      foreach($news_items as $item) {
        ...
        $alamat = $this->xml_parsing_win->value_in('alamat', $item);
        $permohonan = $this->xml_parsing_win->value_in('permohonan', $item);
        $indeks = $this->xml_parsing_win->value_in('indeks', $item);
        $tracking = $this->xml_parsing_win->value_in('tracking', $item);
        $n_kelompok_izin = $this->xml_parsing_win->value_in('n_kelompok_izin', $item);
        ...
        $skm = $this->xml_parsing_win->value_in('skm', $item);
