...
class Permohonan extends MY_Controller {
  ...
  function step1(){
    ...
    //$query	= $otherdb->query("select * from trsektor")->result();
	  $query	= $otherdb->query("select * from trsektor WHERE `n_sektor` != '*Lain-lain'")->result();
  ...
  function step2($id) {
    ...
    //$query	= $otherdb->query("select * from trperizinan where c_aktif = '0' AND c_online = '0'  
    //                           And id in(select trperizinan_id from trperizinan_trsektor where trsektor_id='".$id."')")->result();
  ...
  function savestep3($cid){  // proses simpan step 1, 2 da 3
    ...
    // Input ke database back office status harus diubah jadi 1
  ...
  function addpermohonanbaru(){  // proses simpan data permohonan
    $session  = $this->session->userdata("userlogin");
    $username = $this->session->userdata("username");
    $otherdb  = $this->load->database('otherdb', TRUE); // the TRUE paramater tells CI that you'd like to return the database object.
    ...
    if($this->db->insert('tmpermohonan_portal',$data)){
      ...
      //}
      // EOF() Kirim Notifikasi ke Penerima otoritas 
      ...
      $sql = "select trsyarat_perizinan.* from trsyarat_perizinan 
                    where trsyarat_perizinan.id IN(select trsyarat_perizinan_id from trperizinan_trsyarat_perizinan where trperizinan_id='".$id."' and status='1')";
      $query = $otherdb->query($sql)->result();
