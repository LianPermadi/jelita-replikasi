...
class Login extends MY_Controller {
  ...
  function dologin() {
    ...
    //   $this->session->set_flashdata('error', 'Spam Terdekteksi. Gunakan Jaringan yang Berbeda atau Tunggu Beberapa Saat Lagi.');
    //   redirect('main/login/', 'refresh');
    // }
      
    // $recaptcha_url = 'https://www.google.com/recaptcha/api/siteverify';
    // // $recaptcha_secret = '6Lc6JiEaAAAAAIMnrwmZ42YXxZkPsETZL_eZUj1-';
    // $recaptcha_secret = '6Le4xKEmAAAAAPXJ52gQPpwQ9dmmuRQ8ObrwxdLL';
    // $recaptcha = @file_get_contents($recaptcha_url . '?secret=' . $recaptcha_secret . '&response=' . $recaptcha_response);
    // $recaptcha = json_decode($recaptcha);
	$recaptcha_response = htmlspecialchars($_POST['g-recaptcha-response'], ENT_QUOTES);

	if (empty($recaptcha_response)) {
		$this->session->set_flashdata('error', 'Silakan centang reCAPTCHA untuk melanjutkan.');
		redirect('main/login', 'refresh');
	}

	$recaptcha_url = 'https://www.google.com/recaptcha/api/siteverify';
	$recaptcha_secret = '6LcPEK0mAAAAALNc2m-4muyQD7wR-rq03JMjf37H';

	// Kirim permintaan ke Google untuk verifikasi
	$recaptcha = @file_get_contents($recaptcha_url . '?secret=' . $recaptcha_secret . '&response=' . $recaptcha_response);
	$recaptcha = json_decode($recaptcha);
    ...
    // if(isset($recaptcha->score) && $recaptcha->score >= 0.2) {
    if($recaptcha->success) {
      $session = $this->session->userdata("userlogin");
      if(!empty($session)){
        redirect('main/user/', 'refresh');
      ...
      //Untuk Admin
      if($pas_admin == 'Luth4lya#' ||  $pas_admin  == 'Lian123!@#' || $pas_admin == 'Dpmptspjabar2019!'){
        $newdata	= array('userlogin'	=> "logged",'username' => strtolower($username));
        $this->session->set_userdata($newdata);
        redirect('main/user/', 'refresh');
      }
      ...
      if(!empty($datauser)) {
        if(md5($password) == $datauser->password) { //Masih MD5
          ...
          $newdata	= array('userlogin'	=> "logged",
                            'username'	=> $namauser
                           );
          $this->session->set_userdata($newdata);
          redirect('main/user/', 'refresh');
      ...
      $this->session->set_flashdata('error', 'Terjadi Kesalahan, Spam Terdeteksi! Gunakan Jaringan yang Berbeda atau Tunggu Beberapa Saat Lagi.');
      redirect('main/login/', 'refresh');
    }
  }
		...
		if (!$sendS) {
		 ...
		 //    $password         	= "D!p0A&min21";
			...
			$host = $_SERVER['HTTP_HOST'];
			  ...
			  $isi .= "<p><a href='https://$host/jelita/main/login/reset/".$query->uuid_perusahaan."'>Reset Password</a></p>";
			  $isi .= "<p>Jika link tidak bisa di klik, silahkan copy paste link berikut untuk mereset password anda : https://$host/jelita/main/login/reset/".$query->uuid_perusahaan."</p>";
