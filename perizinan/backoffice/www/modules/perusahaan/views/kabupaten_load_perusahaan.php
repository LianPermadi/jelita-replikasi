<script type="text/javascript">
    ...
    $(document).ready(
		function(){
            $('#'+kabupaten_id+'_id').change(
				function(){
                    $.post(base_url+'perusahaan/'+kecamatan_id, {kabupaten_id: $('#'+kabupaten_id+'_id').val()}, 
						function(response){
...
<div class="contentForm">
    ...
    if($_REQUEST){
        ...
        if(!empty($row)){
			echo form_dropdown($kabupaten_id, $opsi_kabupaten, '0','class = "input-select-wrc notSelect" id="'.$kabupaten_id.'_id"');
        }else{ 
            $opsi_kabupaten['0'] = "------Tidak ada data------";
            echo form_dropdown($kabupaten_id, $opsi_kabupaten, '0','class = "input-select-wrc notSelect" id="'.$kabupaten_id.'_id"');
        }
    }
    ?>
