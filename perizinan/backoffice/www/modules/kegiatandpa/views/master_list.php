      ...
      <table cellpadding="0" cellspacing="0" border="0" class="display" id="pendataan">
        <thead>
          <tr>
            ...
            <th width="10%">Kode Rekening<br>Anggaran<br>Realisasi</th>
            <th width="10%">Aksi</th>
          </tr>
        </thead>
        <tbody>
          ...
          <tr>
            ...
            <td>
              <?php 
              echo '<b>'.$row->nama_tim .'</b><br>';
              
              if (file_exists('assets/tot/sk/SK_Tim_' . $row->nama_tim . '_' . $row->id . '.pdf')) { 
                  echo "<a href='" . base_url() . "kegiatandpa/unduh_sk/" . $row->id . "' style='color:blue;' target='_blank' title='Unduh Berkas'>SK Tim</a>&nbsp;"; 
              } 
              ?>
            </td>
            ...
            <td><?php echo '('.$this->m_renaksi->get_kodering_anggaran_kd($row->kode_ring).')<br> Rp.'.number_format($row->anggaran,0,',','.').'<br> Rp. '.number_format($double,0,',','.');?></td>
            <td>
              <?php 
