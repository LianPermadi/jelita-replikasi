...
// var_dump($tim);die();
  if (!empty($koor)) {
    $id = $koor->id;
    $nama_tim = $koor->nama_tim;
    $kode_pengampu = $koor->pengampu;
    $koordinator = $koor->id_pegawai;
    $kode_tim = $koor->kode_tim;
    $status = $koor->status;
    $tahun = $koor->thn_anggaran;
  } else {
    $nama_tim = "";
    $kode_pengampu = "";
    $koordinator = "";
            ...
            <tbody>
              ...
              <tr>
                <td align="left" width="15%">
                  <b>Kode Koordinator</b>
                </td>
                <td>
                  <input type="text" name="kode_pengampu" style="width:100%" class="input-wrc" value="<?php echo $kode_pengampu; ?>" required="required">
                </td>
              </tr>
              <tr>
                <td align="left" width="15%" class="bg-grid">
                  <b>Nama Koordinator</b>
                </td>
                <td class="bg-grid">
                  <select class="pilihan" name="id_pegawai" style="width:100%">
                      <?php foreach ($pegawai as $pegawai): ?>
                        <option value="<?php echo $pegawai->id; ?>" <?php echo ($koordinator == $pegawai->id ? "selected" : ""); ?>><?php echo $pegawai->n_pegawai; ?></option>
                      <?php endforeach ?>
