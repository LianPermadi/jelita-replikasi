          ...
          <div id="tabs">
            ...
            if(!$back){ 
              ...
              while($data = mysql_fetch_assoc(@$results)){
                ...
                $query = "select A.id, A.pendaftaran_id, A.c_izin_selesai, A.status_berkas, A.d_ambil_izin, B.trperizinan_id, G.tgl_surat, G.tgl_surat_edit 
                          from tmpermohonan A
                ...
                switch($list_kat){
                  ...
                  case 1: // untuk tanggal selesai
                    if($list_state === '0') { // Untuk Seluruh Data
                      $query .= "and (G.tgl_surat_edit between '$tgla' and '$tglb')";
                    }else{
                      $query .= "and (G.tgl_surat_edit between '$tgla' and '$tglb') 
                                 and A.kd_gerai = '$list_state'";
                    }
                  ...
                  while($rows_data = mysql_fetch_assoc(@$hasil_data)){  // menghitung per sektor
                    ...
                    if(substr($data['n_sektor'], 0, 1) != '*') { // untuk bukan izin lain2
                      ...
                      if($rows_data['status_berkas'] == "Izin Disetujui"){
                          ...
                          if($rows_data['tgl_surat_edit'] >= $tgla && $rows_data['tgl_surat_edit'] <= $tglb){
                          	$dan = FALSE;
                            $jumlah_terbit++; $id_jml_terbit .= $rows_data['id'].'^';
                            $sts_jumlah_terbit = 1; $sts_id_jml_terbit = $rows_data['id'].'^';	
                          }
                      ...
                      if($list_kat != 2){
                        ...
                        //if(($rows_data['tgl_surat'] >= $tgla && $rows_data['tgl_surat'] <= $tglb) || $rows_data['status_berkas'] == "proses"){
                        if($dan){                        
                          $jumlah_proses++; $id_jml_proses .= $rows_data['id'].'^';
                          $sts_jumlah_proses = 1; $sts_id_jml_proses = $rows_data['id'].'^';	
                        }
                      ...
                      if(substr($n_sektor, 0, 1) != '*') {
                        ...
                        foreach ($perizinan_sektor as $data_izin){
                          ...
                          $b = '';
			                    $be = '';
			                    $tampil = 'TRUE';
                          if($data_izin->c_aktif == '1' && $data_izin->c_online == '1'){
                            $aktif = ' [Tidak Aktif]';
                            $b = '<span style="color: Red">';
			                      $be = '</span>';
			                      if($dmrekap_izin->jml_masuk == '') $tampil = 'FALSE';
                          }
                          if($tampil){
                            $i1++;
                            //$dmrekap_izin = new dmrekap_izin();
                            ...
                            <tr bgcolor="#FEF9BF">
                      	      ...
                      	      <td>              <font size="2" color="#1A1A1A"><?php echo $b.$n_perizinan . $aktif . $be; ?></font></td>
