...
class Izin extends WRC_AdminCont {
  ...
  public function __construct() {
    ...
    //$kat_cari 
      if(in_array("18", $role)) { //admin
        $this->useradmin = TRUE;
        $this->kat_cari = array('0'=>'Tanggal Permohonan','1'=>'Tanggal Selesai','2'=>'Tanggal Permohonan & Tanggal Selesai');
      } else {
        $this->kat_cari = array('2'=>'Tanggal Permohonan & Tanggal Selesai');
      }
      //EOF() kat_cari
  ...
  public function rekaptest() {  // pertama masuk saat klik menu
    $refresh = $this->input->post('refresh');
    $list_kat = $this->input->post('list_kat');
    $data['list_kat'] = $list_kat;
    $data['kat_cari']   = $this->kat_cari;
    ...
    $this->template->set_metadata_javascript($js);
    $this->session_info['page_name'] = "Rekapitulasi Perizinan";
    $this->template->build('izin_list_test', $this->session_info);
  }

  public function rekaplist() { // masuk setelah klik filter
    $tgla = (!empty($this->input->post('tgla')) ? $this->input->post('tgla') : date("Y-m-d", mktime(0, 0, 0, date("m"), 1, date("Y"))));
    $tglb = (!empty($this->input->post('tglb')) ? $this->input->post('tglb') : date("Y-m-d", mktime(0, 0, 0, date("m")+1, 0, date("Y"))));
    if ($this->useradmin) {
      $list_kat = $this->input->post('list_kat');
    } else {
      $list_kat = (!empty($this->input->post('list_kat')) ? $this->input->post('list_kat') : 2);
    }
    $list_state = $this->input->post('list_state');
    $now = $this->lib_date->get_date_now();
    $tgl_before = date("Y-m-d", mktime(0, 0, 0, date("m"), 1, date("Y")));
      ...
      switch($list_kat){
        case 0: // untuk tanggal terima Berkas
          if($list_state === '0') { // Untuk Seluruh Data
            $val = "E.d_terima_berkas_asli BETWEEN ? AND ?";
          } else {
            $val = "(E.d_terima_berkas_asli BETWEEN ? AND ?) AND E.kd_gerai = ?";
          }
          break;
        case 1: // untuk tanggal selesai
          if($list_state === '0') { // Untuk Seluruh Data
            $val = "I.tgl_surat_edit BETWEEN ? AND ?";
          } else {
            $val = "(I.tgl_surat_edit BETWEEN ? AND ?) AND E.kd_gerai = ?";
          }
          break;
        case 2: // untuk tanggal terima Berkas dan tanggal selesai
          if($list_state === '0') { // Untuk Seluruh Data
            ...
            $filter3 = 1;
            $val = "(E.d_terima_berkas_asli BETWEEN ? AND ?) AND E.kd_gerai = ?";
            $valfil = "(E.d_terima_berkas_asli BETWEEN ? AND ? AND I.tgl_surat_edit >= ? AND I.tgl_surat_edit <= ?) AND E.kd_gerai = ?";
          }
          break;
      }
    }
    ...
    $result_perizinan = '';
        ...
        if ($filter3 == 1) {
          $sql = "SELECT A.id, A.n_sektor, SUM(CASE WHEN ".$val." AND G.id <> 1 THEN 1 ELSE 0 END) AS JMLPERMOHONAN, SUM(CASE WHEN E.status_berkas = 'Izin Ditolak FO' AND ".$valfil." THEN 1 ELSE 0 END) AS JMLTOLAKFO, SUM(CASE WHEN (E.status_berkas = 'Izin Disetujui' AND ".$valfil.") THEN 1 ELSE 0 END) AS JMLSETUJUI, SUM(CASE WHEN (E.status_berkas = 'Izin Disetujui' AND E.d_ambil_izin IS NOT NULL AND ".$valfil.") THEN 1 ELSE 0 END) AS JMLSETUJUIAMBIL, SUM(CASE WHEN (E.status_berkas = 'Izin Disetujui' AND E.d_ambil_izin IS NULL AND ".$valfil.") THEN 1 ELSE 0 END) AS JMLSETUJUIBLMAMBIL, SUM(CASE WHEN (E.status_berkas = 'Izin Ditolak' AND ".$valfil.") THEN 1 ELSE 0 END) AS JMLTOLAK, SUM(CASE WHEN (E.status_berkas = 'proses' AND ".$val.") THEN 1 ELSE 0 END) AS JMLPROSES
            FROM trsektor A
            ...
            $sql_perizinan = "SELECT C.id, A.n_sektor, C.n_perizinan, C.kd_izin, C.c_online, C.c_aktif, SUM(CASE WHEN ".$val." AND C.id IS NOT NULL AND E.pendaftaran_id IS NOT NULL AND G.id <> 1 THEN 1 ELSE 0 END) AS JMLPERMOHONAN, SUM(CASE WHEN ".$valfil." AND C.id IS NOT NULL AND E.status_berkas = 'Izin Ditolak FO' AND E.pendaftaran_id IS NOT NULL THEN 1 ELSE 0 END) AS JMLTOLAKFO, SUM(CASE WHEN ".$valfil." AND E.status_berkas = 'Izin Disetujui' AND E.pendaftaran_id IS NOT NULL THEN 1 ELSE 0 END) AS JMLSETUJUI, SUM(CASE WHEN (".$valfil." AND E.status_berkas = 'Izin Disetujui' AND E.d_ambil_izin IS NOT NULL AND E.pendaftaran_id IS NOT NULL) THEN 1 ELSE 0 END) AS JMLSETUJUIAMBIL, SUM(CASE WHEN (".$valfil." AND E.status_berkas = 'Izin Disetujui' AND E.d_ambil_izin IS NULL AND E.pendaftaran_id IS NOT NULL) THEN 1 ELSE 0 END) AS JMLSETUJUIBLMAMBIL, SUM(CASE WHEN (".$valfil." AND E.status_berkas = 'Izin Ditolak' AND E.pendaftaran_id IS NOT NULL) THEN 1 ELSE 0 END) AS JMLTOLAK, SUM(CASE WHEN (".$val." AND E.status_berkas = 'proses' AND E.pendaftaran_id IS NOT NULL) THEN 1 ELSE 0 END) AS JMLPROSES
              FROM trsektor A
              LEFT JOIN trperizinan_trsektor B on A.id = B.trsektor_id
              LEFT JOIN trperizinan C on B.trperizinan_id = C.id
              LEFT JOIN tmpermohonan_trperizinan D ON C.id = D.trperizinan_id
              ...
              LEFT JOIN tmsk I on H.tmsk_id = I.id
              GROUP BY A.id, C.id
              ORDER BY A.urutan, C.kd_izin";
        } else {
          $sql = "SELECT A.id, A.n_sektor, SUM(CASE WHEN ".$val." AND G.id <> 1 THEN 1 ELSE 0 END) AS JMLPERMOHONAN, SUM(CASE WHEN (E.status_berkas = 'Izin Ditolak FO' AND ".$val.") THEN 1 ELSE 0 END) AS JMLTOLAKFO, SUM(CASE WHEN (E.status_berkas = 'Izin Disetujui' AND ".$val.") THEN 1 ELSE 0 END) AS JMLSETUJUI, SUM(CASE WHEN (E.status_berkas = 'Izin Disetujui' AND E.d_ambil_izin IS NOT NULL AND ".$val.") THEN 1 ELSE 0 END) AS JMLSETUJUIAMBIL, SUM(CASE WHEN (E.status_berkas = 'Izin Disetujui' AND E.d_ambil_izin IS NULL AND ".$val.") THEN 1 ELSE 0 END) AS JMLSETUJUIBLMAMBIL, SUM(CASE WHEN (E.status_berkas = 'Izin Ditolak' AND ".$val.") THEN 1 ELSE 0 END) AS JMLTOLAK, SUM(CASE WHEN (E.status_berkas = 'proses' AND ".$val.") THEN 1 ELSE 0 END) AS JMLPROSES
            FROM trsektor A
            LEFT JOIN trperizinan_trsektor B ON A.id = B.trsektor_id
            LEFT JOIN trperizinan C ON B.trperizinan_id = C.id
            LEFT JOIN tmpermohonan_trperizinan D ON C.id = D.trperizinan_id
         ...
         $sql_perizinan = "SELECT C.id, A.n_sektor, C.n_perizinan, C.kd_izin, C.c_online, C.c_aktif, SUM(CASE WHEN ".$val." AND C.id IS NOT NULL AND E.pendaftaran_id IS NOT NULL AND G.id <> 1 THEN 1 ELSE 0 END) AS JMLPERMOHONAN, SUM(CASE WHEN ".$val." AND C.id IS NOT NULL AND E.status_berkas = 'Izin Ditolak FO' AND E.pendaftaran_id IS NOT NULL THEN 1 ELSE 0 END) AS JMLTOLAKFO, SUM(CASE WHEN ".$val." AND E.status_berkas = 'Izin Disetujui' AND E.pendaftaran_id IS NOT NULL THEN 1 ELSE 0 END) AS JMLSETUJUI, SUM(CASE WHEN (".$val." AND E.status_berkas = 'Izin Disetujui' AND E.d_ambil_izin IS NOT NULL AND E.pendaftaran_id IS NOT NULL) THEN 1 ELSE 0 END) AS JMLSETUJUIAMBIL, SUM(CASE WHEN (".$val." AND E.status_berkas = 'Izin Disetujui' AND E.d_ambil_izin IS NULL AND E.pendaftaran_id IS NOT NULL) THEN 1 ELSE 0 END) AS JMLSETUJUIBLMAMBIL, SUM(CASE WHEN (".$val." AND E.status_berkas = 'Izin Ditolak' AND E.pendaftaran_id IS NOT NULL) THEN 1 ELSE 0 END) AS JMLTOLAK, SUM(CASE WHEN (".$val." AND E.status_berkas = 'proses' AND E.pendaftaran_id IS NOT NULL) THEN 1 ELSE 0 END) AS JMLPROSES
          FROM trsektor A
          LEFT JOIN trperizinan_trsektor B on A.id = B.trsektor_id
          LEFT JOIN trperizinan C on B.trperizinan_id = C.id
          LEFT JOIN tmpermohonan_trperizinan D ON C.id = D.trperizinan_id
        ...
        if ($list_state === '0') {
          if ($filter3 == 1) {
            $result = $this->db->query($sql, array($tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb))->result();

          $result_perizinan = $this->db->query($sql_perizinan, array($tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb))->result();
          } else {
            $result = $this->db->query($sql, array($tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb))->result();

          $result_perizinan = $this->db->query($sql_perizinan, array($tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb, $tgla, $tglb))->result();
          }
          
        } else {
          if ($filter3 == 1) {
            $result = $this->db->query($sql, array($tgla, $tglb, $list_state, $tgla, $tglb, $tgla, $tglb, $list_state, $tgla, $tglb, $tgla, $tglb, $list_state, $tgla, $tglb, $tgla, $tglb, $list_state, $tgla, $tglb, $tgla, $tglb, $list_state, $tgla, $tglb, $tgla, $tglb, $list_state, $tgla, $tglb, $list_state))->result();

            $result_perizinan = $this->db->query($sql_perizinan, array($tgla, $tglb, $list_state, $tgla, $tglb, $tgla, $tglb, $list_state, $tgla, $tglb, $tgla, $tglb, $list_state, $tgla, $tglb, $tgla, $tglb, $list_state, $tgla, $tglb, $tgla, $tglb, $list_state, $tgla, $tglb, $tgla, $tglb, $list_state, $tgla, $tglb, $list_state))->result();
          } else {
            $result = $this->db->query($sql, array($tgla, $tglb, $list_state, $tgla, $tglb, $list_state, $tgla, $tglb, $list_state, $tgla, $tglb, $list_state, $tgla, $tglb, $list_state, $tgla, $tglb, $list_state, $tgla, $tglb, $list_state))->result();

            $result_perizinan = $this->db->query($sql_perizinan, array($tgla, $tglb, $list_state, $tgla, $tglb, $list_state, $tgla, $tglb, $list_state, $tgla, $tglb, $list_state, $tgla, $tglb, $list_state, $tgla, $tglb, $list_state, $tgla, $tglb, $list_state))->result();
          }
    ...
    //$data['result_perizinan'] = $result_perizinan;

    $data_perizinan = array();
    foreach ($result_perizinan as $dat) {
      $sektor = $dat->n_sektor;
      if (!array_key_exists($sektor, $data_perizinan)) {
        $data_perizinan[$sektor] = array();
      }
      $data_perizinan[$sektor][] = array(
                                  'id' => $dat->id,
                                  'n_perizinan' => $dat->n_perizinan,
                                  'kd_perizinan' => $dat->kd_izin,
                                  'c_online' => $dat->c_online,
                                  'c_aktif' => $dat->c_aktif,
                                  'jmlpermohonan' => $dat->JMLPERMOHONAN,
                                  'jmltolakfo' => $dat->JMLTOLAKFO,
                                  'jmlsetujui' => $dat->JMLSETUJUI,
                                  'jmlsetujuiambil' => $dat->JMLSETUJUIAMBIL,
                                  'jmlsetujuiblmambil' => $dat->JMLSETUJUIBLMAMBIL,
                                  'jmltolak' => $dat->JMLTOLAK,
                                  'jmlproses' => $dat->JMLPROSES);
    }
    ...
    $this->template->set_metadata_javascript($js);
    $this->session_info['page_name'] = "Rekapitulasi Perizinan";
    $this->template->build('izin_rekap_test', $this->session_info);
  }
  ...
  public function list_data_OLD($id_key = null, $menu = null) {
    ...
    $data['izin'] = $jdl_izin;
    $data['katagori'] = 'BERDASAR '.$this->kat_cari[$list_kat];
    ...
    $this->template->set_metadata_javascript($js);
    $this->session_info['page_name'] = "List Data Perizinan Berdasar ".$this->kat_cari[$list_kat];
  ...
  public function list_data($id_key = null, $menu = null) {
    ...
    $dmrekap_izin->get_by_id($id_key);
    $trsektor_id = $dmrekap_izin->trsektor_id;
    $trperizinan_id = $dmrekap_izin->trperizinan_id;
    $sektor = new trsektor();
    $sektor->get_by_id($trsektor_id);
    $perizinan = new trperizinan();
    $perizinan->get_by_id($trperizinan_id);
    $jdl_bidang = $sektor->n_sektor;
    if($menu >= 10){ // info per jenis izin
      $jdl_izin = $perizinan->n_perizinan;
    }else{
      $jdl_izin = '';
    }
    $list_izin = '';
    switch($menu){
      case 1:  $jml_list_izin = $dmrekap_izin->jml_masuk;     $list_izin = $dmrekap_izin->id_jml_masuk;      break;
      case 2:  $jml_list_izin = $dmrekap_izin->jml_tolak_fo;  $list_izin = $dmrekap_izin->id_jml_tolak_fo;   break;
      case 3:  $jml_list_izin = $dmrekap_izin->jml_terbit;    $list_izin = $dmrekap_izin->id_jml_terbit;     break;
      case 4:  $jml_list_izin = $dmrekap_izin->terbit_diambil;$list_izin = $dmrekap_izin->id_terbit_diambil; break;
      case 5:  $jml_list_izin = $dmrekap_izin->terbit_proses; $list_izin = $dmrekap_izin->id_terbit_proses;  break;
      case 6:  $jml_list_izin = $dmrekap_izin->jml_tolak;     $list_izin = $dmrekap_izin->id_jml_tolak;      break;
      case 7:  $jml_list_izin = $dmrekap_izin->tolak_diambil; $list_izin = $dmrekap_izin->id_tolak_diambil;  break;
      case 8:  $jml_list_izin = $dmrekap_izin->tolak_proses;  $list_izin = $dmrekap_izin->id_tolak_proses;   break;
      case 9:  $jml_list_izin = $dmrekap_izin->jml_proses;    $list_izin = $dmrekap_izin->id_jml_proses;     break;
      case 10: $jml_list_izin = $dmrekap_izin->jml_masuk;     $list_izin = $dmrekap_izin->id_jml_masuk;      break;
      case 11: $jml_list_izin = $dmrekap_izin->jml_tolak_fo;  $list_izin = $dmrekap_izin->id_jml_tolak_fo;   break;
      case 12: $jml_list_izin = $dmrekap_izin->jml_terbit;    $list_izin = $dmrekap_izin->id_jml_terbit;     break;
      case 13: $jml_list_izin = $dmrekap_izin->terbit_diambil;$list_izin = $dmrekap_izin->id_terbit_diambil; break;
      case 14: $jml_list_izin = $dmrekap_izin->terbit_proses; $list_izin = $dmrekap_izin->id_terbit_proses;  break;
      case 15: $jml_list_izin = $dmrekap_izin->jml_tolak;     $list_izin = $dmrekap_izin->id_jml_tolak;      break;
      case 16: $jml_list_izin = $dmrekap_izin->tolak_diambil; $list_izin = $dmrekap_izin->id_tolak_diambil;  break;
      case 17: $jml_list_izin = $dmrekap_izin->tolak_proses;  $list_izin = $dmrekap_izin->id_tolak_proses;   break;
      case 18: $jml_list_izin = $dmrekap_izin->jml_proses;    $list_izin = $dmrekap_izin->id_jml_proses;     break;  
    }
    $data['tgla'] = $tgla;
    $data['tglb'] = $tglb;
    $data['gerai'] = $gerai;
    ...
    $data['bidang'] = $jdl_bidang;
    $data['izin'] = $jdl_izin;
    $data['katagori'] = 'BERDASAR '.(!empty($this->kat_cari[$list_kat]) ? $this->kat_cari[$list_kat] : '');
    $data['n_menu'] = $this->menu_filter('2', $menu);
    $data['list'] = $list_izin;
    $data['id_key'] = $id_key;
    $data['menu'] = $menu;
    ...
    $this->template->set_metadata_javascript($js);
    $this->session_info['page_name'] = "List Data Perizinan Berdasar ".(!empty($this->kat_cari[$list_kat]) ? $this->kat_cari[$list_kat] : '');
    $this->template->build('view_list_data', $this->session_info);
  }

  public function list_data_test($tipe_rekap = null, $id = null, $list_state = 0, $list_kat = 0, $tgla = 0, $tglb = 0, $jenis_jumlah = null) {
    $username = new user();
    $username->where('username', $this->session->userdata('username'))->get();
    $id_user = $username->id;
    $judul = '';
    ...
    $data['tgla'] = $tgla;
    $data['tglb'] = $tglb;
    $data['gerai'] = $list_state;
    $data['list_kat'] = $list_kat;
    $data['list_state'] = $list_state;
    $data['jenis_jumlah'] = $jenis_jumlah;
    $data['id'] = $id;
    $data['tipe_rekap'] = $tipe_rekap;

    if ($tipe_rekap == 0) {
      $sektor = new trsektor();
      $sektor->get_by_id($id);
      $judul = 'SEKTOR '.$sektor->n_sektor;

      $val_rekap = "K.id = ? ";
    } else {
      $perizinan = new trperizinan();
      $perizinan->get_by_id($id);
      $judul = 'PERIZINAN '.$perizinan->n_perizinan;

      $val_rekap = "I.id = ? ";
    }

    if ($list_state != 0) {
      $val_state = "AND A.kd_gerai = ? ";
    } else {
      $val_state = " ";
    }

    if ($list_kat == 1) {
      $val_kat = "AND G.tgl_surat_edit";
    } else {
      $val_kat = "AND A.d_terima_berkas_asli";
    }

    switch($jenis_jumlah){
      case 1:  $val_jumlah = ""; $val_mohon = "JUMLAH PERMOHONAN"; break;
      case 2:  $val_jumlah = " AND A.status_berkas = 'Izin Ditolak FO'"; $val_mohon = "IZIN DITOLAK FRONT OFFICE"; break;
      case 3:  $val_jumlah = " AND A.status_berkas = 'Izin Disetujui'"; $val_mohon = "JUMLAH IZIN DESETUJUI"; break;
      case 4:  $val_jumlah = " AND (A.status_berkas = 'Izin Disetujui' AND A.d_ambil_izin IS NOT NULL)"; $val_mohon = "IZIN DISETUJUI DIAMBIL"; break;
      case 5:  $val_jumlah = " AND (A.status_berkas = 'Izin Disetujui' AND A.d_ambil_izin IS NULL)"; $val_mohon = "IZIN DISETUJUI BELUM DIAMBIL"; break;
      case 6:  $val_jumlah = " AND A.status_berkas = 'Izin Ditolak'"; $val_mohon = "JUMLAH IZIN DITOLAK"; break;
      case 7:  $val_jumlah = ($list_kat != 2 ? " AND A.status_berkas = 'proses'" : ""); $val_mohon = ($list_kat != 2 ? "JUMLAH DALAM PROSES" : "JUMLAH MELEBIHI PERIODE"); break;
    }

    //$this->lib_date->post_variable($username->id, $tgla, $tglb, $gerai, $id_key, '', '', '', '', $list_kat, $menu);   // post variable

    $sql = "SELECT A.id AS tmpermohonan_id, A.pendaftaran_id, A.kd_status, A.desc_arsip, A.nama_file, C.n_pemohon, E.n_perusahaan, A.kd_gerai, A.d_terima_berkas_asli, A.d_selesai_proses, A.c_izin_selesai, G.tgl_penetapan, G.tgl_surat_edit, I.n_perizinan, A.a_izin, A.d_berlaku_izin, A.status_berkas, K.n_sektor, M.id AS id_trkel
      FROM tmpermohonan A
      LEFT JOIN tmpemohon_tmpermohonan B ON A.id = B.tmpermohonan_id
      LEFT JOIN tmpemohon C ON B.tmpemohon_id = C.id
      LEFT JOIN tmpermohonan_tmperusahaan D ON A.id = D.tmpermohonan_id
      LEFT JOIN tmperusahaan E ON D.tmperusahaan_id = E.id
      LEFT JOIN tmpermohonan_tmsk F ON A.id = F.tmpermohonan_id
      LEFT JOIN tmsk G ON F.tmsk_id = G.id
      LEFT JOIN tmpermohonan_trperizinan H ON A.id = H.tmpermohonan_id
      LEFT JOIN trperizinan I ON H.trperizinan_id = I.id
      LEFT JOIN trperizinan_trsektor J ON I.id = J.trperizinan_id
      LEFT JOIN trsektor K ON J.trsektor_id = K.id
      LEFT JOIN trkelompok_perizinan_trperizinan L ON I.id = L.trperizinan_id
      LEFT JOIN trkelompok_perizinan M ON L.trkelompok_perizinan_id = M.id
      WHERE ".$val_rekap.$val_state.$val_kat." BETWEEN ? AND ?".$val_jumlah;

    if ($list_state != 0) {
      $result = $this->db->query($sql, array($id, $list_state, $tgla, $tglb))->result();
    } else {
      $result = $this->db->query($sql, array($id, $tgla, $tglb))->result();
      //echo $this->db->last_query();die;
    }
      
    $data['result'] = $result;
    $data['katagori'] = $judul;
    $data['n_menu'] = $val_mohon;
    ...
    $this->template->set_metadata_javascript($js);
    $this->session_info['page_name'] = "List Data Perizinan Berdasar ".(!empty($this->kat_cari[$list_kat]) ? $this->kat_cari[$list_kat] : '');
    $this->template->build('view_list_data_test', $this->session_info);
  }
  ...
  function datatables_viewdata(){
    ...
    $jml_chr = strlen($list_izin);
    $list_izin = substr($list_izin,0,$jml_chr-1);
    $obj = explode('^',$list_izin);
  ...
  public function cetak($tgl_a = null, $tgl_b = null, $ctk_asal = null, $tab = null) { // cetak rekap per sektor ke PDF
    ...
    $judul1  = 'Rekapitulasi Perizinan Berdasarkan '.$this->kat_cari[$list_kat];
    $judul1a = 'Periode '.$this->lib_date->mysql_to_human($tgla).' - '.$this->lib_date->mysql_to_human($tglb);
    ...
    $pdf->Ln(5); $pdf->Cell(0,0.5,$judul1a,0,1,'C');
  ...
  public function cetak_list($id_key = null, $menu = null) { // cetak list per sektor ke PDF
  	$dmrekap_izin = new dmrekap_izin();
    $dmrekap_izin->get_by_id($id_key);
    $list_izin = '';
    switch($menu){
    ...
    $alamat2 = 'Website: ' . $web . '   e-mail: ' . $e_mail;
    $alamat3 = strtoupper($kota) . ' - ' . $kdpos;
    $judul  = 'DATA PERMOHONAN SEKTOR ' . $jdl_bidang;
    ...
    //if ($obj){
      foreach ($list_izin as $row) {
        ...
        $n_perusahaan = $permohonan->tmperusahaan->n_perusahaan;
        if($n_perusahaan=='')
          $n_pemohon = $permohonan->tmpemohon->n_pemohon;
        else
          $n_pemohon = $permohonan->tmpemohon->n_pemohon .' / '. $n_perusahaan;
        ...
        if($no_surat == '') $no_surat = $permohonan->tmsk->no_surat;
        
        $tgl_surat = $permohonan->tmsk->tgl_surat;
        $tgl_ambil1 = $permohonan->tmsk->tgl_ambil1;
        $kontak = $permohonan->tmsk->kontak;
        $tgl_terima = $this->lib_date->mysql_to_human($permohonan->d_terima_berkas);
        $tgl_siap = $this->lib_date->mysql_to_human($permohonan->tgl_siap_serah);
        $tgl_surat = $this->lib_date->mysql_to_human($tgl_surat);
        $tgl_ambil1 = $this->lib_date->mysql_to_human($tgl_ambil1);
                
        $isi = array($no.'.',$permohonan->pendaftaran_id,$tgl_terima,$n_pemohon,$permohonan->a_izin,
  ...
  public function cetak_list_test($tipe_rekap = null, $id = null, $list_state = 0, $list_kat = 0, $tgla = 0, $tglb = 0, $jenis_jumlah = null) { // cetak list per sektor ke PDF
    ...
    if ($tipe_rekap == 0) {
      $sektor = new trsektor();
      $sektor->get_by_id($id);
      $judula = 'DATA PERMOHONAN SEKTOR '.$sektor->n_sektor;

      $val_rekap = "K.id = ? ";
    } else {
      $perizinan = new trperizinan();
      $perizinan->get_by_id($id);
      $judula = 'DATA PERMOHONAN PERIZINAN '.$perizinan->n_perizinan;
    ...
    $sql = "SELECT A.*, C.n_pemohon, E.n_perusahaan, G.no_surat, G.no_surat_edit, G.tgl_ambil1, G.tgl_penetapan, G.tgl_surat, G.tgl_surat_edit, G.kontak,  I.n_perizinan, K.n_sektor, M.id AS id_trkel 
      FROM tmpermohonan A 
      LEFT JOIN tmpemohon_tmpermohonan B ON A.id = B.tmpermohonan_id 
      LEFT JOIN tmpemohon C ON B.tmpemohon_id = C.id 
      LEFT JOIN tmpermohonan_tmperusahaan D ON A.id = D.tmpermohonan_id 
      ...
      LEFT JOIN trkelompok_perizinan_trperizinan L ON I.id = L.trperizinan_id 
      LEFT JOIN trkelompok_perizinan M ON L.trkelompok_perizinan_id = M.id 
      LEFT JOIN tmpermohonan_trstspermohonan N ON A.id = N.tmpermohonan_id
      LEFT JOIN trstspermohonan O ON N.trstspermohonan_id = O.id
      LEFT JOIN tmpemohon_trkelurahan P ON C.id = P.tmpemohon_id
      LEFT JOIN trkelurahan Q ON P.trkelurahan_id = Q.id
      LEFT JOIN trkecamatan_trkelurahan R ON Q.id = R.trkelurahan_id
      LEFT JOIN trkecamatan S ON R.trkecamatan_id = S.id
      LEFT JOIN trkabupaten_trkecamatan T ON S.id = T.trkecamatan_id
      LEFT JOIN trkabupaten U ON T.trkabupaten_id = U.id
      LEFT JOIN trkabupaten_trpropinsi V ON U.id = V.trkabupaten_id
      LEFT JOIN trpropinsi W ON V.trpropinsi_id = W.id
      WHERE ".$val_rekap.$val_state.$val_kat." BETWEEN ? AND ?".$val_jumlah;

      if ($list_state != 0) {
      $list_izin = $this->db->query($sql, array($id, $list_state, $tgla, $tglb))->result();
    } else {
      $list_izin = $this->db->query($sql, array($id, $tgla, $tglb))->result();
    }
    ...
    $alamat2 = 'Website: ' . $web . '   e-mail: ' . $e_mail;
    $alamat3 = strtoupper($kota) . ' - ' . $kdpos;
    $judul1 = $val_mohon . ', PERIODE '. $this->lib_date->mysql_to_human($tgla).' - '.$this->lib_date->mysql_to_human($tglb);
    $judul2 = 'Asal Permohonan : ' . $c_asal;
    ...
    $pdf->SetFont('Arial','',11);
    $pdf->Ln(13); $pdf->Cell(0,0.5,$judula,0,1,'C');
    $pdf->Ln(5); $pdf->Cell(0,0.5,$judul1,0,1,'C');
    $pdf->Ln(5); $pdf->Cell(0,0.5,$judul2,0,1,'C');
    $pdf->SetMargins(1,1);
    ...
    $no = 0;
      foreach ($list_izin as $row) {
        ...
        $n_perusahaan = $row->n_perusahaan;
        if($n_perusahaan=='')
          $n_pemohon = $row->n_pemohon;
        else
          $n_pemohon = $row->n_pemohon .' / '. $n_perusahaan;
        
        $no_surat = $row->no_surat_edit;
        if($no_surat == '') $no_surat = $row->no_surat;
        
        $tgl_surat = $row->tgl_surat;
        $tgl_ambil1 = $row->tgl_ambil1;
        $kontak = $row->kontak;
        $tgl_terima = $this->lib_date->mysql_to_human($row->d_terima_berkas_asli);
        $tgl_siap = $this->lib_date->mysql_to_human($row->tgl_siap_serah);
        $tgl_surat = $this->lib_date->mysql_to_human($tgl_surat);
        $tgl_ambil1 = $this->lib_date->mysql_to_human($tgl_ambil1);
                
        $isi = array($no.'.',$row->pendaftaran_id,$tgl_terima,$n_pemohon,$row->a_izin,
        $no_surat,$tgl_surat,$tgl_siap,$tgl_ambil1,$kontak);
        $align = array('R','L','L','L','L','L','L','L','L','L');
        $space = 5;
        $nb=0;
    ...
    //output file PDF
    $pdf->Output($judula.'.pdf','D');
  }

  public function cetak_list_excel($id_key = null, $menu = null) { // cetak list per izin ke Excel
    $dmrekap_izin = new dmrekap_izin();
    $dmrekap_izin->get_by_id($id_key);
    $list_izin = '';
    switch($menu){
    ...
    foreach ($list_izin as $row){
      ...
      if($permohonan->status_berkas == "proses")
        $n_status = $permohonan->trstspermohonan->n_sts_permohonan;
      else
        $n_status = $permohonan->status_berkas;
      ...
      if($tgl_selesai == '') {
        ...
        $durasi = $this->lib_date->lama_durasi($permohonan->d_terima_berkas, $tgl_selesai);
      }
      
      $i++;
      $isi = "<tr>
                <td>".$i."</td>
                <td>"."'".$permohonan->pendaftaran_id."</td>
                <td>".$permohonan->kd_gerai."</td>
                <td>".$permohonan->d_terima_berkas."</td>
                <td>".substr($permohonan->d_entry,11,8)."</td>
                <td>".$permohonan->d_selesai_proses."</td>
  ...
  // public function cetak_list_excel_test($tipe_rekap = null, $id = null, $list_state = 0, $list_kat = 0, $tgla = 0, $tglb = 0, $jenis_jumlah = null) { // cetak list per izin ke Excel

  //   if ($tipe_rekap == 0) {
  //     $sektor = new trsektor();
  //     $sektor->get_by_id($id);
  //     $jdl_izin = 'SEKTOR '.$sektor->n_sektor;
  //     $judula = 'DATA PERMOHONAN SEKTOR '.$sektor->n_sektor;
  ...
  //     $perizinan = new trperizinan();
  //     $perizinan->get_by_id($id);
  //     $jdl_izin = 'PERIZINAN '.$perizinan->n_perizinan;
  //     $judula = 'DATA PERMOHONAN PERIZINAN '.$perizinan->n_perizinan;
  ...
  //   $sql = "SELECT A.id AS tmpermohonan_id, A.*, C.n_pemohon, C.telp_pemohon, C.a_pemohon, E.n_perusahaan, E.nama_pimpinan, E.a_perusahaan, O.n_kelurahan, S.n_kecamatan, T.n_kabupaten, U.n_propinsi, G.tgl_penetapan, G.tgl_surat, G.tgl_surat_edit, G.no_surat, G.no_surat_edit, I.n_perizinan, I.kd_izin, A.a_izin, A.d_berlaku_izin, A.status_berkas, K.n_sektor, M.id AS id_trkel, O.id as idkel, W.n_sts_permohonan
  //     FROM tmpermohonan A
  //     LEFT JOIN tmpemohon_tmpermohonan B ON A.id = B.tmpermohonan_id
  //     LEFT JOIN tmpemohon C ON B.tmpemohon_id = C.id
  //     LEFT JOIN tmpermohonan_tmperusahaan D ON A.id = D.tmpermohonan_id
  ...
  //     LEFT JOIN trkelompok_perizinan_trperizinan L ON I.id = L.trperizinan_id
  //     LEFT JOIN trkelompok_perizinan M ON L.trkelompok_perizinan_id = M.id
  //     LEFT JOIN tmpemohon_trkelurahan N ON C.id = N.tmpemohon_id
  //     LEFT JOIN trkelurahan O ON N.trkelurahan_id = O.id
  //     LEFT JOIN trkecamatan_trkelurahan P ON O.id = P.trkelurahan_id
  //     LEFT JOIN trkabupaten_trkecamatan Q ON P.trkecamatan_id = Q.trkecamatan_id
  //     LEFT JOIN trkabupaten_trpropinsi R ON Q.trkabupaten_id = R.trkabupaten_id
  //     LEFT JOIN trkecamatan S ON P.trkecamatan_id = S.id
  //     LEFT JOIN trkabupaten T ON Q.trkabupaten_id = T.id
  //     LEFT JOIN trpropinsi U ON R.trpropinsi_id = U.id
  //     LEFT JOIN tmpermohonan_trstspermohonan V ON A.id = V.tmpermohonan_id
  //     LEFT JOIN trstspermohonan W ON V.trstspermohonan_id = W.id
  //     WHERE ".$val_rekap.$val_state.$val_kat." BETWEEN ? AND ?".$val_jumlah;
  ...
  //   header("Content-Type:   application/vnd.ms-excel; charset=utf-8");
  //   header("Content-Disposition: attachment; filename=".$judula.".xls");
  //   header("Expires: 0");
  //   header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
  //   header("Cache-Control: private",false);
  ...
  //       $var = "var_teknis".$i;
  //       if($perizinan->$var <> '' ){
  //         $aktif = $this->lib_date->array_property('11',$perizinan->$var);      // Aktifasi Property
  //         if($aktif == 'Ya') {
  //           $hitproperty++;
  //           $nfield = $this->lib_date->array_property('0',$perizinan->$var);// ambil no Data Field property 
  //           if($hitproperty == 1) {
  //             $no_field = array($nfield);
  //           }else{
  //             $tempArray = array($nfield);
  //             $no_field = array_merge ($no_field, $tempArray);
  //           }
  //           $njdl = $this->lib_date->array_property('1',$perizinan->$var);// ambil judul property 
  //           $jdl = $jdl."<td>".$njdl."</td>";
  //         }
  //       }
  //     }
  ...
  //     $wil_lok = new trkelurahan();
  //     $wil_lok->where('id', $row->idkel)->get();
  //     $n_kelurahan = $wil_lok->n_kelurahan;
  //     $wil_lok->trkecamatan->get();
  //     $wil_lok->trkecamatan->trkabupaten->get();
  //     $wil_lok->trkecamatan->trkabupaten->trpropinsi->get();
  ...
  //               <td>".$row->d_selesai_proses."</td>
  //               <td>".$row->n_pemohon."</td>
  //               <td>".$row->telp_pemohon."</td>
  //               <td>".$row->a_pemohon."</td>
  //               <td>".$row->n_propinsi."</td>
  //               <td>".$row->n_kabupaten."</td>
  //               <td>".$row->n_kecamatan."</td>
  //               <td>".$row->n_kelurahan."</td>
  //               <td>".$row->n_perusahaan."</td>
  //               <td>".$row->nama_pimpinan."</td>
  //               <td>".$row->a_perusahaan."</td>
                
  //               <td>".$row->n_perizinan."</td>
  //               <td>".$row->kd_izin."</td>
  //               <td>".$n_status."</td>
  //               <td>".$tgl_selesai."</td>
  //               <td>".$durasi."</td>
  ...
  // }
    ...
    public function cetak_list_excel_test($tipe_rekap = null, $id = null, $list_state = 0, $list_kat = 0, $tgla = 0, $tglb = 0, $jenis_jumlah = null)
    { // cetak list per izin ke Excel
        ...
        switch ($jenis_jumlah) {
            case 1:
                $val_jumlah = "";
                $val_mohon = "JUMLAH PERMOHONAN";
                $val_kat2 = "";
                break;
            case 2:
                $val_jumlah = " AND E.status_berkas = 'Izin Ditolak FO'";
                $val_mohon = "IZIN DITOLAK FRONT OFFICE";
                ($pilihan == 1 ? $val_kat2 = " AND I.tgl_surat_edit >= ? AND I.tgl_surat_edit <= ?" : $val_kat2 = "");
                break;
            case 3:
                $val_jumlah = " AND E.status_berkas = 'Izin Disetujui'";
                $val_mohon = "JUMLAH IZIN DESETUJUI";
                ($pilihan == 1 ? $val_kat2 = " AND I.tgl_surat_edit >= ? AND I.tgl_surat_edit <= ?" : $val_kat2 = "");
                break;
            case 4:
                $val_jumlah = " AND (E.status_berkas = 'Izin Disetujui' AND E.d_ambil_izin IS NOT NULL)";
                ($pilihan == 1 ? $val_kat2 = " AND I.tgl_surat_edit >= ? AND I.tgl_surat_edit <= ?" : $val_kat2 = "");
                $val_mohon = "IZIN DISETUJUI DIAMBIL";
                break;
            case 5:
                $val_jumlah = " AND (E.status_berkas = 'Izin Disetujui' AND E.d_ambil_izin IS NULL)";
                $val_mohon = "IZIN DISETUJUI BELUM DIAMBIL";
                ($pilihan == 1 ? $val_kat2 = " AND I.tgl_surat_edit >= ? AND I.tgl_surat_edit <= ?" : $val_kat2 = "");
                break;
            case 6:
                $val_jumlah = " AND E.status_berkas = 'Izin Ditolak'";
                $val_mohon = "JUMLAH IZIN DITOLAK";
                ($pilihan == 1 ? $val_kat2 = " AND I.tgl_surat_edit >= ? AND I.tgl_surat_edit <= ?" : $val_kat2 = "");
                break;
            case 7:
                $val_jumlah = ($list_kat != 2 ? " AND E.status_berkas = 'proses'" : "");
                $val_mohon = ($list_kat != 2 ? "JUMLAH DALAM PROSES" : "JUMLAH MELEBIHI PERIODE");
                $val_kat2 = "";
                break;
        }


        $sql = "SELECT DISTINCT E.id AS tmpermohonan_id,X.nib, E.*, K.n_pemohon, K.telp_pemohon, K.a_pemohon, M.n_perusahaan, M.nama_pimpinan, M.a_perusahaan, M.npwp, 
        -- Q.n_kelurahan, 
        -- U.n_kecamatan, 
        -- V.n_kabupaten, 
        -- W.n_propinsi, 
        I.tgl_penetapan, I.tgl_surat, I.tgl_surat_edit, I.no_surat, I.no_surat_edit, C.n_perizinan, E.a_izin, E.d_berlaku_izin, E.status_berkas, A.n_sektor, C.n_perizinan, C.kd_izin, O.id AS id_trkel, 
        -- Q.id as idkel, 
        G.n_sts_permohonan, 
        -- X.trkelurahan_id AS trkel_izin, 
        X.npwpPerusahaan as npwpp, X.emailPerusahaan as mail_perus
    FROM trsektor A 
    LEFT JOIN trperizinan_trsektor B ON A.id = B.trsektor_id 
    LEFT JOIN trperizinan C ON B.trperizinan_id = C.id 
    LEFT JOIN tmpermohonan_trperizinan D ON C.id = D.trperizinan_id 
    ...
    LEFT JOIN tmpemohon_trkelurahan P ON K.id = P.tmpemohon_id
    -- LEFT JOIN trkelurahan Q ON P.trkelurahan_id = Q.id
    -- LEFT JOIN trkecamatan_trkelurahan R ON Q.id = R.trkelurahan_id
    -- LEFT JOIN trkabupaten_trkecamatan S ON R.trkecamatan_id = S.trkecamatan_id
    -- LEFT JOIN trkabupaten_trpropinsi T ON S.trkabupaten_id = T.trkabupaten_id
    -- LEFT JOIN trkecamatan U ON R.trkecamatan_id = U.id
    -- LEFT JOIN trkabupaten V ON S.trkabupaten_id = V.id
    -- LEFT JOIN trpropinsi W ON T.trpropinsi_id = W.id
    LEFT JOIN tmpemohon_portal X ON E.id_pemohon_portal = X.id
    WHERE " . $val_rekap . $val_state . "AND (" . $val_kat . " BETWEEN ? AND ?)" . $val_jumlah . $val_kat2 . " GROUP BY E.pendaftaran_id";

        if ($list_kat == 2) {
            $list_izin = $this->db->query($sql, array($id, $tgla, $tglb, $tgla, $tglb))->result();
        } else {
            $list_izin = $this->db->query($sql, array($id, $tgla, $tglb))->result();
        }

        header("Content-Type:   application/vnd.ms-excel; charset=utf-8");
        header("Content-Disposition: attachment; filename=REKAP_IZIN.xls");
        header("Expires: 0");
        header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
        header("Cache-Control: private", false);
        ...
        foreach ($list_izin as $row) {
            ...
            $wil_iz = new trkelurahan();
            // $wil_iz->where('id', $row->trkel_izin)->get();
            $n_kelizin = $wil_iz->n_kelurahan;
            $wil_iz->trkecamatan->get();
            $wil_iz->trkecamatan->trkabupaten->get();
            $wil_iz->trkecamatan->trkabupaten->trpropinsi->get();
            ...
            if ($list_kat == 2 && $jenis_jumlah == 7) {
                if ($row->tgl_surat_edit >= $tgla && $row->tgl_surat_edit <= $tglb) {
                    $tampil = FALSE;
                }
            }
            ...
            if ($tampil) {
                ...
                $isi = "<tr>
                  ...
                  <td>" . $no_surat . "</td>
                  <td>" . $row->a_izin . "</td>
                  <td>" . $wil_iz->trkecamatan->trkabupaten->trpropinsi->n_propinsi . "</td>
                  <td>" . $wil_iz->trkecamatan->trkabupaten->n_kabupaten . "</td>
                  <td>" . $wil_iz->trkecamatan->n_kecamatan . "</td>
                  <td>" . $n_kelizin . "</td>
                  <td>" . $row->keterangan . "</td>
                  <td>=\"$row->kontak_person\"</td>
                  <td>" . $row->mail_perus . "</td>";
                ...
                if (!empty($no_field)) {
                    ...
                    foreach ($no_field as $cek_syarat) {
                        ...
                        if ($row->$var <> '') {
                            ...
                            $cek_posisi = strpos($nisi, '^');
                            //$cekisi = substr($nisi,$cek_posisi+1,$hitung);
                            $cekisi = substr($nisi, $cek_posisi, $hitung);
                            if ($cekisi == '' || $cekisi == '-') {
                                $nisi = substr($nisi, 0, $cek_posisi);
                                if ($nisi == '' || $nisi == '-') $nisi = '-';
                            } else {
  ...
  function kat_filter(){
    ...
    if($menu <= 7) {           // info per sektor
      if($list_kat === '0'){   // per tanggal Permohonan
        ...
        if($gerai === '0') {   // seluruh data
      	  ...
      	  $sql .= " WHERE t7.id <> 1 AND t1.trsektor_id = '$id_key' AND ( t17.tgl_surat between '$tgla' and '$tglb') ";
    		}else{
          //$sql .= " WHERE t7.id <> 1 AND t1.kd_gerai = '$gerai' AND t1.trsektor_id = '$id_key' 
    		  ...
    		  $sql .= " WHERE t7.id <> 1 AND t1.kd_gerai = '$gerai' AND t1.trsektor_id = '$id_key' 
    		            AND ( t17.tgl_surat between '$tgla' and '$tglb') ";
        }
    	}
