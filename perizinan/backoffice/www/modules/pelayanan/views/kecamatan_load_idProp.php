<script type="text/javascript">
    ...
    $(document).ready(function() {
        $('#'+kecamatan_id+'_id').change(function(){
                ...
                $.post(base_url+'pelayanan/pendaftaran/'+kelurahan_id, {
	...
	foreach ($list_kabupaten as $row){
      ...
      if (empty($row->id))
      {
        ...
        if(empty($opsi_kecamatan)){
            $opsi_kecamatan = array(''=>'-------tidak ada data-------');
        }

        echo form_label('Kecamatan');
        echo form_dropdown($kecamatan_id, $opsi_kecamatan, '',
             'class = "input-select-wrc notSelect" id="'.$kecamatan_id.'_id"');
    }
    ?>
</div>