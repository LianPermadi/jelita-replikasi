...
<div id="content">
  <div class="post">
    ...
    <div class="entry">
      ...
      echo '<b>Terdapat '.number_format($jml_History,0).' (History) Permohonan lebih 1 Tahun Dibekukan'."</b><br>";       
      echo $psn;
      ?>
     </div>
          ...
          while ($rows = mysql_fetch_assoc(@$results)){
            ...
            if($this->session->userdata('username') == 'lucky')      $valid = TRUE;
              ...
              <tr>
                ...
                <td valign='top'><?php echo $b. $rows['referensi'].'<br>'.$this->lib_date->mysql_to_human($rows['tglPermohonan']).'<br>'.$rows['id'].$be; ?></td>
                <td valign='top'>
                  <?php
                  if($valid)
                    if($Xusr != 0){
                ...
                <td valign='top'>
                  ...
                  $img_edit = array('src' => base_url() . 'assets/images/icon/property.png',
          		  	...
          		  	$confirm_text = 'Aktifkan kembali permohonan?';
                  $confirm_text2 = 'Hapus permohonan?';

                   $img_delete2 = array('src' => base_url() . 'assets/images/icon/cross.png',
                                      'alt' => 'Delete',
                                      'title' => 'Delete',
                                      'border' => '0',
                  ...
                  $img_delete = array('src' => base_url() . 'assets/images/icon/alert-icon.png',
                                      'alt' => 'Aktifkan Kembali Permohonan',
                                      'title' => 'Aktifkan Kembali Permohonan',
                                      'border' => '0',
                    ...
                    if(!$this->cs){
                      echo anchor(site_url('pelayanan/sementara/kembalikan') . '/' . $rows['id'], img($img_delete)) . "&nbsp;";
                      //if($aut_hapus)
                      echo anchor(site_url('pelayanan/sementara/delete') . '/' . $rows['id'], img($img_delete2)) . "&nbsp;";
                    }
                  ?>
          		  </td>
              </tr>
