<script type="text/javascript">
    $(document).ready(function() {
        $('#kecamatan_lok_id').change(function(){
            $.post('<?php echo  base_url() ;?>pelayanan/pendaftaran/kelurahan_lok', { kecamatan_id: $('#kecamatan_lok_id').val() },
                    function(data) {
                        $('#show_kelurahan_lok').html(data);
                    });
            });
    });
</script>
...
<div class="contentForm">
    ...
    if($_REQUEST){
        ...
        if(!empty($row)){
            echo form_dropdown("kecamatan_lok", $opsi_kecamatan, '0','class = "input-select-wrc notSelect" id="kecamatan_lok_id"');
        }else{ 
