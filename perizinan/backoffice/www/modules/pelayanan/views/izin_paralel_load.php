...
<div class="contentForm" id="show_jenis_izin">
    ...
    if($_REQUEST['jenis_paralel_id'] == "no"){
        ...
        echo form_dropdown('jenis_paralel', $opsi_izin, '','class = "input-select-wrc" id="list_paralel_id"');
    ?>
<div class="contentForm bg-grid" id="show_list_izin">
    <?php
        $paralel_jenis = new trparalel();
        $paralel_jenis->select_min('id')->get();
        $list_izin_paralel = $this->perizinan->where_related($paralel_jenis)->get();
        ...
        echo form_dropdown('list_izin_paralel[]', $opsi_list_izin, '','class = "input-select-wrc" id="list_izin_paralel" multiple="multiple"');
    ?>
</div>
    <?php
    }
