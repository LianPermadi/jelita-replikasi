                            ...
                            <td nowrap="nowrap">
                              ...
                              //      'onClick' => 'return confirm_link(\''.$confirm_text.'\')',
                                ...
                                $img_aktif3 = array(
                                    ...
                                    'alt' => 'SMS & e-mail Terkirim',
                                    'title' => 'SMS & e-mail Terkirim',
                                    'border' => '0'
                                );

                                if($s_serah == 'TIDAK') {
                                    echo anchor(site_url('pelayanan/ambilsk/siap_diserahkan') .'/'. $rows['id'].'/'.$i_ok, img($img_aktif1))."&nbsp;";
								} else {
                                    if($status_bap == 2 || $rows['c_status_bayar'] == 1 || $idkelompok !== '4') {
//                                        echo anchor(site_url('pelayanan/ambilsk/diambil') .'/'. $rows['id'], img($img_aktif2))."&nbsp;";
...
//										echo anchor(site_url('rekapitulasi/DetailTahun').'/'. $data['id'] .'/'. $tgla.'/'. $tglb , $f_jumlah, 'class="link2-wrc" rel="rekapitulasi_box"');
										...
										$pesan = "BPMPT: Izin dg no pendaftaran: " . $rows['pendaftaran_id'];
										if($i_ok=='T')
										...
										echo "<a>" . img($img_aktif3) . "</a>";
//										echo anchor(site_url('pelayanan/ambilsk/sendSMSGateway()') .'/'. $rows['id'].'/'.$i_ok, img($img_aktif3));
...
<!-- Create PBS -->
        ...
        <div class="post">
			...
			<h3><a class="page-help" href="<?php echo site_url('monitoring/cetak_penyerahan_izin/1/'.$tgla.'/'.$tglb)?>">
                   ...
                   if ($lokasi_user === 'BPMPT Prov. Jabar' || $lokasi_user === 'BPPT Prov. Jabar') {
