...
<form id="smsdialog" action="<?php echo base_url(); ?>pelayanan/tandatangan/sendSMSGateway" method="post" style="display:none">
...
<!--        <div class="entry">
            <fieldset id="half">
                ...
                <?php echo form_open('pelayanan/tandatangan');?>
                <div id="statusRail">
                    <div id="leftRail">
                        <?php
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            ...
                            <td valign='top'><?php echo $rows['n_pemohon'] .'<br>'. $n_perusahaan; ?></td>
                            <td valign='top'><?php echo $rows['n_perizinan'];?></td>
                            <td valign='top'>
                                <?php 
						        if($rows['keterangan'] == '')
						            echo $rows['a_izin'];
                            ...
                            <td nowrap="nowrap">
                              ...
                              //      'onClick' => 'return confirm_link(\''.$confirm_text.'\')',
                                ...
                                if($s_serah == 'TIDAK') {
//                                    echo anchor(site_url('pelayanan/tandatangan/siap_diserahkan') .'/1/'. $rows['id'].'/'.$i_ok, img($img_aktif1))."&nbsp;"; // 1 = pertama kali
								} else {
                                    if($status_bap == 2 || $rows['c_status_bayar'] == 1 || $idkelompok !== '4') {
//										echo anchor(site_url('pelayanan/tandatangan/input_data').'/'.$rows['id'].'/'.$rows['idizin'],img($img_aktif2), 'class="link2-wrc"');
                                        //echo "<a class ='kirimsms' onClick='isino(\"".$rows['telp_pemohon']."\",\"Surat ".$rows['n_perizinan']." dengan no pendaftaran : ".$rows['pendaftaran_id']." sudah selesai. Mohon segera diambil.\");return false;' href='".base_url()."pelayanan/tandatangan/sendSMSGateway/".$rows['telp_pemohon']."/Surat ".$rows['n_perizinan']." dengan no pendaftaran : ".$rows['pendaftaran_id']." sudah selesai. Mohon segera diambil.'>".img($img_aktif3)."</a>";
...
//										echo anchor(site_url('pelayanan/tandatangan/siap_diserahkan') .'/2/'. $rows['id'].'/'.$i_ok, img($img_aktif3)); // 2 info berikut
