<?php
/**
 * Description of Administrasi Penandatangan Naskah Izin
 * @author PBS Created : 14 September 2016
 */
class tandatangan extends WRC_AdminCont {
    ...
    public function index() {  // Penandatanganan Izin pertama masuk saat klik menu izin
		...
		//if ($lokasi_user === 'DPMPTSP Prov. tasikmalaya' || $lokasi_user === 'BPMPT Prov. tasikmalaya' || $lokasi_user === 'BPPT Prov. tasikmalaya') {
            $query = "SELECT  A.id, A.pendaftaran_id, A.c_status_bayar, A.d_terima_berkas, A.a_izin, A.keterangan,
					  ...
					  AND K.c_cetak != 0
					  AND I.status_bap = 1" . $query_filter."order by A.id DESC";
		} else {
            $query = "SELECT  A.id, A.pendaftaran_id, A.c_status_bayar, A.d_terima_berkas, A.a_izin, A.keterangan,
                     A.d_perubahan, A.d_perpanjangan, A.d_daftarulang, A.siap_serah, A.status_berkas,
                     C.id idizin, C.n_perizinan, E.n_pemohon,
        ...
        $this->session_info['page_name'] = "Administrasi Penandatangan Izin";
        $this->template->build('tandatangan_list', $this->session_info);
    }

	public function list_index() { // Kembali dari proses Administrasi Penandatangan Izin
        $kd_filter = $this->input->post('kd_filter');
		$no_daftar = $this->input->post('kt_cari');
		$username = new user();
        $username->where('username', $this->session->userdata('username'))->get();
    ...
    public function sendSMSGateway() {
		...
		//redirect('pelayanan/tandatangan');
    }
	...
	public function kirim_ulang_sms($n_hp = NULL, $n_email = NULL, $i_ok = NULL) {
        ...
        $n_judul = 'Status Perizinan';
        if($i_ok=='T')
    	    $n_pesan = 'BPMPT: Izin dg no daftar: '.$permohonan->pendaftaran_id.' DISETUJUI, dan bisa di ambil di tempat anda mendaftar '.$stat;
		else
		    $n_pesan = 'BPMPT: Izin dg no daftar: '.$permohonan->pendaftaran_id.' DITOLAK, mohon utk mengambil berkas permohonan di tempat anda mendaftar '.$stat;
        ...
        if($this->settings->status == 1){
   		...
   		// EOF() Kirim ke media SMS  via gammu dan e-mail via PHPMailer()
		redirect('pelayanan/tandatangan/list_index');
    }
    ...
    public function siap_diserahkan($sts_info=NULL, $id_daftar=NULL, $i_ok=NULL) { // Untuk Siap diserahkan dan notifikasi ulang ke pemohon
        ...
        if (!$update) {
    		...
    		if($id_portal > 0){ //untuk pendaftaran OnLine
				...
				$stat = '<Tunjukkan SMS ini>'; //max 35 dgt
		    } else {
            ...
            //'BPMPT: Izin dg no daftar: 0000000000000000000 DISETUJUI, dan bisa di ambil di tempat anda mendaftar 00000000000000000000000000000000000';
            $n_judul = 'Status Perizinan';
			if($i_ok=='T')
	    	    $n_pesan = 'BPMPT: Izin dg no daftar: '.$permohonan->pendaftaran_id.' DISETUJUI, dan bisa di ambil di tempat anda mendaftar '.$stat;
			else
            ...
            if($this->settings->status == 1){
				...
				if($jm == 2){
				    $gammu = $this->load->database('gammu', TRUE);                          // the TRUE paramater tells CI that you'd like to return the database object.
	    		    $data = array('DestinationNumber' => $n_hp2,'TextDecoded' => $n_pesan); // 'CreatorID' => "Gammu",
	                $gammu->insert('outbox',$data);
				}
				// EOF() Kirim SMS
...
//        $data['data_th'] = $tahun->get();
        $this->load->vars($data);
        $this->load->view('view_tandatangan', $data);
    }
...
//		$data['tglambil'] = $sk->tgl_ambil1;
    ...
    public function diambil_old($id_daftar = NULL, $tgla = NULL, $tglb = NULL) {    // diganti dengan func diserahkan
        ...
        if (!$update) {
            ...
            $g = $this->sql($u_ser);
            //$p = $this->db->query("call log ('Penyerahan Izin','Penyerahan izin ".$permohonan->pendaftaran_id."','".$tgl."','".$u_ser."')");
//            redirect('pelayanan/tandatangan/list_index' . '/' . $tgla . '/' . $tglb);
			redirect('pelayanan/tandatangan/list_index');
        }
    }
