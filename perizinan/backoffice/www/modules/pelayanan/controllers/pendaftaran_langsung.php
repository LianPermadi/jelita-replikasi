 ...
 * @author Obi
 */
class Pendaftaran_langsung extends MY_Controller {
...
//PBS        $uploads = $_FILES['file_upload']['name'];
//PBS        if (!empty($uploads)) {
	    $BypassPBS = '1';
	    if ($BypassPBS == '1') {
...
//PBS            $field = 'file_upload';
//PBS            if (!$this->upload->do_upload($field)) {
            if ($BypassPBS == '2') {
                $x = array('<p>', '</p>');
...
//PBS                $uploaded = $this->upload->data();
//PBS                $file = $uploaded['file_name'];
...
//PBS                $data2['lampiran'] = base_url() . 'uploads/lampiran/' . $file;
                $data2['lampiran'] = base_url() . 'uploads/lampiran/Logo tasikmalaya.Jpg';
