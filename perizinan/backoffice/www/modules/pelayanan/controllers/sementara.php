...
class Sementara extends WRC_AdminCont {
  ...
  public function __construct() {
    ...
    $this->All = FALSE;
    $this->hapus = FALSE;
    $this->pendaftaran_online = FALSE;
    $this->cs = FALSE;
    $list_auths = $this->session_info['app_list_auth'];
    
    foreach ($list_auths as $list_auth) {
      if($list_auth->id_role === '22') {  // pendaftaran Online
        $this->pendaftaran_online = TRUE;
        $enabled = TRUE;
      }
  ...
  public function index() { 
    ...
    //start update status hidden Nirwan
    $this->db->where('hidden', '0');
    $this->db->where('tampil', '1');
    $this->db->order_by('tglPermohonan', 'ASC');
    $query    = $this->db->get('tmpemohon_portal');
    ...
    $datmohon = $query->result();

    if ($datmohon) {
      foreach ($datmohon as $row) {
        $tglasli  = strtotime($row->tglPermohonan);
        $tglakhir = strtotime($row->tglPermohonan.' +15 weekdays');
        ...
        if ($today > $tglakhir) {
          ...
          $id = $row->id;
          $this->db->where('id', $id);
          $updt = $this->db->update('tmpemohon_portal', $data); 
          if ($updt) {
            //Start Kirim e-mail
            $email = $row->emailPerusahaan;
            $base_url         = '../assets/pendaftaran/';
            $host             = "smtp.gmail.com";
            $emailpengirim    = "ptsptasikmalaya@gmail.com"; //sebelumnya dpmptsptasikmalaya@gmail.com
            ...
            $mailer->Subject = 'Informasi Permohonan Perizinan anda di Dinas PMPTSP Tasikmalaya';
            ...
            $isi  = "<h2>Informasi Permohonan Perizinan anda di Dinas DPMPTSP Tasikmalaya</h2><hr>";
            $isi .= "<p>Pengajuan Perizinan : <br>".$row->isi_izin."<br></p><p> Atas Nama : <br>".(!empty($row->namaPerusahaan) ? $row->namaPerusahaan : $row->namaPemohon)."<br></p><p>Yang sedang dalam masa verifikasi, dinyatakan tidak memenuhi persyaratan perizinan dikarenakan proses verifikasi persyaratan perizinan tidak aktif selama lebih dari 15 hari, silahkan login untuk melihat informasi lebih lanjut. <a href='https://spekta.tasikmalayakab.go.id/spekta/main/login'>Login Disini</a></p>";
            $isi .= "<p>Jika link tidak bisa di klik, silahkan copy paste link berikut untuk menuju halaman login : https://spekta.tasikmalayakab.go.id/spekta/main/login</p>";
            $isi .= "<p>Terima kasih atas perhatiannya.<br>- Dinas PMPTSP Tasikmalaya</p>";
            $isi .= "<p><small>Harap abaikan email ini jika sudah mendapatkan nomor pendaftaran/resi.</small></p>";
            // $isi .= "<br><hr>";
    ...
    //end update status hidden Nirwan
    
    
    $username = new user();
    $username->where('username', $this->session->userdata('username'))->get();
    $backoffice     = $this->load->database('default', TRUE);
    $portal         = $this->load->database('otherdb', TRUE);
    $db_portal      = $portal->database;
    $db_backoffice  = $backoffice->database;
    if($this->All){
      $query = "SELECT A.*, 
                (SELECT COUNT(*) FROM $db_portal.asistensi B WHERE B.id_permohonan = A.id_permohonan_portal) as CA
                FROM $db_backoffice.tmpemohon_portal A
                WHERE A.tampil = 1 AND (A.hidden = 0 OR A.hidden = 2) order by CA ASC, A.revisi DESC, A.id DESC";
    }else{
      $query = "SELECT A.*, 
                (SELECT COUNT(*) FROM $db_portal.asistensi Z WHERE Z.id_permohonan = A.id_permohonan_portal) as CA 
                FROM $db_backoffice.tmpemohon_portal A
                INNER JOIN trperizinan as B ON A.izin = B.id
                INNER JOIN trperizinan_user AS C ON C.trperizinan_id = B.id
                WHERE A.tampil = 1 AND (A.hidden = 0 OR A.hidden = 2)
                AND C.user_id = '".$username->id."'"."order by CA ASC, A.revisi DESC, A.id DESC";
    }
    $data['list'] = $query;
  ...
  public function listmelebihi() { 
    ...
    if($this->All){
      $query = "SELECT A.*, 
                (SELECT COUNT(*) FROM $db_portal.asistensi B WHERE B.id_permohonan = A.id_permohonan_portal) as CA
                FROM $db_backoffice.tmpemohon_portal A
                WHERE A.tampil = 1 AND A.hidden = 1 order by CA ASC, A.revisi DESC, A.id DESC";
    }else{
      $query = "SELECT A.*, 
                (SELECT COUNT(*) FROM $db_portal.asistensi Z WHERE Z.id_permohonan = A.id_permohonan_portal) as CA 
                FROM $db_backoffice.tmpemohon_portal A
                INNER JOIN trperizinan as B ON A.izin = B.id
                INNER JOIN trperizinan_user AS C ON C.trperizinan_id = B.id
                WHERE A.tampil = 1 AND A.hidden = 1
                AND C.user_id = '".$username->id."'"."order by CA ASC, A.revisi DESC, A.id DESC";
    }
    ...
    $this->template->set_metadata_javascript($js);
    $this->session_info['page_name'] = "Data Permohonan OnLine Melebihi Masa Verifikasi";
    $this->template->build('sementara_list_melebihi', $this->session_info);
  }

  public function kembalikan($uid = NULL) {
          $data = array('hidden' => '2');
          $id = $uid;
          $this->db->where('id', $id);
          $updt = $this->db->update('tmpemohon_portal', $data);
          ...
          if ($updt) {
            $this->session->set_flashdata('sukses', "Berhasil mengembalikan permohonan.");
            redirect('/pelayanan/sementara/listmelebihi');
          } else {
            $this->session->set_flashdata('gagal', "Gagal mengembalikan permohonan.");
            redirect('/pelayanan/sementara/listmelebihi');
          }
  }
...
//         echo $query2;
  ...
  public function edit($id_daftar=NULL) {  // edit pada proses permohonan sementara
    ...
    $data_kel_izin  = new trkelurahan();
    $data_kelurahan = $data_kel_izin->get_by_id($u_daftar->trkelurahan_id);
    $n_kab_izin       = $data_kelurahan->trkecamatan->trkabupaten->n_kabupaten;
    $n_kec_izin       = $data_kelurahan->trkecamatan->n_kecamatan;
    $n_kel_izin       = $data_kelurahan->n_kelurahan;

    $data['n_kab_izin'] = $n_kab_izin;
    $data['n_kec_izin'] = $n_kec_izin;
    $data['n_kel_izin'] = $n_kel_izin;
    ...
    $data['kel_id'] = $u_daftar->trkelurahan_id;
  
    $data['tgl_daftar'] = date("Y-m-d");
    $data['tgl_daftar_ol'] = $u_daftar->tglPermohonan;
    $data['tgl_survey'] = "";
    ...
    $data['indeks_izin'] = $data_izin->indeks;
    if ($data_izin->integrasi != 0) {
      $sqli = "SELECT * FROM integrasi WHERE id_tmpemohon_portal = ?";
      $hasili = $this->db->query($sqli, array($id_daftar));
      $resulti = $hasili->result();

      $data['integrasi'] = $resulti;
    }

    $data['lat'] = (!empty($u_daftar->lat) ? $u_daftar->lat : "");
    $data['lon'] = (!empty($u_daftar->lon) ? $u_daftar->lon : "");
    ...
    $data['syarat_izin'] = $syarat_perizinan->where_related($data_izin)->order_by("(urutan * -1) DESC, status ASC, urutan = 0, urutan")->get();
    ...
    $sql  = "select tm_pemohon.username from tm_pemohon where tm_pemohon.id='".$u_daftar->id_pemohon."'";
  ...
  public function save() {   // Simpan Pendaftaran Baru 
    ...
    $permohonan->lon = $this->input->post('lon');
    $permohonan->lat = $this->input->post('lat');
    ...
    if($this->input->post('id_pemohon')) $pemohon->get_by_id($this->input->post('id_pemohon'));
    ...
    if($this->input->post('nama_perusahaan')) {
      ...
      if ($this->input->post('id_perusahaan')) $perusahaan->get_by_id($this->input->post('id_perusahaan'));
    ...
    //Kembalikan status editable jika ada editable
    $data = array('editable' => '0');
    $id_permohonan_portal = htmlspecialchars($_POST['id_permohonan_portal'],ENT_QUOTES);
    $otherdb->where('id', $id_permohonan_portal);
    $otherdb->update('tmpermohonan_portal', $data);
    //End kembalikan status
    
    //Kirim SMS ke Pemohon
    //End Kirim SMS ke Pemohon
    
    // update status tampil tmpemohon_portal
    $data = array("tampil"  => "0", "revisi" => "0");
    $this->db->where('id', htmlspecialchars($_POST['id_daftar_ol'],ENT_QUOTES));
    if($this->db->update('tmpemohon_portal', $data)) {
      if(!empty($no_pendaftaran)) {
        $this->session->set_flashdata('sukses', "Berhasil Memproses Data Dengan Nomor Pendaftaran : ".$no_pendaftaran);
      }else{
        $this->session->set_flashdata('gagal', "Gagal Memproses Data.");
      }
      redirect('pelayanan/sementara');
    }else{
      echo "Error";
  ...
  public function delete($uid = NULL) {
    ...
    public function edit_OLD($id_daftar = NULL, $id_link = NULL) {  // edit pada proses permohonan sementara
        $u_daftar = $this->pemohon_portal->get_by_id($id_daftar);
    ...
    public function update() {
        ...
        $permohonan->d_survey = $this->input->post('tgl_survey');
        $permohonan->a_izin = $this->input->post('lokasi_izin');
        $permohonan->trkelurahan_id = $this->input->post('trkelurahan_id');
...
//            }
	...
	public function asistensi(){
		...
		redirect('/pelayanan/sementara/edit/'.$id_kembali);
	}
    ...
    if ($this->db->update('tmpemohon_portal', $datarevisi) && $otherdb->update('tmpermohonan_portal', $data)) {
      ...
      $email = $user->emailPerusahaan;
          $base_url         = '../assets/pendaftaran/';
          $host             = "smtp.gmail.com";
          $emailpengirim    = "ptsptasikmalayakab@gmail.com"; //sebelumnya dpmptsptasikmalaya@gmail.com
          $namapengirim     = "DPMPTSP TASIKMALAYA";
          $password         = "";
          $targetpengiriman = $email;
          require("".$base_url."back/plugins/phpmailer/class.phpmailer.php");
          ...
          // $isi  = "<img src = 'https://spekta.tasikmalayakab.go.id/spekta/assets/2016/images/logo_bpmpt.png' style = 'max-width:35%; max-height:35%; line-height: 100%; outline: none; text-decoration: none; border: 0 none;'><br>";
          $isi  = "<h2>Informasi Permohonan Perizinan anda di Dinas DPMPTSP Tasikmalaya</h2><hr>";
          $isi .= "<p>Pengajuan Perizinan : <br>".$perizinan->n_perizinan."<br></p><p> Atas Nama : <br>".$user->namaPerusahaan."<br></p><p>Membutuhkan revisi persyaratan, silahkan buka akun anda untuk melakukan revisi. <a href='https://spekta.tasikmalayakab.go.id/spekta/main/login'>Login Disini</a></p>";
          $isi .= "<p>Jika link tidak bisa di klik, silahkan copy paste link berikut untuk menuju halaman login : https://spekta.tasikmalayakab.go.id/spekta/main/login</p>";
          $isi .= "<p>Terima kasih atas perhatiannya.<br>- Dinas PMPTSP Tasikmalaya</p>";
          $isi .= "<br><hr>";
          // $isi .= "<p><small>&copy; Dinas Penanaman Modal dan Pelayanan Terpadu Satu Pintu Kabupaten Tasikmalaya - ".date('Y')."<br>Jalan Windu Nomor 26<br>Tasikmalaya, Tasikmalaya, Indonesia. 40263.</small></p>";
  ...
  public function gis($lat, $lon) {    // Ambil data pemohon izin
        $data['lat'] = $lat;
        $data['lon'] = $lon;
        $data['page_name'] = "Titik Lokasi Izin";

        $this->load->vars($data);
        $this->load->view('pick_list_gis', $data);
    }
}