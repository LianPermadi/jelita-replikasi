 ...
 * Catatan : Untuk Ke Server Aktifkan Fuction cek_status_npwp() di baris paling bawah
 */
...
class Pendaftaran extends WRC_AdminCont {
    ...
    public function index() {  // Permohonan Izin Baru (pertama masuk saat click menu Pendaftaran-Permohonan Izin Baru)
        ...
        $this->session_info['page_name'] = "Permohonan Izin";
    ...
    public function list_index($id_persyaratan = NULL) {
        ...
        $this->template->set_metadata_javascript($js);
        $this->session_info['page_name'] = "Data Permohonan Izin";
        $this->template->build('pendaftaran_list', $this->session_info);
    }
    ...
    public function create() {    // Masuk saat click Plus atau pendaftaran permohonan baru
	    $katagori = $this->input->post('status');
        $this->username->where('username', $this->session->userdata('username'))->get();
		$lokasi_user = $this->session->userdata('lokasi');
		...
		switch ($katagori) {
      case 0:  $kat = 'Izin Baru'; break;
			case 1:  $kat = 'Daftar Ulang Izin'; break;
			case 2:  $kat = 'Perpanjangan Izin'; break;
			case 3:  $kat = 'Perubahan Izin'; break;
			case 4:  $kat = 'Pencabutan Izin'; break;
			case 5:  $kat = 'Penutupan Izin'; break;
        }
        $this->template->set_metadata_javascript($js);
        $this->load->vars($data);
        $this->session_info['page_name'] = "Entry Data Permohonan ".$kat;
    ...
    public function create2($id_jenis) {
        ...
        $this->template->set_metadata_javascript($js);
        $this->load->vars($data);
        $this->session_info['page_name'] = "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX ".$kat;//"Entry Data Permohonan Izin Baru";
        $this->template->build('pendaftaran_edit', $this->session_info);
    }
...
//        $statusOnline2 = $this->settings->status;
        ...
        $this->load->vars($data);
        if ($id_link == "1") {
            $this->session_info['page_name'] = "Entry data edit Permohonan ".$kat;
        } else {
            $this->session_info['page_name'] = "Edit Permohonan ".$kat;
        }
        $this->template->build('pendaftaran_edit', $this->session_info);
    }
...
//        $statusOnline2 = $this->settings->status;
        ...
        $this->template->set_metadata_javascript($js);
        $this->load->vars($data);
        $this->session_info['page_name'] = "Pengembalian Berkas Permohonan ".$kat;
        $this->template->build('pendaftaran_edit', $this->session_info);
    }
...
//        $this->form_validation->set_rules('jenis_investasi','j_investasi', 'required');
		...
		if($data_tahun == 0){
			echo 'SESI BERAKHIR !!!, PENYIMPANAN GAGAL '; echo anchor(site_url('pelayanan/pendaftaran/'), ' Kembali '); die;
		}
        //Per Tahun Auto Restart NoUrut
        if ($data_id->d_tahun === $data_tahun) {           // jika tidak ganti tahun untuk suatu permohonan
            $data_urut = $data_id->i_urut + 1;
		    $year = new year();                            // PBS
			$year->where('tahun', $data_tahun);            // PBS
			$year->update('no_urut', $data_urut);          // PBS
        } else {                                           // jika ganti tahun untuk suatu permohonan 
			$year = new year();                            // PBS
...
//      $jam = date("H:i:s A");
  ...
  public function cetak_bukti($id_daftar = NULL) {
    ...
    //$link = 'http://spekta.tasikmalayakab.go.id/spekta/main/ceksts/index/';
    $link = 'https://spekta.tasikmalayakab.go.id/spekta/main/ceksts/index/';
    $codeContents = 'ID.'.$permohonan->pendaftaran_id.'_Nama:_'.$nama.'_Tgl Daftar:_'.$tgl_daftar;
    $codeContentskey = 'Gerai: '.$tampil_gerai.' TglCtk: '.$tgl_skr.' User: '.$user;
    $codeContents = str_replace(' ', '_',$codeContents);
    $key = md5($codeContents.' '.$codeContentskey);
    $codeContents = $link.$codeContents.'_idkey.'.$key;
    ...
    if($logo->value!=="") {
        $odf->setImage('logo', 'uploads/logo/' . $logo->value, '1.5', '2');
    }else {
        $odf->setVars('logo', ' ');
    } 
        
    $sektor = $permohonan->trsektor_id;
      $odf->setImage('bebas_biaya', 'uploads/logo/BebasBiaya.png', '3.5', '1.2');
    //if($jenis_izin->c_tarif == 1){  // jika bertarif
    ...
    //$odf->setImage('barcode', 'assets/barcode/' . $id_daftar . '.png', '5.0', '1.0');
      $odf->setImage('barcode', $pngAbsoluteFilePath, '2.5', '2.5');       //qrcode
    ...
    $lokasi = ltrim($lokasi,'; ');
    $listeArticles = array(array('property' => 'Nomor', 'content' => $permohonan->pendaftaran_id,),
...
//------------------------------EOF() SCRIPT MANTRA------------------------------//
    ...
    public function cek_status_npwp() {
        $npwp = $_POST['npwp'];
        $returnstat = "TV";
        $nonpwp = "";
        $namanpwp = "";
        ...
        $url = 'http://ex-1.pajak.go.id/djpws/kswp/'.$izin.'/'.$npwp.'/BajwLbeQnjbtPNhDOgmVvMlpwwdLAW';

        $result = file_get_contents($url);
        $json_obj = json_decode($result);
        if ($json_obj || !empty($json_obj)) {
          $status = $json_obj->status;
          $nonpwp = $json_obj->NPWP;
          $namanpwp = $json_obj->nama;
          if ($status == "VALID") {
            $returnstat = "V";
          } else {
            $returnstat = "TV";
        ...
        $returnArr = [$returnstat, $nonpwp, $namanpwp, $status];     // aktifkan jika ingin kirim ke server
