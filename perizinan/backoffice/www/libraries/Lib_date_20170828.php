...
// Create PBS
    ...
    public function hit_durasi($tgl_entry, $vdurasi) {  // mengambil hari terakhir selama vdurasi hari dari v_hari
		...
		while ($i == 0){
		   ...
		   If ($vdurasi == $vdurasi_cek-$hari_libur){
           	   // proses selesai
			   $i = 1;
		   } else {
			   // masih proses
...
// EOF PBS
	...
	public function data_property($v_id = NULL, $pil = NULL) { // mengambil data properti dalam satu record {v_id=no_id_izin, pil = pilihan output}
		...
		switch ($pil) {
            ...
            case 2: $kirim = $n_var; break;  // ambil data
...
//		}
	...
	public function isi_property($v_id = NULL, $var = NULL, $pil = NULL) { // mengambil isi properti per field {v_id=no_id_permohonan, var=nomor field, pil = pilihan output}
		...
		switch ($pil) {
            ...
            case 2: $kirim = $nama; break;                                                 // ambil nama variabel
			case 3: $kirim = substr($nilai,0,strpos($nilai,'^')); break;                   // ambil nilai variabel awal
			case 4: $kirim = substr($nilai,strpos($nilai,'^')+1,strlen($nilai)); break;    // ambil nilai variabel akhir
			case 5: $kirim = substr($nilai,strpos($nilai,'^')+1,strlen($nilai));           // ambil nilai variabel dg cek akhir atau awal mana yg terisi
			        if($kirim == '-' || $kirim == '') $kirim = substr($nilai,0,strpos($nilai,'^'));
			        break;
        }
		return $kirim;
    }
