...
class Template
{
    ...
    public function set_metadata($name, $content, $type = 'meta')
    {
        ...
        switch($type)
        {
        	...
        	break;
                ...
                case 'stylesheet':
                        $this->_metadata[$content] = '<link rel="'.$type.'" type="type/css" href="'.$content.'" />';
        }
     ...
     * @param     string        $type           type of the rel
     * @return    void
     * @author    Dichi Al Faridi
     */
    public function set_style($type, $url) {
        $url = htmlspecialchars(strip_tags($url));

        if ($type === 'css') {
            $url = site_url('assets/css/'.$url);
            $this->_metadata[$url] = '<link rel="stylesheet" type="text/css" href="'.$url.'" />';
        } else if ($type === 'js') {
            $url = site_url('assets/js/'.$url);
            $this->_metadata[$url] = '<script type="text/javascript" src="'.$url.'"></script>';
        }

        return $this;
    }

    public function set_metadata_javascript($string = NULL) {
        $metadata_url = '<script type="text/javascript" charset="utf-8">';
        $metadata_url = $metadata_url . $string;
        $metadata_url = $metadata_url . '</script>';

        $this->_metadata['js'] = $metadata_url;
        return $this;
    }
