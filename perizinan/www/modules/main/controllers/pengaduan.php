...
//pb        }
    ...
    function index() {
        ...
        $base_url_websevices = $_SERVER['HTTP_X_FORWARDED_PROTO'].'://'.$_SERVER['HTTP_HOST'].'/backoffice';
        ...
        foreach ($dt_status as $item_stat) {
			...
			$durasi = $this->xml_parsing_win->value_in('durasi', $item_stat);
			$satuan = $this->xml_parsing_win->value_in('satuan', $item_stat);
            // $durasi = '15';
			// $satuan = 'hari';
...
//		redirect('main/pengaduan/get_dt_pengaduan');   // kalo mau cek langsung tampilan
    ...
    function save() {
        ...
        $token = base64_encode("h1x7HrZ4eG");
        ...
        if($responseKeys["success"]) { //Jika captcha berhasil
            ...
            $this->session->set_flashdata('error', "Captha Error, Robot Terdeteksi! Silahkan tunggu beberapa saat sebelum mengisi form kembali.");
            redirect('main/pengaduan');
        }
    }
