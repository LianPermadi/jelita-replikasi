...
class User extends MY_Controller {
  ...
  function __construct() {
    ...
    $backoffice = $this->load->database('otherdb', TRUE);
    // if($detect->isMobile()) {
  ...
  function index() {
    ...
    if($username == 'wjis' || $username == 'wjis1' || $username == 'wjis2' || $username == 'wjis3' || $username == 'wjis4'){
    	?>
    	<form action="<?php echo base_url() . 'main/login/logout_wjis'; ?>" method="post"><?php
    	  echo '<strong style="color: blue; font-size: 24px;">Anda Berhasil Login Event WJIS 2024<br>OPEN SESSION</strong>';
    	  ?>
    	  <br><button type="submit" class="btn btn-primary pull-right">End Session<i class='fa fa-arrow-circle-right'></i></button>
    	  <?php
    	  die();
    	  ?>
    	</form>
    ...
    */
    ...
    if(count($permohonan_portal)>0){
      ...
      $portal = $this->load->database('default', TRUE);
      $backoffice = $this->load->database('otherdb', TRUE);
      $sql = "select A.*, B.hidden 
              from $portal->database.tmpermohonan_portal A
              INNER JOIN $backoffice->database.tmpemohon_portal B ON A.id = B.id_permohonan_portal
              where A.no_permohonan is null and A.id_pemohon =? order by d_entry desc";
      $permohonan_portal_sementara =	$this->db->query($sql, array($user->id))->result();
    ...
    $sql = "select A.*, B.hidden 
              from $portal->database.tmpermohonan_portal A
              INNER JOIN $backoffice->database.tmpemohon_portal B ON A.id = B.id_permohonan_portal
              where A.no_permohonan is not null and A.id_pemohon =? order by d_entry desc LIMIT 3";
    $permohonan =	$this->db->query($sql, array($user->id))->result();
    if(!empty($permohonan)){
      $ada2 = 1;
      $id2 = array();
      ...
      $array = implode("','",$id2);
      //$sql = "select tmpermohonan.pendaftaran_id as 'nomor_pengajuan',trstspermohonan.n_sts_permohonan_dashboard as 'status'
      ...
      $sql = "select tmpermohonan.pendaftaran_id as 'nomor_pengajuan', tmpermohonan.kd_status, trstspermohonan.n_sts_permohonan_dashboard as 'status'
              from tmpermohonan_trstspermohonan,tmpermohonan,trstspermohonan 
              where tmpermohonan_trstspermohonan.trstspermohonan_id < '13' 
              and tmpermohonan.id = tmpermohonan_trstspermohonan.tmpermohonan_id 
              and tmpermohonan.kd_status = trstspermohonan.id
              and tmpermohonan_trstspermohonan.tmpermohonan_id in
              (select id from tmpermohonan where pendaftaran_id in('".$array."')) LIMIT 3";
      ...
      foreach($permohonan2 as $dat){
        ...
        array_push($kd_status,$dat->nomor_pengajuan,$dat->kd_status);
      }
      $array = implode("','",$id2);
      $portal = $this->load->database('default', TRUE);
      $backoffice = $this->load->database('otherdb', TRUE);
      $sql = "select A.*, B.tglPermohonan from $portal->database.tmpermohonan_portal A 
              INNER JOIN $backoffice->database.tmpemohon_portal B ON  A.id = B.id_permohonan_portal 
              where A.no_permohonan in('".$array."') order by A.d_entry desc LIMIT 3";
  ...
  function datapemohon() {
    ...
    if(!empty($query->kelurahan1)){
      $propinsi1	=	$otherdb->query("select * from pabar_backoffice.trpropinsi where id=".$query->propinsi1." order by n_propinsi asc")->first_row();
      $kabupaten1	=	$otherdb->query("select * from pabar_backoffice.trkabupaten where id=".$query->kabupaten1."")->first_row();
      $kecamatan1	=	$otherdb->query("select * from pabar_backoffice.trkecamatan where id=".$query->kecamatan1."")->first_row();
      $kelurahan1	=	$otherdb->query("select * from pabar_backoffice.trkelurahan where id=".$query->kelurahan1."")->first_row();
    ...
    $propinsi2 = $otherdb->query("select * from pabar_backoffice.trpropinsi where id=".$query->propinsi2." order by n_propinsi asc")->first_row();
    $kabupaten2 = $otherdb->query("select * from pabar_backoffice.trkabupaten where id=".$query->kabupaten2."")->first_row();
    $kecamatan2 = $otherdb->query("select * from pabar_backoffice.trkecamatan where id=".$query->kecamatan2."")->first_row();
    $kelurahan2 = $otherdb->query("select * from pabar_backoffice.trkelurahan where id=".$query->kelurahan2."")->first_row();

$alamat_pemohon  = "";

// cek propinsi
if ($propinsi2) {
    $alamat_pemohon .= $propinsi2->n_propinsi."<br>";
}

// cek kabupaten
if ($kabupaten2) {
    $alamat_pemohon .= $kabupaten2->n_kabupaten."<br>";
}

// cek kecamatan
if ($kecamatan2) {
    $alamat_pemohon .= $kecamatan2->n_kecamatan."<br>";
}

// cek kelurahan
if ($kelurahan2) {
    $alamat_pemohon .= $kelurahan2->n_kelurahan."<br>";
}

// terakhir alamat perusahaan (kalau ada)
if (!empty($query->almtPerusahaan)) {
    $alamat_pemohon .= $query->almtPerusahaan;
}
  ...
  function ubahdatapemohon(){
    ...
    $data['kabupaten3']	= $kabupaten2;
    $data['kecamatan3']	= $kecamatan2;
    $data['kelurahan3']	= $kelurahan2;
	...
	function doeditdata(){
	  ...
	  if($jenis=="pemohon"){
	    ...
	    $no_ktp_perusahaan				=	htmlspecialchars($_POST['no_ktp_perusahaan'],ENT_QUOTES);
	    $no_ktp_direktur		 =	htmlspecialchars($_POST['no_ktp_pemohon'],ENT_QUOTES);
    ...
    // var_dump($jenis);die();
	    ...
	    $data = array("namaPerusahaan" 	=> $nama_pemohon,
	                  ...
	                  "ktpPerusahaan" 	=> $no_ktp_perusahaan,
	  ...
	  if($jenis=="perusahaan"){
	    ...
	    $query			= $this->db->get_where('tm_pemohon', array('emailPerusahaan' => $email_perusahaan, 'username !='=>$username, 'status'=>'1'))->first_row();
  ...
  function doeditdokumen(){
    ...
    if(!empty($_FILES['akta']['name'])){
    	...
    	$akta	= "akta";
    	$akta2	= "akta.".end($temp);
      $dir_dest	= $base_url."pemohon/".$username;
      if (!is_dir($dir_dest)) {
          mkdir($dir_dest, 0775, true); // recursive & izin akses cukup
      }
    	$dir_dest = $dir_dest."/dokumen/";
      if (!is_dir($dir_dest)) {
          mkdir($dir_dest, 0775, true); // recursive & izin akses cukup
      }
  ...
  function permohonan() {
    ...
    foreach($permohonan as $status){
      $permohonan[$no]->status_berkas = '';
      $permohonan[$no]->status = "Online";
      $permohonan[$no]->status2 = "0";
      $permohonan[$no]->obj = $status->lokasi_izin;

      $skm = $otherdb->get_where("skm_data_skm",array('resi'=>$status->no_permohonan))->first_row();

      $permohonan[$no]->skm = 2;
      if (!empty($skm->flag_skm)) {
        $permohonan[$no]->skm = $skm->flag_skm;
      }
      
      if(empty($status->no_permohonan)){
        $belakang = $otherdb->get_where("tmpemohon_portal",array('id_permohonan_portal'=>$status->id))->first_row();
        if(count($belakang)==0){
          $permohonan[$no]->status = "IZIN DITOLAK";
        }
      }else{
        ...
        if(count($databackoffice)==0){
          ...
          $permohonan[$no]->retribusi = $databackoffice->c_status_bayar;
          $permohonan[$no]->approve   = $databackoffice->approve;

          $sqlizin  = "select * from trperizinan where id='".$status->id_perizinan."'";
          $trperizinan = $otherdb->query($sqlizin)->first_row();

          $otherdb->select('*');
          $otherdb->from('trkelompok_perizinan');
          $otherdb->join('trkelompok_perizinan_trperizinan', 'trkelompok_perizinan.id = trkelompok_perizinan_trperizinan.trkelompok_perizinan_id');
          $otherdb->where('trkelompok_perizinan_trperizinan.trperizinan_id', $trperizinan->id);
          $kelompok = $otherdb->get();

          foreach ($kelompok->result() as $kel) {
            $permohonan[$no]->kelompok = $kel->trkelompok_perizinan_id;
          }
          ...
          //if(count($stat)!=0){
            if ($databackoffice->status_berkas == "Izin Ditolak") {
              $permohonan[$no]->status_berkas = $databackoffice->status_berkas;
              $permohonan[$no]->status = "IZIN DITOLAK";
              $permohonan[$no]->status2 = $databackoffice->kd_status;
            } elseif($databackoffice->status_berkas == "Izin Disetujui") {
              $permohonan[$no]->status_berkas = $databackoffice->status_berkas;
              $permohonan[$no]->status = "IZIN DISETUJUI";
              $permohonan[$no]->status2 = $databackoffice->kd_status;
            } else {
              $permohonan[$no]->status_berkas = $databackoffice->status_berkas;
              $permohonan[$no]->status = "proses";
              $permohonan[$no]->status2 = $databackoffice->kd_status;
            }
        ...
        $permohonan[$no]->tglmsk = (!empty($databackoffice->d_terima_berkas) ? $databackoffice->d_terima_berkas : $status->d_entry);					
      }
      // echo $no;
      $no++;
    }
  ...
  function history() {
    ...
    foreach($permohonan as $status){
      ...
      $permohonan[$no]->status = "Online";
      $permohonan[$no]->status2 = "0";
      $permohonan[$no]->gagal = 0;

      if(empty($status->no_permohonan)){
        $belakang = $otherdb->get_where("tmpemohon_portal",array('id_permohonan_portal'=>$status->id))->first_row();
        if(count($belakang)==0){
          $permohonan[$no]->status = "GAGAL (Berkas Persyaratan tidak ter-upload dengan sempurna, silahkan ulangi Permohonan Perizinan)";
          $permohonan[$no]->gagal = 1;
        }
      }else{
        $databackoffice = $otherdb->get_where("tmpermohonan",array("pendaftaran_id"=>$status->no_permohonan))->first_row();
        if(count($databackoffice)==0){
          ...
          $sqlizin  = "select * from trperizinan where id=?";
          $trperizinan = $otherdb->query($sqlizin, array($status->id_perizinan))->first_row();
  ...
  function doreset(){
  	...
  	$username	= $this->session->userdata("username");
  	$pwlama		=	htmlspecialchars($_POST['password_lama']);
  	$pwbaru		=	htmlspecialchars($_POST['password_baru']);
  	$pwkonf		=	htmlspecialchars($_POST['konfirm_password']);
  	$query	= $this->db->get_where('tm_pemohon', array('username' => $username,'status'=>'1'))->first_row();
    if($pwbaru!=$pwkonf){
      $this->session->set_flashdata('error', 'Terjadi Kesalahan, Password Baru dan Konfirmasi Password Tidak Sesuai');
      redirect('main/user/reset', 'refresh');
    }
    //var_dump($query->password);die;
  	if(!empty($query) && (md5($pwlama) == $query->password || password_verify($pwlama, $query->password))){
      $data = array('password'  => password_hash($pwbaru, PASSWORD_DEFAULT));
      $this->db->where('username', $username);
      $this->db->update('tm_pemohon', $data); 
