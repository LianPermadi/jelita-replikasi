       ...
       //function addpermohonanbaru_OLD(){
		...
		///////////////////////// Input permohonan baru
			...
			$sqlsms = "select user.id, user.no_hp, user.email from user 
                       where user.id IN(select user_id from user_user_auth where user_auth_id = 22 )";
			...
			foreach($querysms as $kirimsms){
				...
				$izin_user = $otherdb->query("select * from trperizinan_user where user_id = ". $kirimsms->id . " AND trperizinan_id = ".$id)->first_row();
				...
				if($cekid != ''){
	    			...
	    			if($telp_pemohon != ''){
    		    	    ...
    		    	    $data = array(
		    	        	   ...
		    	        	   'TextDecoded'		=> "BPMPT OnLine NoRef.".$nomor." Tgl:".$tgl_masuk." a/n:".$pemohon." Izin:".$nizin,
	    	    		       // 'CreatorID' 			=> "Gammu",
    	    		    );
