...
class Permohonan extends MY_Controller {
    ...
    function step3($id) {
		...
		// $sql		= "SELECT 
							// trproperty.n_property,trproperty.id 
						// FROM 
							// trproperty,trperizinan_trproperty 
						// WHERE 
							// trproperty.id=trperizinan_trproperty.trproperty_id
							// AND trperizinan_trproperty.trperizinan_id='".$id."'
							// AND trperizinan_trproperty.c_order>0
						// ORDER BY
							// trperizinan_trproperty.c_order";
	...
	function addpermohonanbaru(){
		...
		// $lokasi_izin				=	nl2br(htmlspecialchars($_POST['lokasi_izin'],ENT_QUOTES));
		...
		$jenis				= $user->jenis;
		// $eksis	= $otherdb->query("select * from tmpermohonan_portal order by i_urut desc limit 1")->first_row();
		...
		///////////////////////// Input permohonan baru
			...
			$path2			= "assets/userassets/pemohon/".$user->username."/dokumen_pengajuan/";
			...
			//////////////////  fungsi input persyaratan ////////////////////////////////
				...
				//////////////////////// retrive persyaratan yang sudah ada
					...
					foreach($syarat as $sya){
						...
						array_push($array_syarat,$sya->id_persyaratan);
				...
				////////////////////// Cek type file
					...
					foreach($query as $syarat){
						...
						////////////////////////// cek jika data sudah ada di sistem dan tidak mengupload baru maka ekstensi tidak dicek
						
						// if(in_array($syarat->id,$array_syarat) && empty($_FILES["file_".$syarat->id]["name"])){
				...
				////////////////////// input ke table & upload file
					...
					foreach($query as $syarat){
						...
						// if(empty($_FILES["file_".$syarat->id]["name"])){
							...
							// $syarat_pemohon	=	$this->db->get_where("tm_pemohon_persyaratan", array("id_pemohon"=>$user->id,"id_persyaratan"=>$syarat->id))->first_row();
							
							// $data = array(
							   // 'tmpermohonan_id'		=> $eksis->id,
							   // 'trsyarat_perizinan_id'	=> $syarat->id,
							   // 'nama_file' 					=> $syarat->id.".pdf",
							   // 'nomor_surat'				=> $syarat_pemohon->nomor_surat,
							   // 'tanggal_surat'				=> $syarat_pemohon->tanggal_surat,
							   // 'masa_berlaku_surat'	=> $syarat_pemohon->masa_berlaku_surat
							// );
						...
						if(!empty($_FILES["file_".$syarat->id]["name"]))
						{
							$data = array(
							   'tmpermohonan_id'		=> $eksis->id,
							   'trsyarat_perizinan_id'	=> $syarat->id,
							   // 'nama_file' 					=> $_FILES["file_".$syarat->id]['name'],
							   'nama_file' 					=> $syarat->id.".pdf",
						...
						///////////////////////////// input data persyaratan ke persyaratan permohonan
							...
							///////////////////////////// input data persyaratan ke table dokumen persyaratan yang dimiliki
						
							// $cari	=	$this->db->get_where("tm_pemohon_persyaratan",array("id_persyaratan"=>$syarat->id,"id_pemohon"=>$user->id))->result(); // cek data persyaratan udah punya atau belum
							...
							if(file_exists($path2.$syarat->id.".pdf") && !empty($_FILES["file_".$syarat->id]["name"])){  /////////// cek file udah ada di folder dokumen_pengajuan
								...
								$this->db->where('id_persyaratan', $syarat->id);
								$this->db->where('id_pemohon', $user->id);
								
								$this->db->update('tm_pemohon_persyaratan', $data); 
								
								unlink($path2.$syarat->id.".pdf");
								$lokasi=$_FILES['file_'.$syarat->id]['tmp_name'];
								move_uploaded_file($lokasi,$dir.$file);
							
							}else if(!file_exists($path2.$syarat->id.".pdf") && !empty($_FILES["file_".$syarat->id]["name"])){
							...
							copy($dir.$file , $dir2.$file); ////////// copy file persyaratan di dokumen_persyaratan ke persyaratan pengajuan
							
							//////////////////// upload file pengajuan
							
							
							// $file2	=	$string.".pdf";
							// $lokasi2=$_FILES['file_'.$syarat->id]['tmp_name'];
							// $dir2=$path."/";
							// move_uploaded_file($lokasi2,$dir2.$file2);
			...
			$data	= array(
			   ...
			   'faxPerusahaan'				=> $user->faxPerusahaan,
			   'almtPerusahaan'				=> $user->almtPerusahaan,
			   'telpPerusahaan'				=> $user->telpPerusahaan,
			   'tglPermohonan'				=> date("Y-m-d"),
			   'propinsi2'							=> $user->propinsi2,
			   'kabupaten2'					=> $user->kabupaten2,
			   'kecamatan2'					=> $user->kecamatan2,
		...
		if($permohonan->editable=="1" && count($belakang)>0){
			...
			$sql 		=	"select trsyarat_perizinan.* 
								...
								where trsyarat_perizinan.id IN(select 
									trsyarat_perizinan_id 
									from trperizinan_trsyarat_perizinan 
									where trperizinan_id='".$permohonan->id_perizinan."')";
			$query 	= $otherdb->query($sql)->result();
