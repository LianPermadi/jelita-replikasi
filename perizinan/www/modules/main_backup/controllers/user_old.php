...
class User extends MY_Controller {
    ...
    function index() {
		...
		$ada2	= 0;
		/*
		$permohonan_portal	=	$this->db->order_by("id","desc");
		$permohonan_portal	=	$this->db->get_where("tmpermohonan_portal",array("id_pemohon"=>$user->id))->first_row();
		
		$total_permohonan	=	count($permohonan_portal);
		
		if($total_permohonan>0){
			
			$sql 	= "select * from trperizinan where id='".$permohonan_portal->id_perizinan."'";
			$nama	= $otherdb->query($sql)->first_row();
			
			$status	= "Online";
			$track = 0;
			
			if(!empty($permohonan_portal->no_permohonan)){
			
				$databackoffice	= $otherdb->get_where("tmpermohonan",array("pendaftaran_id"=>$permohonan_portal->no_permohonan))->first_row();
				...
				if(count($stat)!=0){
					...
					$status = $stat->n_sts_permohonan;
					
				}	
				
				$track	=	$otherdb->order_by("id","desc");
				$track	=	$otherdb->get_where("tmtrackingperizinan", array("pendaftaran_id"=>$permohonan_portal->no_permohonan))->result();
		...
		*/
		...
		// get data permohonan dari portal
		$permohonan_portal =	$otherdb->query("select id_permohonan_portal from tmpemohon_portal where id_pemohon=".$user->id."")->result();
		//
		...
		foreach($permohonan_portal as $dat){
			...
			array_push($id,$dat->id_permohonan_portal);
		...
		// $this->db->select("*"); 
		// $this->db->from("tmpermohonan_portal"); 
		// $this->db->where("no_permohonan is null"); 
		// $this->db->where_in("id",$array); 
		// $permohonan_portal_sementara = $this->db->get();
		
		// print_r($permohonan_portal_sementara);
		// die;
		
		if(count($permohonan_portal_sementara)>0){
		...
		$sql = "select * from tmpermohonan_portal where no_permohonan is not null and id_pemohon=".$user->id."";
		...
		if(count($permohonan)>0){
			...
			// $sql = "select id from tmpermohonan where pendaftaran_id in('".$array."') ";
			// $permohonan2 =	$otherdb->query($sql)->result();
			
			$sql = "select tmpermohonan.pendaftaran_id as 'nomor_pengajuan',trstspermohonan.n_sts_permohonan as 'status'
			...
			foreach($permohonan2 as $dat){
				...
				// $stat = array(
					// 'nomor_pengajuan' => $dat->nomor_pengajuan,
					// 'status' => $dat->status
				// );
			...
			// print_r($status);
			// $key = array_search('0006400201062015001', $status);
			
			$array = implode("','",$id2);
			
			
			$sql = "select * from tmpermohonan_portal where no_permohonan in('".$array."')";
		...
		// $data['load']				= "user/dashboard";
		$data['load']					= "user/status_proses";
		// $data['permohonan']			= $permohonan_portal;
		$data['ada']					= $ada;
		$data['ada2']					= $ada2;
	...
	function doeditdata(){
		...
		// if($jenis=="perusahaan"){
			...
			$no_ktp_pemegang_kuasa		=	htmlspecialchars($_POST['no_ktp_pemegang_kuasa'],ENT_QUOTES);
			$no_npwp_pemohon				=	htmlspecialchars($_POST['no_npwp_pemohon'],ENT_QUOTES);
			$no_akta_pemohon				=	htmlspecialchars($_POST['no_akta_pemohon'],ENT_QUOTES);
			...
			$alamat_pemegang_kuasa		=	nl2br(htmlspecialchars($_POST['alamat_pemegang_kuasa'],ENT_QUOTES));
			...
			$email_pemegang_kuasa		=	htmlspecialchars($_POST['email_pemegang_kuasa'],ENT_QUOTES);
			...
			$fax_pemohon						=	htmlspecialchars($_POST['fax_pemohon'],ENT_QUOTES);
	...
	function doeditdokumen(){
		...
		if(!empty($_FILES['akta']['name'])){
			...
			$handle	= new Upload($_FILES['akta']);
			
			if ($handle->uploaded) {

				$dir_dest	= $base_url."pemohon/".$username."/dokumen/";
				
				$handle->file_new_name_body	= $akta;
				...
				$this->session->set_flashdata('error', 'Terjadi Kesalahan, mohon mengulangi proses upload dokumen akta');
				redirect('main/user/ubahdokumenpemohon', 'refresh');
				
			}
			
			$data	= array(
			   "scan_akta_pemohon"	=> $akta2
			);
		...
		foreach($permohonan as $status){
				...
				if(empty($status->no_permohonan)){
					...
					if(count($stat)!=0){
						...
						$permohonan[$no]->status = $stat->n_sts_permohonan;
    ...
    // function permohonanbaru() {
		
		// $data['title'] = "Permohonan Perizinan";
		// $data['load'] = "user/permohonanbaru";
        // $this->load->view('template_user',$data);
    // }
}
